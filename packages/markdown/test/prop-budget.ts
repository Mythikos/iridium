 ...
 * not import: `@iridium/testkit` carries the boundary tag `node` while `@iridium/markdown` is `iso`, and
...
// `@iridium/markdown` compiles without `node` types (it must stay isomorphic), so the environment is
 ...
 * each name unmatched, each mutant untested and each mutant reported as survived. Without a fixed seed
 * every mutant that only a property file covers survives. A malformed value is fatal rather than silently
