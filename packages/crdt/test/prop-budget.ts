 ...
 * not import: `@iridium/testkit` carries the boundary tag `node` while `@iridium/crdt` is `iso`, and
 * a spec file may import the testkit only from `apps/server`, `apps/e2e` or `tooling/mutation`. The
 * env keys and the defaults are therefore mirrored here — deliberately, and only here, so that the
 * package still has exactly one definition of its budget and `IRIDIUM_PROP_RUNS` still raises it.
 */
import { VerbosityLevel, type SizeForArbitrary } from 'fast-check';

// `@iridium/crdt` compiles without `node` types (it must stay isomorphic), so the environment is
// read through `globalThis` rather than through a `process` the package is not allowed to know.
 ...
 * each name unmatched, each mutant untested and each mutant reported as survived. This package is
 * the one that proves it: `src/dominates.ts` is covered by `dominates.prop.spec.ts` alone, so
 * without a fixed seed every mutant in it survives. A malformed value is fatal rather than silently
 * ignored, because the failure it causes is a plausible-looking mutation score rather than an error.
 * Unset, the seed stays fresh per run, which is what the ordinary lanes want.
 */
