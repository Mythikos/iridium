...
The MVP deployment is one server container, one MySQL 9.7 LTS instance, one persistent attachment volume, and one TLS-terminating reverse proxy. There is exactly one server process per deployment: it owns every active Y.Doc, the in-process `TicketStore`, `AuthzBus`, rate-limit counters, and the per-vault structural mutex (skeleton A17, A23, A24, F9). Horizontal scaling is a documented non-goal for the MVP; every in-process singleton sits behind an interface so the Redis phase in the post-MVP roadmap does not change this topology's contract.

```mermaid
flowchart TB
  subgraph Host["Docker host or VM (infra/compose.prod.yaml)"]
    direction TB
    CADDY["caddy (resolved 2.x tag + digest)<br/>TLS 1.2+ termination, ACME or provided certs or `tls internal`<br/>(no security headers — the app emits them)<br/>reverse_proxy → server:4000<br/>WebSocket passthrough on /collab<br/>flush_interval -1 on /mcp<br/>/metrics only from the monitoring CIDR, else 404"]
    SRV["server: ghcr.io/&lt;org&gt;/iridium-server:&lt;version&gt;<br/>Node 24.21.0, non-root, read_only rootfs, cap_drop ALL<br/>REST /api/v1 · WS /collab · MCP /mcp · /healthz /readyz /metrics<br/>in-process jobs · piscina projection workers"]
    ...
    DB[("mysql:9.7.2-oraclelinux9<br/>my.cnf baked before migration 0001<br/>init/01_roles.sh → iridium_app, iridium_migrator, iridium_backup<br/>volume mysql-data · binlog retained 7 d")]
  ...
  WEB["Browser (cookie session)"] -->|HTTPS| CADDY
  DESK["Electron main (bearer session)"] -->|HTTPS + WSS| CADDY
  AGENT["MCP clients (Bearer irid_pat_…)"] -->|HTTPS| CADDY
...
| Outbound network inventory: the server makes no outbound connections except to MySQL and, when `ATTACHMENTS_DRIVER=s3`, to the configured S3 endpoint. Caddy contacts the ACME directory only in the public `Caddyfile` variant; `Caddyfile.internal` (`tls internal`) makes no outbound connection at all. No telemetry, no update checks from the server (desktop clients poll the server's own `/desktop/updates/*`). | `docs/ops/deployment.md` (published as the "what leaves the box" statement) |
...
| Service | Definition |
|---|---|
| `mysql` | `image: mysql:9.7.2-oraclelinux9` (never `latest`, which is the innovation line; never `9.7` floating because a repo bug once silently upgraded 8.4 hosts to 9.7 — pin the patch), `command` omitted (all settings come from the mounted `infra/docker/mysql/my.cnf`), `volumes: [mysql-data:/var/lib/mysql, ./docker/mysql/my.cnf:/etc/mysql/conf.d/iridium.cnf:ro, ./docker/mysql/init:/docker-entrypoint-initdb.d:ro]`, `environment: MYSQL_ROOT_PASSWORD_FILE=/run/secrets/mysql_root_password, MYSQL_DATABASE=iridium`, `ports: ["127.0.0.1:3306:3306"]`, `healthcheck: {test: ["CMD", "mysqladmin", "ping", "-h", "127.0.0.1", "--silent"], interval: 5s, timeout: 3s, retries: 20}` |
...
The default developer loop is `docker compose up -d mysql` → `pnpm install` → `pnpm turbo run dev`; Testcontainers (10-testing-and-quality.md) starts its own `mysql:9.7.2` for integration tests and does not depend on this compose file.
...
services:
  mysql:
    image: mysql:9.7.2-oraclelinux9
...
{$IRIDIUM_SITE} {                  # typically `:443` or an internal name; ACME is never attempted
  ...
  # … the @collab, @mcp, handle /metrics and catch-all blocks are byte-identical to Caddyfile
}
```
...
| `build` | `base` | `COPY --from=prune /app/out/json/ .`, `COPY --from=prune /app/out/pnpm-lock.yaml ./`, `COPY --from=prune /app/out/pnpm-workspace.yaml ./` → `RUN --mount=type=cache,id=pnpm,target=/pnpm/store pnpm install --frozen-lockfile` (the pruned lockfile must install cleanly; CI runs this stage on every PR to catch the turbo-prune regression class) → `COPY --from=prune /app/out/full/ .` → `pnpm turbo run build --filter=@iridium/server... --filter=@iridium/web` (tsdown bundles the server with every `@iridium/*` workspace package inlined into `dist/main.mjs`; Vite 8 builds the web bundle) → `pnpm deploy --filter=@iridium/server --prod /prod/server` (third-party production dependencies only; `@node-rs/argon2` resolves to its prebuilt `linux-x64-gnu`/`linux-arm64-gnu` binary — no toolchain is installed in any stage, and the build fails if a native build script is attempted because `allowBuilds` does not list it) |
| `runtime` | `node:24.21.0-bookworm-slim` | `apt-get install` of `ca-certificates`, `tini`, and the MySQL 9.7 client tools from the official MySQL APT repository (`mysql-community-client-core`, pinned to the `9.7.2` package revision chosen at M0, repository GPG key verified) — this provides `mysqldump`, `mysql`, and `mysqlbinlog` for the CLI; `groupadd -g 10001 iridium && useradd -u 10001 -g iridium -M iridium`; `COPY --from=build --chown=10001:10001 /prod/server/dist /app/dist`, `/prod/server/node_modules /app/node_modules`, `/prod/server/package.json /app/package.json`, `/prod/server/migrations /app/migrations`, `/prod/server/web /app/web` (the built SPA); `COPY infra/docker/iridium /usr/local/bin/iridium` (a 3-line shell shim: `exec node --enable-source-maps /app/dist/main.mjs "$@"`); `ENV NODE_ENV=production IRIDIUM_WEB_DIR=/app/web`; `USER 10001:10001`; `WORKDIR /app`; `HEALTHCHECK --interval=15s --timeout=5s --start-period=30s --retries=3 CMD ["node", "/app/dist/healthcheck.mjs"]`; `ENTRYPOINT ["tini", "--", "node", "--enable-source-maps", "/app/dist/main.mjs"]`; `CMD ["serve"]` |
...
| Image runs as UID 10001 with a read-only rootfs and starts to `/readyz` green against `mysql:9.7.2` and `mysql:8.4.11` | `ci.yml › integration` (`compose up` smoke), `nightly.yml › mysql-8.4-lane` | privilege or writable-path regression |
| `iridium --version` inside the image equals the Changesets version and the `iridium_build_info` metric | `release.yml` | version drift between image, web bundle, installers, bridge |
...
Changesets keeps one product version across the server image, the web bundle, the desktop installers, and `iridium-mcp` (skeleton A1, A54). Image tags: `1.2.3` (immutable), `1.2` and `1` (floating, updated by `release.yml`), never `latest`. The desktop update feed and `GET /meta {serverVersion, apiVersion, minClientVersion}` report the same version string, and `iridium_build_info{version, commit, node}` exposes it to Prometheus so a dashboard can show which version each environment runs.
...
- `IRIDIUM_FAULT` and `IRIDIUM_E2E` are refused when `NODE_ENV=production` (`config.test_knob_in_production`).
...
| `PORT` | int, `4000` | |
| `PUBLIC_ORIGIN` | URL, **required**; `https:` unless `NODE_ENV=development` | cookie flags, WS Origin allowlist, CSRF host, MCP `resource`, set-password links, desktop feed URLs |
...
| `UV_THREADPOOL_SIZE` | int, `8` | set explicitly by `main.ts` before any I/O if absent (argon2 + fs) |
...
| `ATTACHMENTS_ENCRYPTION` | `none` only in MVP | `aes256gcm` and the reserved keyring `ATTACHMENT_KEY_V<n>[_FILE]` are refused with `config.not_implemented` until §G-4 is decided |
| **Authentication policy floors** | | |
| `ARGON2_MEMORY_KIB`, `ARGON2_TIME_COST` | int, `65536`, `3` | calibrated by `iridium doctor --argon2` |
...
| `WS_MAX_PAYLOAD_BYTES` | `2097152` | `@fastify/websocket` `maxPayload` |
...
| `UPDATE_LOG_RETENTION_DAYS` | `7` | prune of compacted `note_updates` |
...
| `MCP_OAUTH_ENABLED` | bool, `false` | post-MVP seam; `true` is refused in MVP builds |
| `MCP_RATE_LIMIT_PER_HOUR`, `MCP_RATE_LIMIT_BURST_PER_MIN`, `MCP_PROCESS_CEILING_PER_MIN` | `3000`, `120`, `600` | |
| **Retention and jobs** | | |
...
| **Test-only** | | |
| `IRIDIUM_FAULT`, `IRIDIUM_E2E` | refused in production | fault registry (10-testing-and-quality.md) |
...
| Attachment key | `iridium keys rotate attachment` | Reserved for §G-4; the command exists and exits `3` with `not_implemented` in MVP | — |
...
Three lines are deliberately **absent**, because each would be a startup or a naming failure rather than a tuning choice:

- `default_authentication_plugin` — deprecated in 8.0.27 and **removed in 8.4.0**. An unknown variable in `my.cnf` makes `mysqld` exit at startup, and this same file is mounted into the dev compose, `compose.prod.yaml`, the Testcontainers fixture, and the `mysql:8.4.11` nightly lane, so the database would never come up — before migration `0001`, before any drill. `authentication_policy` above is the supported spelling on both 8.4 and 9.x, and `init/01_roles.sh` already writes `IDENTIFIED WITH caching_sha2_password` per user.
...
`ops.mysql-config.spec.ts` boots the shipped file on `mysql:9.7.2` and `mysql:8.4.11` and asserts that the server starts, that no variable is rejected, and that `SHOW BINARY LOGS` reports `binlog.000001`.

Optional and documented, not baked: InnoDB tablespace encryption via `component_keyring_file` (Community Edition on 8.4 and 9.7; the keyring must be loaded early, so it is a `[mysqld]` `early-plugin-load`/component manifest change made before the first data write) — `docs/ops/security.md` gives the exact steps and states that the backup dump is plaintext regardless (encrypt the backup target). The CJK `ngram_token_size` line is added only if §G-5 is answered yes.
...
-- BACKUP_ADMIN: from MySQL 8.0.21, --single-transaction together with --source-data makes
...
Table-level grants require the tables to exist, which is why the app role's rights are a migration rather than init SQL. In deployments where the DBA will not give the migrator `GRANT OPTION`, `0034_grants` detects the missing privilege (`SHOW GRANTS FOR CURRENT_USER()`), records itself as applied with `metadata.skipped='no_grant_option'`, and `iridium migrate grants --print` emits the exact `GRANT` statements for the DBA; `/readyz` then reports `grants: unverified` until `iridium doctor --db-roles` confirms the effective privileges.
...
- Long-running migrations (table rebuilds) carry a leading comment `-- iridium: long-running`; `migrate status` shows them as `[long-running]`, `migrate up` refuses them without `--allow-long-running`, and the boot path never applies them (readiness reports `migrations: pending (operator action required)`), so pulling an image can never take a site down for an unbounded rebuild without an operator deciding when.
...
- CI: every PR runs the full migration set on `mysql:9.7.2`, then `kysely-codegen 0.20.0` and diffs against `apps/server/src/db/schema.ts`; the nightly lane repeats on `mysql:8.4.11`. A migration that only works on 9.x fails the nightly lane (all SQL stays 8.0.13-compatible while §G-3 stands).
...
| Projection | `projection.completed`, `projection.timeout`, `projection.invalid_content`, `projection.reindex.progress` | info / warn |
...
| Jobs and transfers | `job.started`, `job.succeeded`, `job.failed`, `import.scanned`, `import.committed`, `export.completed` | info |
...
| `iridium_tokens_active` | gauge | — | `access_tokens` not revoked and not expired, sampled every 60 s |
...
| `grants` | `0034_grants` applied and not marked skipped, or `doctor --db-roles` last verified them | applied with `metadata.skipped='no_grant_option'` | app role demonstrably lacks `INSERT` on `audit_events` (probed once at boot with a rolled-back transaction) |
...
`readyz.integration.test.ts` drives every row: a pending migration, a killed pool (Toxiproxy), a read-only attachment directory, `innodb_flush_log_at_trx_commit=2` with and without `READYZ_STRICT_DURABILITY`, an injected 40 s writer backlog (`IRIDIUM_FAULT=store.slow:45000`), a budget driven to 80 % and then to 100 % of a lowered `COLLAB_MAX_LOADED_DOCS` (`warn`, then `fail` with the whole response `503`), a clock offset, a missing pepper version, and an `access_log` table whose newest partition boundary is 10 days ahead (`warn`, and the whole response still `200` — the case that proves this check can never close the door). It also asserts that the check-name set equals `ReadyzCheckName` in 09-api-reference.md, so the fourteen names in this table, the `iridium_readyz_check_status{check}` label, and the alert expressions cannot drift apart.
...
| 2a | — | reads `manifest.json`, re-hashes `dump.sql.zst`, refuses on mismatch (`restore.dump_hash_mismatch`, exit `4`); compares `manifest.mysql_settings` with the target's `SHOW GLOBAL VARIABLES` and refuses on an index-, collation-, or packet-affecting difference (`restore.settings_drift`, exit `4`, overridable with `--accept-settings-drift`) | the remedy printed is the exact `my.cnf` lines, because those settings are fixed at FULLTEXT-index-build and table-creation time and cannot be corrected after the load |
...
4. Start a **second, empty** `MySqlContainer` from the same image selector, run the roles script, and `iridium restore --from <tmp> --verify --verify-all`.
...
One product version covers the server image, the web bundle inside it, the desktop installers, and `iridium-mcp` (A1, A54). `release.yml` publishes, for tag `v1.2.3`:
...
| Desktop installers | GitHub release assets **and** the update feed directory published to the server with `iridium desktop-updates publish <dir>` | nsis (assisted, `perMachine`) + msi (Windows), dmg + zip (macOS, notarised), AppImage + deb + rpm (Linux) |
| `latest.yml` / `latest-mac.yml` / `latest-linux.yml` + blockmaps | `DESKTOP_UPDATES_DIR/<channel>/`, served at `/desktop/updates/<channel>/` | electron-updater generic provider |
| `iridium-mcp` bundle | inside the desktop app's `resources/bin/` and downloadable from `/desktop/tools/` | §G-7 decides npm publication |
| Changelog | Changesets-generated `CHANGELOG.md` + the GitHub release body | carries the operator flags below |

### Release-note flags an operator must read

The changeset template requires one of these tags on every change that affects deployment, and `release.yml` fails if a release contains a migration without a `[migration]` flag in the changelog (a grep over the generated changelog versus the diff of `apps/server/migrations/`):
...
| `[api]` | `apiVersion` increments, `minClientVersion` rises | desktop clients below `minClientVersion` show "update required"; plan the fleet update per `desktop_update_policy` |
...
| 14 | If the release has `[api]`: publish the desktop installers (`iridium desktop-updates publish`) and confirm `GET <PUBLIC_ORIGIN>/api/v1/desktop/update-policy` — the absolute path, because it is an `/api/v1` route despite serving the desktop host, unlike the static `/desktop/updates/*` feed (09-api-reference.md §2.16) — returns the intended `{policy, channel, minVersion}` | clients prompt or silently update per policy | a `minClientVersion` bump without a published installer strands the fleet — `release.yml` refuses that combination |
...
- **Desktop clients do not roll back automatically.** A client newer than the rolled-back server still speaks `apiVersion` N while the server speaks N-1; the N/N-1 window means that works for one cycle. If the rollback crosses two `apiVersion` steps, the update feed must be reverted too (`iridium desktop-updates publish --channel stable --allow-downgrade --yes <previous dir>`, which rewrites `latest*.yml`; electron-updater will not downgrade an installed app, so affected users reinstall from the previous installer — this is called out in the release notes of any release that increments `apiVersion`).
...
| Electron | Adopt a new major while the running major is still inside the supported three-version window, so an adoption is never forced by an expiring version; the upgrade is a normal release whose gate is the end-to-end update test. `electron-builder` 27 is its own post-MVP milestone with the same gate |
| MySQL | Patch releases applied on the site's own schedule; LTS-to-LTS hops only (9.7 → the next LTS), never into an innovation release; the 8.4 nightly lane keeps the SQL portable while §G-3 stands |
...
| Disk | `mysql-data` (dump-proportional), `attachments-data` (user uploads), `staging-data` (imports, cleaned by `transfer_cleanup`), `exports-data` (24 h TTL), `updates-data` (installers) | `iridium_storage_free_bytes` + the `IridiumDiskLow`/`IridiumDiskCritical` alerts |
...
| `durable_ack_ms` p95 | < 1 s | fsync latency or `DB_POOL_PERSIST` contention; this is the "Saved" latency users feel |
...
| `iridium keys rotate pepper\|audit\|cursor\|attachment --secrets-dir <d>` | none (writes files only, no database access, no role URL) | generates the next version, writes it `0400`, prints the one `<NAME>_V<n+1>_FILE` env line to add; idempotent-refusing (exit `3` onto an existing file); `attachment` exits `3` with `not_implemented` while §G-4 stands. It does **not** put the key in charge — `keys promote` does |
...
| Access review | `iridium tokens list --unused-for-days 30`, `iridium access-log export --token <id> --from --to`, and the admin console's per-token agent-activity view (07-client-applications.md) |
...
`docs/compliance-checklist.md` is the shipped form of this table, generated at M7/M8 with each evidence cell hyperlinked to the test file, workflow job, or document that proves it. It exists because security reviews and procurement questionnaires ask for a control map, and because every control in Iridium already has a named test — the checklist is an index, not a promise. The threat model it pairs with is a different table and lives elsewhere: the `T1`–`T17` rows are **04-auth-and-access-control.md §12**, which is what every `T<n>` citation in the plan resolves to (14-risks-and-open-questions.md's identifier registry and D14-01 fix that), and `docs/threat-model.md` is the shipped rendering of that section. 14-risks-and-open-questions.md carries no threat summary — its `R-T<n>` rows are a separate namespace (`T16` is prompt injection through note content; `R-T16` is the single-process ceiling).

The rule for this table: a control row may only cite evidence that is an **automated check** (a test name or a CI job) or a **shipped artefact** (a config file, a document, a signed release asset). "We reviewed it" is not evidence.
...
| C8 | Session management | 32-byte CSPRNG secrets, SHA-256 at rest, id-embedded lookup; `__Host-` cookie `Secure; HttpOnly; SameSite=Lax` for web; bearer held only by the Electron main process for desktop; idle + absolute expiry; new row per login; password change revokes other sessions; logout deletes the row and sends `Clear-Site-Data` | `sessions.integration`, `desktop.attachments-no-token-in-renderer`, `preload-surface` |
...
| C16 | Agent credentials cannot be minted by an operator | the CLI has no token-create command; creation is a user action with step-up | `iridium --help --json` inventory contract (`cli.contract.spec`) |
| C17 | Leaked-credential detection | published scanner regex `irid_(pat\|ses\|tkt\|spl)_[0-9A-Za-z]{16}_[0-9A-Za-z]{49}` with a CRC check digit, documented in `SECURITY.md` for the site's secret-scanning tooling | `tokens.format.unit`, `logging-redaction` |
...
| C28 | Encryption at rest | documented: encrypted volumes for `mysql-data` and `attachments-data`, plus MySQL InnoDB tablespace encryption via `component_keyring_file` with exact steps; application-level attachment envelope encryption is designed, with columns reserved, pending §G-4 | `docs/ops/security.md`, `attachments` schema columns, §G-4 |
| C29 | Secrets management | every secret has a `<NAME>_FILE` twin; files must not be world-readable; versioned peppers, audit keys, and cursor keys; rotation commands; boot refusal on a key-version downgrade; redaction in logs and in the config summary | `config.unit`, `logging-redaction`, `ops.key-rotation.drill.spec` |
...
| C31 | Code integrity of the desktop client | Azure Trusted Signing with a fixed `publisherName` verified by `electron-updater` on every future update, macOS Developer ID + notarisation, `enableEmbeddedAsarIntegrityValidation` and `onlyLoadAppFromAsar` fuses | `release.yml` packaged smoke, `desktop.update-check-local-feed`, §G-8 (identity ownership) |

### Durability, integrity, and recovery

| # | Control | Implementation | Evidence |
|---|---|---|---|
| C32 | "Saved" means durably saved | a COMMIT containing the user's update plus a broadcast state vector that dominates the client's whole local vector; `innodb_flush_log_at_trx_commit=1` and `sync_binlog=1` enforced by readiness | `collab.durable-ack.chaos` (kill-after-ack ×20 in CI, ×200 nightly, plus `store.throw`, crash-before-commit, crash-after-commit-before-ack, slow DB), `persistence.model.prop`; acceptance row "Durable saving" |
...
| C39 | Change management | forward-only migrations, one DDL per file with idempotent guards, `GET_LOCK` serialisation, expand/contract, `[migration]`/`[long-running]`/`[config]` release flags, fail-closed readiness, signed releases with one version across all artefacts | `migrations.integration` (9.7.2 + 8.4.11), `readyz.integration`, `ci.yml › gen-drift`, `release.yml` |
| C40 | Data retention and deletion | trash retention, checkpoint thinning, update-log pruning, access-log partition drop, audit archive (export before move, never silent deletion), user anonymisation rather than row deletion | `retention-jobs.integration`, `iridium_job_last_success_timestamp` per job |
...
| C43 | Health and readiness | `/healthz` (liveness, no DB) and `/readyz` (fail-closed checklist of fourteen checks) | `readyz.integration` |
| C44 | Monitoring and alerting | prom-client catalogue, Grafana dashboard JSON, a Prometheus rule file with a runbook per rule, a shipped `prometheus-scrape.yml` plus the IP-restricted `/metrics` route that makes a scrape possible at all, and for sites without Prometheus `GET /admin/system` (including `storage.volumes[*].freeBytes`) plus `iridium doctor --alerts` over the `iridium_doctor: instant` rule subset | `ops.alert-rules.spec` (promtool + one unit test per rule + the annotation/CLI-coverage assertions), `nightly.yml › compose.prod clean-VM boot` (the three `/metrics` reachability cases) |
...
| C48 | Data residency and egress | self-hosted by definition; the server makes no outbound connection except MySQL and, when configured, S3; no telemetry; desktop update checks go to the site's own server | the "what leaves the box" statement in `docs/ops/deployment.md`; `nightly.yml › compose.prod clean-VM boot` runs with an egress-denying network and still passes — it boots the committed `infra/caddy/Caddyfile.internal` (`tls internal`, Caddy's local CA, `IRIDIUM_SITE=:443`), which is the variant that needs neither the ACME directory nor a resolvable public name |
| C49 | Accessibility | keyboard-complete tree, tabs, palette; axe-core checks in component tests; high-contrast and reduced-motion support; all strings in a typed table | component suite axe assertions, `packages/ui/src/i18n/en.ts` |
| C50 | Documentation | `docs/ops/{deployment,configuration,backup-restore,upgrade,security,audit-log,mcp-clients}.md`, `docs/runbooks/*`, `docs/agents/*`, `docs/adr/*`, `docs/threat-model.md`, `docs/compliance-checklist.md`, `SECURITY.md` | M8 exit criteria; the CLI inventory contract test keeps `runbooks/cli.md` truthful |
...
| OAuth 2.1 for MCP clients that require it (claude.ai / Claude Desktop custom connectors) | §G-1 | the verifier dispatches on token prefix; PRM, `resource_metadata`, CIMD, and consent are an additive milestone behind `MCP_OAUTH_ENABLED` with no URL changes |
...
| `updates-data` | accumulated desktop installers across releases | `ls DESKTOP_UPDATES_DIR/<channel>` |

**Resolution**, in increasing order of consequence — stop as soon as the alert clears:

1. **Reclaim the cheap, safe space first**: `iridium jobs run transfer_cleanup`, then `iridium jobs run update_log_prune`, then `iridium jobs run access_log_partitions` (drops partitions past retention — instant), then delete superseded installers from `updates-data` (keep the current and previous release).
...
| OPS-04 | The MySQL 9.7 client tools (`mysqldump`, `mysql`, `mysqlbinlog`) are installed in the runtime image, pinned to the `9.7.2` package revision, with the repository key verified; `release.yml` asserts `mysqldump --version` reports `9.7.x` | `iridium backup`/`restore` are part of the product, not a separate toolbox the operator must assemble; a version mismatch between dump tool and server is a classic silent-corruption source |
...
| OPS-11 | Roles are created by `infra/docker/mysql/init/01_roles.sh` (a shell script reading the three `*_PASSWORD_FILE` secrets) — **this replaces the skeleton A48 artefact name `init/01_roles.sql`**, because the official image expands nothing inside a `.sql` file, so the three passwords would have to be literals in a checked-in file; the `.sh` form is the only variant that can read the mounted secrets, and every reference in this section (the topology diagram, the compose environment comments, the restore and backup runbooks) uses the `.sh` name — while the app role's **table** grants live in migration `0034_grants`. When the migrator lacks `GRANT OPTION`, `0034_grants` records itself applied with `metadata.skipped='no_grant_option'`, `iridium migrate grants --print` emits the statements for a DBA, and `/readyz` reports `grants: unverified` until `doctor --db-roles` confirms them | Table grants require the tables to exist, so they cannot be init SQL. A script (not `.sql`) is the only way the official image can read password files instead of embedding passwords. Enterprise DBAs frequently withhold `GRANT OPTION`, and that must not block deployment |
...
| OPS-43 | `infra/docker/mysql/my.cnf` **omits** `default_authentication_plugin` (removed in MySQL 8.4.0; an unknown variable makes `mysqld` exit) in favour of `authentication_policy = caching_sha2_password`, omits `ft_min_word_len` (MyISAM-only; `innodb_ft_min_token_size` governs InnoDB FULLTEXT), and writes `log_bin = binlog` rather than `log_bin = ON` (the argument is the log base name). `ops.mysql-config.spec.ts` boots the shipped file on 9.7.2 and 8.4.11 and asserts `SHOW BINARY LOGS` reports `binlog.000001` | These are corrections to the baked-settings list of skeleton A9 / decision-log ADR 0009, not new tuning: the file is mounted into the dev compose, `compose.prod.yaml`, the Testcontainers fixture, and the 8.4 nightly lane, so a removed variable means the database never starts and a wrong base name breaks every `binlog.NNNNNN` path in the manifest and the PITR runbook |
