...
**Status.** Accepted (2026-09-11); the `integration` project's single MySQL image is **superseded in part by A59 (2026-09-12)**: the project runs as a two-entry matrix over `mysql:8.4.11` and `mysql:9.7.2-oraclelinux9`, with an unset `IRIDIUM_MYSQL_IMAGE` resolving to the floor. **ADR file.** `docs/adr/0051-testing-harnesses.md`.
...
**Status.** Accepted (2026-09-11); the packaging, signing and in-application-update clauses are **superseded in part by the owner's answer to G8 (2026-09-12)**, marked inline in the Decision. The hardening, the fuses, the privileged scheme, the IPC discipline, the TLS posture and main-only credential custody stand unchanged. **ADR file.** `docs/adr/0053-electron-shell.md`.
...
The ADRs above record the decisions the plan is founded on. While writing the sections, their authors also had to settle 430 narrower questions that the decision skeleton did not reach: a column width, a lock order, a message name, a naming convention. Each is recorded in full, with its rationale, in the "Decisions made in this section" table at the end of the file named below. This index exists so that a decision can be found from its identifier alone, and so that no section-level decision is invisible from here.
...
| OPS-04 | `11-operations-and-deployment.md` | The runtime image ships one set of MySQL client tools (`mysqldump`, `mysql`, `mysqlbinlog`), the `9.7.2` package revision from the official MySQL APT repository with the repository key verified, an… |
...
| OPS-24 | `11-operations-and-deployment.md` | The readiness checklist adds `mysql_version` (OPS-62), `grants`, `key_versions`, `projection_workers`, `tls_cert`, and `access_log_partitions` (03 D03-03 / I-20) to the skeleton's set — fifteen che… |
...
Per file: 01 14, 02 29, 03 24, 04 32, 05 28, 06 36, 07 48, 08 33, 09 31, 10 42, 11 64, 12 19, 13 15, 14 15.
