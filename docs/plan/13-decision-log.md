...
**Amended 2026-09-23.** The full profiles run at **250** examples, not 500. The 500-example count had never completed: the administrator profile latched on a fixture-authentication defect within ninety seconds of starting, so its real cost was first measured only after that was fixed. Measured on one machine against the same deployment, the administrator profile takes 7.5 min at 100 examples, 14.0 min at 250 and **127.7 min at 500** — nine times the cost for twice the examples, because by then the deployment holds the hundred thousand rows the run itself created and every later request pays for them. The coverage does not follow that curve: 100 examples found 2 of the 9 findings, while 250 found 8 of them plus one the 500-example run missed. A GitHub-hosted job is killed at six hours, so 500 examples for both profiles is not coverage that can complete; 250 is the count that keeps the findings and fits.
