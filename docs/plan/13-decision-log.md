...
**Status.** Accepted (2026-09-11); **amended 2026-09-13**: the isolation pattern applies to a second consumer of the TypeScript compiler API, `openapi-typescript` 7.13.0, whose type printer calls `ts.factory` and therefore fails under TypeScript 7.0.2 (found when `pnpm gen` step 3 first ran at M0). It is declared by a second leaf package, `tooling/api-codegen` (`@iridium/api-codegen`), with its own `"typescript": "npm:@typescript/typescript6@6.0.2"` and no sources; `peerDependencyRules.allowedVersions` in `pnpm-workspace.yaml` accepts TypeScript 6 for the `openapi-typescript>typescript` peer only, and the `mutation` named catalog carries the Vitest 4.1.11 companions `tooling/mutation` declares, so that lane's peers stay inside one Vitest line while the workspace keeps pnpm's default peer resolution and one shared Vitest 5 instance. `guards.mutation-lane.guard` names exactly the two manifests. `packages/api-client` still owns the generated `paths.d.ts` and compiles it with TypeScript 7. **ADR file.** `docs/adr/0002-mutation-lane.md`.
