...
This file is the authoritative record of every settled architecture decision for Iridium. Each entry corresponds one-to-one with a row of the decision skeleton (A1–A57) plus the single limits policy (A.1). Every entry is **accepted** as of 2026-09-11; nothing here is provisional. The eight open questions that could still change the work are not decisions and live in `14-risks-and-open-questions.md` (skeleton §G); where an ADR below depends on one of them, the ADR names the question and states the default that is in force until the user answers.
...
| Field | Meaning |
|---|---|
| **Id / title** | The skeleton id (`A1`…`A57`, `A.1`) and a short title. The id is stable and is what other plan sections cite. |
| **Status** | Always `Accepted (2026-09-11)` in this revision. The vocabulary for later revisions is `accepted`, `superseded by Ann`, `deprecated`. An accepted ADR's *Decision* text is never edited; a change is a new ADR that supersedes it. |
...
**Status.** Accepted (2026-09-11). **ADR file.** `docs/adr/0001-monorepo-toolchain.md`.
...
**Status.** Accepted (2026-09-11). **ADR file.** `docs/adr/0002-mutation-lane.md`.
...
**Consequences.** Positive: compile-error mutants are detected; the second TypeScript is confined to one devDependency-only package that nothing imports. Negative: Stryker forces `bail: 1`, single-threaded workers, coverage off, and does not support Browser Mode, so only the `unit` project is mutated (component/integration lanes are not); the Vitest 5 compatibility is unverified until the spike.

**Verification.** M0 spike `docs/spikes/stryker-vitest5.md` (records whether the `V4` fallback was taken); nightly `mutation` job with `break` enforced; M1 gate: Stryker ≥ 70 on `auth/**`, `authz/**`, `tokens`, `collab/persistence/**`, `@iridium/crdt`; M8 gate: `break 80`.
...
**Status.** Accepted (2026-09-11). **ADR file.** `docs/adr/0051-testing-harnesses.md`.
...
**Verification.** This ADR is itself the verification framework; its own checks: M0 exit (all Vitest projects and Playwright projects run empty-green on ubuntu + windows, Electron launch on three OSes), `docs/spikes/k6-yjs-bundle.md`, `docs/spikes/stryker-vitest5.md`, coverage thresholds enforced in `merge-reports`.
...
**Status.** Accepted (2026-09-11). **ADR file.** `docs/adr/0052-ci-pipelines.md`.
...
**Consequences.** Positive: a dependency with a disallowed license, an OpenAPI drift, a duplicate Yjs instance, a failing chain verification, or an unsigned installer cannot merge or ship. Negative: three-OS Electron E2E and Docker-based lanes make CI minutes the scarce resource; the ≤ 15 minute critical-path budget is protected by sharding and by moving long suites to nightly.
...
**References.** Digest §8.2 (runner labels, action tags, `actions-setup-mysql`), §9.2 (Renovate, Changesets 3, pnpm 12 CI semantics), §7.2 (GPL plugin); all four plans' CI sections; gap fix (license scan). Implemented in `10-testing-and-quality.md` and `11-operations-and-deployment.md`.
...
**Verification.** M0 spike `docs/spikes/fastify-websocket-hocuspocus.md`; `authz.route-policy.boot.guard` (every route has `config.auth`); `rest.route-index.contract`; `mcp.dual-era.contract` through the mounted route; `readyz.integration`.
...
**Verification.** M0: migrations 0001–0034 apply on `mysql:9.7.2` and `8.4.11`; kysely-codegen diff step in `static`; `readyz.integration` (503 with a pending migration); `migrations.integration` (two concurrent `migrate up` runs — one waits on `GET_LOCK`); the M8 upgrade rehearsal (M1-era backup → current).
...
**Status.** Accepted (2026-09-11). **ADR file.** `docs/adr/0009-mysql-version.md`.
...
**Status.** Accepted (2026-09-11). **ADR file.** `docs/adr/0015-yjs-state-encoding.md`.
...
**Consequences.** Positive: backups, restores and loads scale with content rather than tombstones; compaction (which loads into a `Y.Doc` anyway) produces a GC'd, small state; `yjs_major` marks every blob for a future v14 migration. Negative: V2 encoding is roughly twice as slow as V1 (yjs #675) — acceptable at compaction cadence; the spike is a hard M0 gate; readers of the log must never treat `update_v1` and `snapshot` interchangeably (enforced by the branded types).

**Verification.** M0 spike `docs/spikes/spike-onloaddocument-v2.md` (pass, or fallback executed and recorded); `persistence.model.prop` (fast-check model: random update sequences persisted, compacted at random points, reloaded — the reloaded `toString()` equals the model); `collab.restart-no-duplication.integration` (spec §9 row "Initialization/reconnection"); `collab.initial-state-only-path.guard`; `restore --verify` sample-loads snapshots and compares hashes (A47).
...
**Status.** Accepted (2026-09-11). **ADR file.** `docs/adr/0017-hocuspocus-embedded.md`.
...
**Verification.** M0 spike `docs/spikes/fastify-websocket-hocuspocus.md`; `kernel.smoke.integration`; `collab.convergence.integration` (three clients, overlapping positions); `collab.viewer-enforcement.integration` (`SyncStatus(false)`, no state change); `collab.baseline-on-connect.integration`; `security.ws-origin.integration`; `collab.limits.integration`; every hook has a unit test asserting it never rejects.
...
**Status.** Accepted (2026-09-11). **ADR file.** `docs/adr/0019-saved-ack-protocol.md`.
...
**Status.** Accepted (2026-09-11). **ADR file.** `docs/adr/0029-password-hashing.md`.
...
- **Format** (all kinds): `irid_<kind>_<id16 base62>_<secret43 base62><crc6 base62>`, `kind ∈ {pat, ses, tkt, spl}` with `oat` and `scim` reserved. The secret is 32 CSPRNG bytes; the CRC32 covers everything before it. Published scanner regex: `irid_(pat|ses|tkt|spl)_[0-9A-Za-z]{16}_[0-9A-Za-z]{49}`.
...
- **Verifier**: implements `OAuthTokenVerifier.verifyAccessToken` returning `AuthInfo {token, clientId: 'pat:' + id, scopes, expiresAt (always set), resource: new URL(PUBLIC_ORIGIN + '/mcp'), extras: {principal}}`.
...
**Status.** Accepted (2026-09-11). **ADR file.** `docs/adr/0032-mcp-transport.md`.
...
**Decision.** Pins: `@modelcontextprotocol/server` 2.0.0, `@modelcontextprotocol/node` 2.0.0, `@modelcontextprotocol/fastify` 2.0.0 (with `hostHeaderValidation([PUBLIC_HOST])`); development and test only: `/client` 2.0.0, `/conformance` 0.1.16, `@modelcontextprotocol/inspector` 2.6.0. The handler is built once: `const node = toNodeHandler(createMcpHandler(buildIridiumMcpServer, {legacy: 'stateless', responseMode: 'json'}))`. The route is
...
**Status.** Accepted (2026-09-11). **ADR file.** `docs/adr/0033-mcp-auth.md`.
...
| Alternative | Why rejected |
|---|---|
| PRM without `authorization_servers`, 401 without `resource_metadata` (plan-agent-first, plan-product-dx; digest Topic 6) | A discoverable document still invites probing clients into a chain that cannot complete; the spec's own field is OPTIONAL, so omitting the document entirely is conformant and unambiguous. |
| Full OAuth 2.1 authorization server in the MVP (digest Topic 10) | No authorization server exists; every target client accepts a static header today; the specification permits custom authentication. It is the first post-MVP epic (G1). |
...
**Consequences.** Positive: Claude Code, Cursor, VS Code, Windsurf, and the Messages API connector all work with a pasted token and nothing else; no discovery chain can half-complete; the OAuth path is designed and additive, so adopting it changes no URL and no existing token. Negative: claude.ai and Claude Desktop custom connectors require OAuth outside the limited "Request headers" beta, so those surfaces use the `iridium-mcp` bridge (A36) until the OAuth epic lands — this is G1, and the plan states the default rather than hiding it; a user who types the token without the `Bearer ` prefix produces `Authorization: <token>`, which the server must reject with a clear message (tested).

**Verification.** `mcp.auth.mcp` (missing, malformed, prefix-less, expired, revoked, and wrong-vault tokens; exact `WWW-Authenticate` value; absence of `resource_metadata`); `mcp.no-prm.contract` (a GET of `/.well-known/oauth-protected-resource` and `/.well-known/oauth-protected-resource/mcp` both 404); `mcp.scopes.mcp`; `mcp.fail-closed.mcp` (server-level and vault-level); the nightly real-client matrix asserts a static-header Claude Code session lists the six tools (the direct regression test for #59467).

**References.** Digest §3.2 (Claude Code #59467, claude.ai connector auth), §3.4, §6.2 (RFC 9728 fields), §11.5; spec §7; brief requirement 5; plan-risk-first ADR-15; judges 1, 2, 3; G1. Implemented in `06-mcp-and-agent-access.md` and `04-auth-and-access-control.md`.
...
**Status.** Accepted (2026-09-11). **ADR file.** `docs/adr/0036-stdio-bridge.md`.

**Context.** Claude Desktop's local configuration is stdio-only, and claude.ai custom connectors require OAuth outside a limited header beta (digest §3.2), so a stdio-to-HTTP bridge is the only path for those surfaces until the OAuth epic (G1). The obvious third-party option is `mcp-remote`, which digest §3.2 flags hard: it changed hands (geelen/Cloudflare → punkpeye/Glama), shipped 100+ versions including 13 on 2026-09-11 alone, and handles user tokens on disk under `~/.mcp-auth`. Two source plans proposed instead to share a tool package between the MCP server and a bridge built on a `RestReadApi`, which would mean two implementations of the same six tools and a second read backend to keep in parity.
...
| No bridge (wait for OAuth) | Claude Desktop is a primary target client for a markdown product; the bridge is small and reuses the SDK on both sides. |
...
**References.** Digest §3.2 (Claude Desktop stdio-only, claude.ai connector auth, `mcp-remote` ownership and cadence), §3.4, §3.5; brief requirement 5; judges 2, 3; G7. Implemented in `06-mcp-and-agent-access.md` and `11-operations-and-deployment.md`.
...
**Status.** Accepted (2026-09-11). **ADR file.** `docs/adr/0039-search.md`.
...
**References.** Digest §5.2 (InnoDB FULLTEXT, `innodb_ft_min_token_size`), §7.4 (filter `yaml`/`html` from plain text), §11.27, §3.6; spec §3, §4, §8; plan-risk-first ADR-22; G5. Implemented in `03-data-model.md` and `08-markdown-pipeline-import-export.md`.
...
**References.** Digest §7.2 (Obsidian syntax facts, GPL plugin, basename resolution, strict line breaks, task states), §7.5, §11.12; spec §3, §7, §10; judges 1, 2, 3; G2. Implemented in `08-markdown-pipeline-import-export.md` and `07-client-applications.md`.

### A44 — Attachments: content-addressed storage behind a driver interface, served by id, explicit deletion only

**Status.** Accepted (2026-09-11). **ADR file.** `docs/adr/0044-attachments.md`.
...
**References.** Digest §7.4 (`<img>` cannot send Authorization), §4.2 and §7.2 (Electron privileged scheme pattern, CVE-2026-70604), §6.2 (upload hardening), §11.13; spec §2, §4, §8; plan-risk-first ADR-23; plan-enterprise GC stance; G4. Implemented in `08-markdown-pipeline-import-export.md`, `07-client-applications.md`, `09-api-reference.md`.
...
**Status.** Accepted (2026-09-11). **ADR file.** `docs/adr/0041-editor-stack.md`.
...
**Status.** Accepted (2026-09-11). **ADR file.** `docs/adr/0053-electron-shell.md`.
...
**Decision.** Electron 44.3.0 with three plain build configurations — main built by tsdown as ESM (`deps.neverBundle: ['electron']`, with all imports awaited before `whenReady`), preload built by tsdown as a **single CommonJS file** (sandboxed), and the renderer built by the shared Vite config with `base: './'`. `vite-plugin-electron` 1.1.2 is permitted only as a development orchestrator; electron-vite 5/6 and Forge 7/8 are not used. Packaging is electron-builder 26.16.1 (installed from the `v26` tag) with electron-updater 6.8.9 against a **generic** provider at `https://<server>/desktop/updates/<channel>/`, `autoDownload: false`, an administrator policy of `disabled | prompt | silent` plus a channel from `GET /desktop/update-policy`, Windows `verifyUpdateCodeSignature` with a fixed `publisherName` (Azure Trusted Signing; the identity must be chosen before the first external build), macOS Developer ID with notarisation producing dmg and zip, Linux AppImage (auto-updating) plus deb and rpm, and Windows targets nsis (assisted, `perMachine`) and msi. `protocols: [{name: 'Iridium', schemes: ['iridium']}]`; `extraResources: bin/iridium-mcp` (A36).
...
**References.** Digest §6.2 (OWASP secrets management, envelope encryption), §5.2 (MySQL dump and XtraBackup facts), §10.2 (enterprise expectations); spec §8, §9; plan-enterprise graft; gap fixes (secrets bundle, trigger exclusion, blocking verification). Implemented in `11-operations-and-deployment.md` and `03-data-model.md`.
...
Every row of the skeleton decision table has exactly one ADR above. This index is the lookup other sections cite: the id is stable, the title is the ADR heading (shortened), and the last column names the plan section(s) that implement the decision. Ordered by id.
...
| A9 | MySQL 9.7 LTS primary, 8.4 LTS certified; baked `my.cnf` | 2 | `03-data-model.md`, `11-operations-and-deployment.md` |
...
| A33 | MCP authentication: PAT bearer only, no PRM in the MVP | 5 | `06-mcp-and-agent-access.md`, `04-auth-and-access-control.md` |
...
| A53 | Electron shell: Electron 44.3.0, three plain configs, electron-builder 26, full hardening | 7 | `07-client-applications.md`, `09-api-reference.md` (§D.4), `11-operations-and-deployment.md` |
...
| A55 | Accessibility, internationalisation, and browser support | 7 | `07-client-applications.md`, `10-testing-and-quality.md` |
...
### Decisions that depend on an open question

These ADRs are accepted with a stated default; the named question in `14-risks-and-open-questions.md` (skeleton §G) is the only thing that would change them, and each ADR says what changes.

| Question | ADRs affected | Default in force |
|---|---|---|
| G1 — native claude.ai / Claude Desktop OAuth connectors at MVP | A33, A36 | No: static-header clients plus the `iridium-mcp` bridge; OAuth is the first post-MVP epic |
| G2 — read-only Obsidian-syntax rendering at MVP | A43, A42 | Post-MVP flag; detection and `note_links` ship now |
| G3 — MySQL 8.4 LTS as a required target | A9, A52 | Both: 9.7 primary, 8.4 nightly lane |
| G4 — attachment encryption at rest | A44, A47 | Volume/TDE only; schema columns reserved |
| G5 — CJK search at MVP | A39, A9 | No: default parser, `innodb_ft_min_token_size=2` |
| G6 — browser support commitment at 1.0 | A55, A52 | Chromium-class; Firefox/WebKit nightly smoke |
| G7 — publish `@iridium/mcp-bridge` to npm | A36, A52 | Bundle with the desktop app and serve from `/desktop/tools/` only |
| G8 — Windows installer formats, fleet update policy, signing identities | A53, A52 | NSIS + MSI; updates `prompt`; identities provisioned before M5's first external build |
...
| D13-4 | Spike documents live at `docs/spikes/<slug>.md`, one per M0/M1 spike, and each must record the outcome **and** whether the ADR's recorded fallback was taken (for example `docs/spikes/electron-ws-origin.md`, `docs/spikes/onloaddocument-v2.md`, `docs/spikes/stryker-vitest5.md`, `docs/spikes/k6-yjs-bundle.md`, `docs/spikes/fastify-websocket-hocuspocus.md`). | Several ADRs (A15, A51, A53, A2) are accepted with an explicit fallback. A spike that does not record which branch was taken leaves the codebase in an undocumented state. |
...
The ADRs above record the decisions the plan is founded on. While writing the sections, their authors also had to settle 388 narrower questions that the decision skeleton did not reach: a column width, a lock order, a message name, a naming convention. Each is recorded in full, with its rationale, in the "Decisions made in this section" table at the end of the file named below. This index exists so that a decision can be found from its identifier alone, and so that no section-level decision is invisible from here.
...
| D01-10 | `01-vision-scope-and-principles.md` | The non-goals beyond spec §10 listed at the end of 4.4 are binding, not indicative. Agent write access of any kind, OAuth for MCP, MCP notifications/sessions/SSE/binary resources, comments, SCIM, M… |
...
| ARCH-19 | `02-system-architecture.md` | Every singleton interface (`AuthzBus`, `EpochTable`, `TicketStore`, `RateLimitStore`, `SearchIndex`, `StorageDriver`, `SettingsStore`, `JobScheduler` claim protocol) ships with a parameterised… |
...
| D03-15 | `03-data-model.md` | One invariant register (`I-01` … `I-23`) implemented once in `apps/server/src/db/invariants.ts` and consumed by `iridium doctor`, `iridium restore --verify` and the three model property suites |
...
| D04-16 | `04-auth-and-access-control.md` | Bounded failure auditing: `user.login.failed`, `token.denied`, `collab.connection.rejected` and `mcp.access.denied` are deduplicated per key per window (60 s for `user.login.failed` and… |
...
| D04-26 | `04-auth-and-access-control.md` | One PAT verification module and one symbol: `apps/server/src/auth/tokens/verify.ts` exporting `verifyToken(raw, { surface: 'mcp' \| 'rest' })`, wrapped (never duplicated) by… |
...
| D04-28 | `04-auth-and-access-control.md` | `bearerOnly` is the primary mechanism that keeps a browser session off `/mcp`; `ignoreCookies` is defence in depth, and neither may be dropped. Fastify merges instance-level hooks ahead of route-le… |
...
| D05-19 | `05-collaboration-and-durability.md` | D3-01's zero-length degradation lives in three named `@iridium/crdt` exports (`SV_STORED_MAX_BYTES`, `storedSv`, `recordedSv`) used by the writer, the compactor and the loader; the wire always carr… |
...
| D06-12 | `06-mcp-and-agent-access.md` | Add three metrics to the A49 registry: `iridium_mcp_tool_errors_total{tool}`, `iridium_token_auth_failures_total{reason}` (`malformed`, `unknown_id`, `secret_mismatch`, `revoked`, `expired`… |
...
| D06-15 | `06-mcp-and-agent-access.md` | Two structural guards: `apps/server/src/mcp/` may not import `@iridium/crdt`, `collab/`, or any mutating service (boundary rule plus `mcp.no-write-imports.unit.spec`); and `verifyToken` — the singl… |
...
| D10-22 | `10-testing-and-quality.md` | Two property budgets — `PROP` (`numRuns` 200 PR / 5 000 nightly, 60 s interrupt) for pure files in the `unit` project and `PROP_DB` (20 × 60 PR / 200 × 300 nightly, 600 s interrupt… |
| D10-23 | `10-testing-and-quality.md` | `docs/acceptance-map.json` carries a `sinceMilestone` per `(rowId, layer)` entry, `docs/milestones/CURRENT` names the open milestone, and `[area:<name>]` is a third legal requirement tag. |
...
| D10-33 | `10-testing-and-quality.md` | Single-process document ownership is enforced by a boot lease: `SELECT GET_LOCK('iridium_collab_owner', 0)` on one dedicated `dbPersist` connection held for the process's lifetime, released after t… |
...
| OPS-04 | `11-operations-and-deployment.md` | The MySQL 9.7 client tools (`mysqldump`, `mysql`, `mysqlbinlog`) are installed in the runtime image, pinned to the `9.7.2` package revision, with the repository key verified; `release.yml` asserts… |
...
| OPS-24 | `11-operations-and-deployment.md` | The readiness checklist adds `grants`, `key_versions`, `projection_workers`, `tls_cert`, and `access_log_partitions` (03 D03-03 / I-20) to the skeleton's set — fourteen checks, the set… |
...
| D12-1 | `12-milestones.md` | Each milestone exit from M1 onward is tagged `v0.<N>.0` (`v0.1.0` at M1 through `v0.8.0` at M8's release candidate, then `v1.0.0` at M8 exit) using Changesets' `git-tag`, with one fixed version acr… |
| D12-2 | `12-milestones.md` | `release.yml` builds and publishes the server image (with SBOM and provenance) for every milestone tag from M1 onward, not only for `1.0.0`. Installer signing is required only from M5. |
...
| D12-7 | `12-milestones.md` | Each §G open question is bound to the entry of the milestone whose work it changes (table 13.1). If it is unanswered at that gate, the skeleton's default is implemented, the alternative becomes a p… |
...
| D12-16 | `12-milestones.md` | `docs/milestones/CURRENT` is a committed one-line file naming the last exited milestone. The exit record of §13.2 advances it in the same commit as the tag, and it is the single input that arms the… |
...
| D14-01 | `14-risks-and-open-questions.md` | Identifier namespaces are fixed and non-overlapping: `R-T`/`R-P`/`R-O` risks, `S` spikes, `ASM` assumptions, `G` open questions, `D14-` decisions from this section; `A<n>` remains the decision tabl… |
...
| D14-04 | `14-risks-and-open-questions.md` | Every open question's default is encoded as a named flag, constant, CI lane or reserved column (never as an implicit assumption in code), and `docs/open-questions.md` must be empty before the 1.0 tag |
...
| D14-12 | `14-risks-and-open-questions.md` | A release tag requires a green `ops.backup-restore.drill.chaos` from the most recent nightly run, a non-stale `iridium_backup_last_verified_timestamp`, and a verified audit chain; `release.yml` che… |
...
| D14-14 | `14-risks-and-open-questions.md` | Likelihood and impact are scored with the scales defined in this section, re-scored only at a milestone gate, and every re-score is recorded in that gate's record with its reason |

Per file: 01 13, 02 28, 03 22, 04 29, 05 28, 06 25, 07 42, 08 33, 09 28, 10 39, 11 59, 12 16, 13 12, 14 14.
