...
| One `Principal`, one `authorize()` | Four credential kinds (password, session, collab ticket, integration token) resolve to one `Principal` type from `@iridium/contracts/authz.ts`; every surface calls the single `authorize()` in `apps/server/src/authz/authorize.ts` (A30). |
...
Reserved kinds (schema-valid, never issued in MVP): `oat` (OAuth 2.1 access tokens minted by the post-MVP authorization server, `access_tokens.kind='oauth'`) and `scim` (SCIM provisioning bearer, `kind='scim'`).
...
|---|---|---|
| `kind` | `pat` \| `ses` \| `tkt` \| `spl` (reserved `oat`, `scim`) | literal |
...
- Total length is 75 characters for the three-letter kinds. Underscores are not base62 characters, so a double-click selects the whole token in terminals and editors.
- Published secret-scanning regex: `irid_(pat|ses|tkt|spl)_[0-9A-Za-z]{16}_[0-9A-Za-z]{49}`. The CRC lets scanners and the server reject malformed strings offline; a string that fails the regex or the CRC never touches the database (this is the first line of the login/verification pipelines and bounds the cost of credential floods).
...
- `display_prefix` for PATs is `irid_pat_<id16>_` (26 characters), shown in lists; the remainder is shown exactly once at creation.
- Property test `auth.token-format.prop` (fast-check): for every generated token `parseToken(format(t)) = t`; flipping any single character invalidates the CRC or the regex; two issuances never share an `id16`.
...
| Action (route / CLI) | Same transaction | After COMMIT (`AuthzBus`, §8) | PATs |
|---|---|---|---|
| `POST /me/password {currentPassword, newPassword}` (self, step-up) | verify current (limiter A on failure) → policy → new hash → `password_changed_at` → revoke every other session of the user (`revoked_reason='password_change'`) → `authz_version+1` → audit `user.password.changed` | `user.password_changed {userId, keepSessionId}` → gateway closes all WS connections of the user except those of the current session | untouched (A28/D13-6); the settings UI offers "also revoke my integration tokens", which issues `POST /me/tokens/revoke-all` as a second call under the same step-up window |
| `POST /admin/users/:id/disable` (`server:users`, step-up) | `status='disabled'` → revoke all sessions (`user_disabled`) → `authz_version+1` → audit `admin.user.disabled` | `user.disabled {userId}` → every WS connection closed `revoked` | rows untouched; verifier rejects because `users.status ≠ 'active'` → `401` on the next call (spec §7 "revocation also stops integration access") |
...
| `DELETE /admin/users/:id` (soft: `status='deleted'`) | as disable + anonymisation rules from 03-data-model.md; audit `admin.user.deleted` | `user.disabled` | rejected as above |
...
- The `/collab` upgrade never reads cookies: authentication there is by ticket only (§7). `/mcp` never reads cookies either: the route is `bearerOnly`, so §6.1 skips the cookie branch entirely, and the route-level `ignoreCookies` hook strips the header and the parsed jar for the phases that follow (A32).
...
- Boot assertion (`authz.route-policy.boot`): every route with a non-safe method is CSRF-guarded unless `config.auth.bearerOnly === true` (only `/mcp`).
...
Step-up is required for (A26): creating, rotating and revoking PATs (`POST /me/tokens`, `POST /me/tokens/:id/rotate`, `DELETE /me/tokens/:id`, `POST /me/tokens/revoke-all`); `POST /me/password`; every `/admin/*` mutation; `POST /vaults/:id/archive|unarchive`; `POST /notes/:id/revisions/:rev/restore`; `DELETE /nodes/:id?purge=true`; `PUT /admin/settings`. `sessions.mfa_verified_at` is reserved so that a later MFA milestone can make step-up require a second factor without changing the route contract.
...
| `token` | `auth/tokens/verify.ts` (PAT bearer) | `/mcp`, the ★ read-only REST routes of 09-api-reference.md |
...
| MCP `POST /mcp` | `onRequest` host/origin guards + `preHandler` `patAuth` → `verifyToken(raw, {surface:'mcp'})` → `request.mcpAuthInfo` | every tool and resource handler calls `authorize(principal, …, {vaultId, surface:'mcp'})` through `ContentReadCore` | HTTP 401 with `WWW-Authenticate` when the bearer is missing/invalid; `isError` content for every in-tool denial (never 403) |
...
authenticate(request): Principal | null
  ...
  if request.headers.authorization:
      ...
      elif parsed.kind === 'pat': principal = verifyToken(raw, { surface })
      else                                                             → 401 invalid_credentials   // 'tkt'/'spl' are not HTTP credentials
...
- **Collab tickets and set-password links are not HTTP credentials.** `irid_tkt_` is accepted only inside the Hocuspocus auth message; `irid_spl_` only in the `POST /auth/set-password` body. Presenting either as a bearer is `401`, and it is *not* consumed (a probe cannot burn a victim's link).
- **`bearerOnly` is load-bearing, not cosmetic.** It is the only thing that stops a `__Host-iridium_session` cookie from producing a principal on `/mcp`, because `authenticate()` is an instance-level `onRequest` hook (registered by the `auth` plugin in boot step 4) and Fastify runs instance-level hooks *before* a route's own `onRequest` array — so `/mcp`'s `ignoreCookies` has not yet run when this function executes. Removing `bearerOnly` in the belief that `ignoreCookies` already covers the case would reopen the CSRF surface on `/mcp` (§6.5, 06-mcp-and-agent-access.md "Mounting `/mcp` on Fastify").
...
- `401` responses carry `WWW-Authenticate: Bearer realm="iridium"` only on bearer-only routes (`/mcp`, per A33); cookie-authenticated routes omit it so browsers never show a native auth dialog.
...
Denials inside a tool are `isError: true` results with a single shared text for *both* not-found and forbidden ("No note with that id or path is available to this token"), never HTTP `403` — an HTTP 403 makes conforming clients start an OAuth step-up flow that Iridium does not implement in MVP (A33/A34). Protocol-level failures (unknown tool, malformed params) remain JSON-RPC errors. Every tool call writes an `access_log` row with `note_ids` (§11.5), including denied calls (`status='denied'`).
...
**The Electron contingency.** Whether the `app://iridium` renderer actually sends `Origin: app://iridium` on a WebSocket upgrade is verified by the M0 spike `spike-electron-ws-origin` on Windows, macOS and Linux (E/M0). If any platform omits or alters it, the recorded fallback executes: socket ownership moves to the main process (`IpcWebSocket`, A53), main opens the socket with the `ws` library and an explicit `Origin: <PUBLIC_ORIGIN>` header plus the bearer-authenticated ticket relay, and the renderer talks to main over `iridium:collab:open|send|close` with `iridium:event:collab-message|collab-close`. The allowlist does not change, and no bypass is introduced: main is a trusted process that can legitimately assert the web origin. The decision is recorded in `docs/spikes/electron-ws-origin.md`.
...
|---|---|---|
| REST (next request) | Denied on the first request issued after COMMIT. No cache, no TTL, no grace. | `authz.rest-revocation.integration` |
| MCP (next call) | Denied on the first `tools/call` after COMMIT, including a call already in flight when it re-enters `ContentReadCore` for a second vault. | `mcp.revocation.contract` |
...
```ts
verifyToken(raw: string, opts: { surface: 'mcp' | 'rest' }): Promise<{ principal: TokenPrincipal; authInfo: AuthInfo }>
```

It is `verifyToken` rather than `verifyPat` because §9.2 and §13 make this module dispatch on the credential prefix, so `irid_oat_…` routes to a different validator once §G-1 is answered "yes"; a PAT-specific name would be wrong at that moment. `apps/server/src/mcp/verifier.ts` (the SDK's `OAuthTokenVerifier`) only wraps it, so there remains exactly one verification path.

It is called by `authenticate()` (§6.1) whenever a bearer parses as `kind === 'pat'`, and by the MCP `patAuth` preHandler. It performs one indexed read of `access_tokens` joined to `users`, plus the allowlist read, on **every** request — the "fresh token row per call" of A23 — and returns the `TokenPrincipal` shape shown in §5.1 together with the MCP SDK `AuthInfo` (`{token, clientId:'pat:'+id, scopes, expiresAt, resource:new URL(PUBLIC_ORIGIN+'/mcp'), extras:{principal}}`). `surface` is set by the authenticating route: `'mcp'` for `/mcp`, `'rest'` for the ★ read routes. Nothing else in the server constructs a token principal.

### 9.2 The four invariants `authorize()` enforces
...
| Aspect | `/mcp` (`surface:'mcp'`) | ★ REST read routes (`surface:'rest'`) |
|---|---|---|
...
| Denial shape | shared `isError` text for not-found and forbidden; **never** HTTP 403 (A33) | `ProblemDetails`: `404 not_found` for out-of-scope vaults, `403 forbidden` for a missing scope, `403 token_scope_insufficient` for a mutating route |
...
`/mcp` is the one route that opts out of that plugin: its `config.rateLimit = mcpBucket` is `{enabled: false}`. The plugin evaluates in an `onRequest` hook, where `patAuth` (a `preHandler`) has not run, so a `tok:<id>` `keyGenerator` there would silently degrade to an IP key and the documented per-token budgets would not exist. All four `/mcp` layers therefore live in `apps/server/src/mcp/rate-limit.ts` behind the `RateLimitStore` interface — the per-IP *failure* budget checked by `mcpIpGate` in `onRequest` and consumed by `patAuth` on its `401` path, the two token-keyed budgets and the process ceiling charged by `chargeRateLimit` in the `preHandler` — which is the arrangement 06-mcp-and-agent-access.md specifies (D06-20) and the one the route-policy boot assertion reads off the route.
...
| `credential_type` | `session` \| `pat` \| `ticket` \| `setpw` \| `cli` \| `system` \| `none` (the last only for pre-authentication failures) |
| `credential_id` | `sessions.id`, `access_tokens.id` or `password_setup_tokens.id` — so "which credential did this" is answerable without guessing |
...
| `action` | `mcp.get_note`, `mcp.search_notes`, `mcp.resources.read`, `rest.notes.markdown`, … |
...
pino SIEM events emitted by this section's code, consumed by log-based alerting (A49): `auth.login.succeeded`, `auth.login.failed`, `auth.login.blocked`, `auth.setpw.consumed`, `auth.reauth.failed`, `auth.session.expired`, `authz.denied {reason:'csrf'|'origin'|'forbidden'|'not_found'|'step_up'|'token_scope'|'ticket_session_mismatch'}`, `authz.revocation.swept {event, connectionsClosed, durationMs}`, `collab.connection.rejected`, `collab.write.rejected`, `token.auth.failed {reason}`. All of them carry `request_id` and principal ids only; `redact` strips `authorization`, `cookie`, `set-cookie`, `*.password`, `*.token`, `*.secret` (A49), and the metric `iridium_authz_bus_handler_errors_total` exists so a silently failing revocation subscriber is visible.
## 12. Threat model (T1–T17)

Adopted per A57 as a plan section. **This section is where the `T<n>` namespace lives** (14-risks-and-open-questions.md D14-01 registers `T<n>` against 04-auth-and-access-control.md §12): every `T1`–`T17` citation anywhere in the plan resolves against the table below, and `docs/threat-model.md` is the shipped form of *this* table, not a second source. The control-to-evidence map that pairs with it is a different table — the compliance checklist of 11-operations-and-deployment.md, shipped as `docs/compliance-checklist.md` — and it cross-links to these rows rather than restating them. 14-risks-and-open-questions.md's `R-T<n>` rows are a third, unrelated namespace (risks, not threats). Each row names the threat, the mechanisms that mitigate it (with the section or file that specifies them), and the named automated tests that hold the mitigation in place. Rows T1, T2, T11–T15 and T17 are mitigated mostly outside this section; they are listed in full because the table is the artefact security reviewers read, and because "not my section" is not an acceptable gap in a threat model.
...
| **T2** | Pathological Markdown stalls the server (quadratic emphasis, autolink blowup, deep nesting) | pre-scan caps (2 MiB source, blockquote depth 32, list indent 64 cols, 20 000 lines/paragraph); piscina worker isolation with a 10 s timeout and worker kill/respawn; browser preview in a dedicated Web Worker with a 2 s timeout; linear autolink transform only (A42, A.1) | `markdown.pathological`, `projection.timeout.integration` |
...
| **T7** | Token leakage (agent config files, shell history, logs, URLs, database dump) | SHA-256 at rest with `token_id` lookup; secret displayed once; never in a URL (PAT in a header, ticket in the auth message); pino redaction; published secret-scanning regex `irid_(pat\|ses\|tkt\|spl)_[0-9A-Za-z]{16}_[0-9A-Za-z]{49}` with an offline CRC check; mandatory expiry; rotation with bounded overlap; per-token rate limit and `access_log`; `tokens revoke-all`; snippets use environment indirection instead of literals (§2.2, §9, 06-mcp-and-agent-access.md) | `auth.token-format.prop`, `logging-redaction.test`, `mcp.auth.contract`, `tokens.rotation.integration` |
| **T8** | Session theft (cookie exfiltration, CSRF, fixation, renderer credential theft) | `__Host-` prefixed `HttpOnly; Secure; SameSite=Lax` cookie; custom header + Fetch Metadata CSRF guard on every mutating cookie request including multipart (§4.4); new session row per login (fixation impossible); idle + absolute expiry; channel binding (web cookie-only, desktop bearer-only); desktop token held only by the Electron main process, encrypted with `safeStorage`; renderer holds only single-use 60 s tickets (§4.5, §7) | `security.csrf`, `sessions.integration`, `desktop.preload-surface`, `desktop.attachments-no-token-in-renderer` |
...
| **T10** | Insider administrator abuse or audit tampering | every privileged action requires step-up and is audited inside its own transaction; HMAC chain per `chain_id` with locked heads; `BEFORE UPDATE/DELETE` triggers `SIGNAL SQLSTATE '45000'`; `iridium_app` holds only `INSERT, SELECT` on `audit_events`; `iridium audit verify-chain`; vault managers see admin actions inside their vault; admin-owned tokens get no implied access (§9.2, §11, A8, A46) | `audit.chain.integration`, `db-grants.integration`, `admin.*.integration`, `authz.admin-owned-token.unit` |
...
| **T12** | Electron escape (Node access from the renderer, navigation to remote content, deep-link injection, privileged-scheme CORS bugs of the CVE-2026-70604 class) | `sandbox:true`, `contextIsolation:true`, `nodeIntegration:false`, `webviewTag:false`; `app://iridium` registered with `corsEnabled:true`; `will-navigate` and `setWindowOpenHandler` deny; permission/device handlers deny-by-default; electron-builder fuses; zod-validated IPC with a synchronous `senderFrame.origin` check; zod-validated deep links; main-only credential custody (§6.7, A53) | `desktop.webPreferences`, `desktop.ipc-origin`, `desktop.ipc-contract`, `desktop.hardening` (3 OSes), `desktop.deep-link-fuzz` |
...
flowchart LR
  ...
  subgraph Trusted["Trusted, operator-controlled"]
    ...
    S["Iridium server<br/>REST + /collab + /mcp"]
    D[("MySQL 9.7")]
    F[("Attachment store")]
  end
...
| Credentials (passwords, sessions, tickets, PATs) | grant everything the owner has | argon2id + pepper, SHA-256 for high-entropy secrets, one-time links, main-process custody, mandatory expiry, revocation |
...
Assumed adversaries: (1) a curious or malicious **employee with a valid account** in some vault — the main adversary the RBAC and isolation tests target; (2) an **unauthenticated network attacker** reaching the public HTTPS endpoint; (3) a **hostile note author** trying to escape the renderer or the agent; (4) a **compromised agent host** holding a PAT; (5) an **honest-but-careless operator** (secrets in the wrong place, a forgotten token). Explicitly out of scope for MVP: a compromised server host, a malicious database administrator, and a compromised end-user workstation — each of which is documented in `docs/threat-model.md` with the operational controls (disk encryption, MySQL TDE guidance, backups, signed installers) that reduce, but do not eliminate, the exposure.
## 13. Extension seams (post-MVP, designed now)

Spec §10 defers enterprise SSO; the brief requires a clean path to OAuth-based MCP access. Nothing below is built in MVP. Each item is listed with the seam that already exists, so adding it is additive and does not touch `authorize()`, the route contract or the collaboration hooks.

| Extension | Seam that exists in MVP | What the extension adds | Ordering |
|---|---|---|---|
| **OAuth 2.1 authorization server for MCP** | `auth/tokens/verify.ts` already implements the SDK's `OAuthTokenVerifier.verifyAccessToken` and returns a full `AuthInfo` (with `resource` and `expiresAt` always set); `access_tokens.kind` already has the `'oauth'` value; the verifier dispatches on the credential prefix, so `irid_oat_…` rows route to a different validator without touching the tools; `MCP_OAUTH_ENABLED` is reserved | PKCE S256 authorization endpoint, consent screen, RFC 8707 resource indicators, RFC 9207 issuer identification, Client ID Metadata Documents, `/.well-known/oauth-protected-resource` with `authorization_servers`, the `resource_metadata` parameter on 401 challenges, `insufficient_scope` step-up, `oauth_clients` / `oauth_authorization_codes` / `oauth_refresh_tokens` tables (designed in C.11, not created) | First post-MVP epic; required only if §G-1 is answered "yes" |
| **OIDC single sign-on** | `SessionIssuer.issue(user, {kind, method, …})` is the single funnel every login method ends in (§4.1); `auth_providers` and `identities` tables are designed (C.11); `users.status`/`authz_version` already drive revocation; the desktop deep-link scheme `iridium://` is registered (A53) | `openid-client` 6.8.8 with PKCE S256, discovery, nonce/state; `POST /auth/oidc/start` + `/auth/oidc/callback`; JIT provisioning into `users` + `identities`; domain routing; desktop flow = system browser → `iridium://auth/callback?code=…` exchanged for a desktop session | After OAuth for MCP |
...
Two rules keep these seams honest: (1) every new login method must end in `SessionIssuer.issue()`, and (2) every new credential kind must produce a `Principal` and go through `authorize()`. `authz.seams.unit` asserts that the `Principal` union and the `RouteAuth` type have not grown a bypass shape (a route policy with neither `public`, `self`, `serverAdmin` nor `permission` fails to type-check).
...
|---|---|---|---|
| Token format and CRC | `auth.token-format.prop`, `auth.token-parse.unit` | unit, property | M1 |
...
| Sessions | `sessions.integration` (issue, sliding idle, absolute cap, channel binding, per-user cap eviction, expiry finalisation), `auth.logout.integration` (`Clear-Site-Data`, cookie cleared), `auth.session-verify-parity.unit` (table-driven over the row states {live, revoked, idle-expired, absolute-expired, owner disabled, owner deleted}: `verifySession` and `loadLiveSession` accept and reject the identical set, so the shared `checkLiveRow` cannot drift; `loadLiveSession` never accepts a raw `irid_ses_…` string and `verifySession` never accepts a bare session id) | integration, unit | M1 |
...
| Token principals | `token.effective-permissions.prop`, `authz.admin-owned-token.unit`, `token.reserved-scopes-inert.unit`, `tokens.self-revoke-all.integration` (two live tokens → `200 {count: 2}`; each token's next MCP call `401`; one `token.revoked_all {scope:'user', count:2}` row with both ids in `targets` plus two `token.revoked {reason:'revoke_all_self'}` rows; a second call `200 {count: 0}`; a PAT principal refused; without step-up `403 step_up_required`) | property, unit, integration | M1/M3 |
...
| D04-26 | **One PAT verification module and one symbol**: `apps/server/src/auth/tokens/verify.ts` exporting `verifyToken(raw, { surface: 'mcp' \| 'rest' })`, wrapped (never duplicated) by `apps/server/src/mcp/verifier.ts`. | The plan named the same module `verifier.ts` and `verify.ts` and the same function `verifyPat` and `verifyToken`; with `mcp/verifier.ts` already fixed by A32, the auth-side name must be `verify.ts`, and a credential-neutral symbol survives the `irid_oat_…` branch that §13 designs. |
