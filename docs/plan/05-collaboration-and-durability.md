...
| Single history | A note's Y.Doc is built from Markdown exactly once (`NoteService.initialize`) and reloaded from persisted binary state thereafter; Markdown projections are output, never input | `initialized_at` guard, source-scan guard tests `collab.initial-state-only-path.guard` and `collab.no-reinit.guard` |
...
3. The guard test `deps.single-instance.guard`: the five packages must resolve to exactly one version each in `pnpm-lock.yaml` (the `static` job records `pnpm why yjs lib0 y-protocols @codemirror/state @codemirror/view` beside it as evidence), and the renderer bundle analysis asserts one chunk owner per package.
...
`prefixSuffixDiff(current, target)` computes the longest common prefix and suffix (in UTF-16 units, never splitting a surrogate pair) and returns the single middle edit that turns `current` into `target`; applying `delete(start, deleteLength)` then `insert(start, insert)` on a `Y.Text` whose `toString()` equals `current` yields exactly `target` (property `crdt.prefix-suffix-diff.prop`).
...
| `beforeHandleMessage` / `connected` | Initializes the native connection's writer latches at whichever boundary runs first, before a queued sync frame can apply while earlier connected hooks await SQL. Initialization notices occur once; a read-only sync write still receives the existing refusal notice. The connected path covers idle connections (A17, 2026-09-20 amendment). |
...
- `collab.initial-state-only-path.guard` — scans the first-party sources under `apps/`, `packages/` and `tooling/` for `new Y.Doc(`, under whatever binding `yjs`'s `Doc` is imported as, and fails unless the occurrence is inside `packages/crdt/src/**`, `apps/server/src/collab/persistence/initial-state.ts`, `@iridium/testkit`, or a test file.
- `collab.no-reinit.guard` — scans the same trees for the writes into the note body (`getContent(…).insert(` and `insertChunked(`) and fails unless the occurrence is inside `packages/crdt/src/initial-state.ts` (the one Markdown → Y.Doc path of `NoteService.initialize`) or the chunker it inserts through, the revision-restore path under `apps/server/src/revisions/`, the fenced `ServerEdit` chunk insertion in `apps/server/src/collab/gateway.ts` that the content repair uses, or a test file; `getText('content')` itself may appear only in `packages/crdt/src/doc.ts`.
...
3. Logs `persist.backpressure` at `warn`, counts `iridium_persist_failures_total{reason="backpressure"}` (which the persist-failure alert reads) and increments `iridium_persist_queue_depth` / `iridium_persist_backlog_age_seconds` (both already exported continuously). `collab.backpressure.chaos` asserts the event.
...
sequenceDiagram
  ...
  W->>DB: BEGIN
  W->>DB: nodes FOR SHARE; notes FOR UPDATE; note_docs FOR UPDATE (note N)
...
The LF invariant additionally has its own guard test (`collab.lf-invariant.guard`): it parses every non-test source under `apps/server/src/notes`, `transfer` and `collab` and fails on any string or template literal whose decoded value contains `\r`, so no server content path can produce one. The runtime halves are DB-backed: `transfer.fixtures.integration` asserts that no `note_projections.markdown` of the fixture corpus contains `\r`, and `collab.content-invalid.chaos` that a `\r` sent by a hostile client is detected, locked and repaired.
...
- Every structural mutation follows the owner fence → vault-X → explicit node/note/document parents → derived children → audit head order in 02-system-architecture.md. Projection publication instead takes vault-S immediately after the owner fence, before any consistent read, then the same parent order; raw updates and explicit checkpoints omit the vault gate. Sorted point locks avoid unrelated rows acquired by an optimizer-selected scan. Purge starts synchronous local queue fencing under its short admission lock, awaits disposal outside it, then reacquires vault-X and revalidates before child-first deletion (`note_links`, `note_search`, `note_projection_terms`, `note_projections`, revisions, updates, documents, notes, trash entries, nodes). A queued writer rechecks its lifetime after locking the head, so a concurrent restore cannot revive it.
- **External side effects run after COMMIT.** Broadcasts, document disposal and `AuthzBus` delivery never await inside the transaction. Synchronous local queue fencing is part of trash/purge admission while the structural lock prevents a competing restore; owned closing marks are released if validation or the final transaction fails.
...
`collab.live-revocation.integration` covers downgrade → rejected → upgrade → re-attach → saved as one test, and asserts that the text the viewer typed while read-only is exactly the text that ends up committed after the upgrade.
...
1. `ReadinessState` flips to `not_ready` (`readiness.beginDrain()`, idempotent), so `/readyz` returns 503 and every request outside the ops routes — new `/collab` upgrades, REST, and calls to **both** MCP mounts, `/mcp` and the connector mount `/mcp/connect` (06-mcp-and-agent-access.md) — is refused with 503 `not_ready` and `Retry-After: 5` in its path's envelope (ARCH-06; D09-30). The gate is the readiness state, not a route list (02-system-architecture.md ARCH-02), so a mount added later is drained without a second change here; both MCP mounts are named because the two mounts are one surface behind two credentials (06-mcp-and-agent-access.md), and a drain that refused one and served the other would keep half the agent traffic arriving at a process whose worker pool and database pools are being torn down. The MCP exchanges already admitted are ended in the drain's first phase, `mcp`, before step 2: `subscriptions/listen` streams receive their complete frame, the rest answer 503 `not_ready` or are ended (02-system-architecture.md ARCH-06, amended for the `mcp` phase). No collaboration step depends on it. The `jobs` phase follows, still before step 2: the scheduler stops claiming jobs and running import/export jobs are marked `failed {error:'shutdown'}` (02-system-architecture.md §"Shutdown sequence" step 3).
...
| `collab.restart-no-duplication.integration` | Two clients open an imported note, the server restarts (graceful and hard), both reconnect and reopen the note; the text contains the imported content exactly once, and `note_updates` has no duplicate `seq` |
...
|---|---|---|
| Three clients converge, including overlapping positions | `collab.convergence.integration`, `convergence.model.prop` | integration, property |
...
| One Yjs instance | `deps.single-instance.guard` (lockfile + startup guard; `pnpm why` evidence and bundle analysis beside it) | static/unit |
| Documents are never rebuilt from Markdown | `collab.initial-state-only-path.guard`, `collab.no-reinit.guard` (source-scan guards), `import.commit.integration` (once-only init, idempotent resume) | static, integration |
| Viewer enforcement on the socket | `collab.viewer-enforcement.integration` (update refused, `SyncStatus(false)`, text still exportable) | integration |
| Live revocation, downgrade and upgrade | `collab.live-revocation.integration` (removal, disable, session revoke, downgrade → `rejected` → upgrade → re-attach → `saved`, closure ≤ 1 s after COMMIT) | integration |
| Awareness cannot impersonate | `collab.awareness-identity.integration` | integration |
| The awareness rate cap actually drops | `collab.awareness-rate.integration` — the 11th awareness frame within one second is not broadcast to a second connection on the same document, `iridium_collab_messages_total{type="awareness_dropped"}` increments by exactly 1, no CLOSE(7) frame is sent, a Sync `Update` sent immediately afterwards still applies (only type 1 is filtered), a `Ping(9)` still gets its `Pong`, and a second note open on the same socket keeps its own full budget | integration |
...
| Checkpoints and thinning | `revisions.thinning.integration`, `projection.monotonic.integration` | integration |
| Graceful shutdown | `collab.graceful-shutdown.chaos` (every acked state on disk, clients reconnect to identical content) | chaos |
| Capacity | admission-control refusal in `collab.admission-budget.integration`; `perf.workspace.e2e` and the k6 SLOs (`durable_ack_ms p95 < 1 s`, `yjs_propagation_ms p95 < 250 ms`, `projection_lag_ms p95 < 12 s`, RSS < 1.5 GB at 300 VUs / 60 docs) | integration, E2E, load |
