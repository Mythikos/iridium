...
| Item | Rule |
|---|---|
| Entity ids | UUIDv7, canonical lowercase string (`018f3a2e-7b1c-7d3e-9a4b-2c5d6e7f8091`) on every surface; `BINARY(16)` in MySQL. Branded types in `@iridium/contracts/ids.ts` (`UserId`, `VaultId`, `NodeId`, `NoteId`, `AttachmentId`, `TokenId`, `SessionId`, `JobId`). A `NoteId` is the `NodeId` of a node with `kind='note'`. |
...
|---|---|---|
| `Authorization: Bearer <credential>` | request | `irid_ses_…` (desktop session) or `irid_pat_…` (integration token). When present, cookies are ignored entirely. |
...
|---|---|---|---|
| `unauthenticated` | 401 | any non-public route | Missing, unknown, expired or revoked session/PAT. The `WWW-Authenticate: Bearer realm="iridium", error="invalid_token"` challenge belongs to the `bearerOnly` route `/mcp` alone (skeleton A33, §4.3), which answers its own OAuth-shaped body instead of this envelope; every `/api/v1` route omits the header so a browser never shows a native auth dialog (04-auth-and-access-control.md §6.1). `detail` never says whether the credential existed. |
...
The closed enum `ErrorCode` in `@iridium/contracts/errors.ts` is exactly this table; `security/problem.unit.test.ts` asserts every code thrown by `apps/server/src` is in the enum and `openapi.contract.spec` asserts every enum member appears in at least one documented response in `openapi.json`.
...
| Cursor payload | `base64url(JSON{v:1, k, a, f, t, tv?, exp}) + "." + base64url(HMAC-SHA256(MCP_CURSOR_KEY, JSON bytes))`; `k` ∈ `notes\|tree\|search\|revisions\|attachments\|trash\|links\|audit\|access\|sessions\|tokens\|jobs\|users\|vaults`, `a` = after-key tuple, `f` = SHA-256 of the canonical filter object, `t` = principal key (`ses:<sessionId>` for user principals, `pat:<tokenId>` for token principals), `tv` = `tree_version` (notes/tree listings only), `exp` = unix seconds (issue + 3600). |
...
| Bucket | Limit | Key |
|---|---|---|
| Authenticated | 600/min | principal (`ses:<id>` or `pat:<id>`) |
...
Exceeding a bucket → `429 rate_limited` with `retry-after`; PAT `429`s also write an `access_log` row with `status='rate_limited'`.
...
export const Attachment = z.strictObject({
  ...
  originalName: z.string(), pathHint: z.string().max(760), inlineable: z.boolean(),   // path_hint is VARCHAR(760) (03 D3-04); inlineable = inline disposition allowed (png/jpeg/gif/webp/avif)
...
export const Token = z.strictObject({
  id: TokenId, name: z.string(), displayPrefix: z.string(), kind: z.enum(['pat','oauth','scim']),
...
At MVP `features` contains `mcp`, `desktop-updates`, `attachments`, `import`, `export`, `search`; `obsidian-compat-rendering`, `oauth`, `smtp` are reserved names that post-MVP milestones turn on. Response is cacheable for 300 s.
...
Request:  {}
Response 200: { count: z.int().min(0) }        // 0 when nothing was live; idempotent
...
SnippetClient = SnippetClientSchema   // z.enum(['claude-code','claude-code-mcp-json','claude-desktop','cursor','vscode','windsurf','claude-ai-connector','messages-api','custom-mcp-client','curl','mcp-remote'])
...
Query: { vaultId: VaultId.optional(), tokenId: TokenId.optional(), userId: UserId.optional(),
         surface: z.array(z.enum(['mcp','rest','export'])).optional(),
...
AccessLogEntry = z.strictObject({
  occurredAt: Timestamp, tokenId: TokenId, user: UserRef, surface: z.enum(['mcp','rest','export']),
...
The predicate is applied in SQL, never as a post-filter, and `agent-activity.integration` asserts exactly these boundaries (06-mcp-and-agent-access.md D06-02). `noteIds` are resolved to current paths by the UI on expansion, not by the route. `clientName`/`clientVersion` come from MCP `clientInfo` or the `User-Agent` and are labelled untrusted in the UI.
...
| Method | Path | Auth | Notes |
|---|---|---|---|
| GET | `/desktop/update-policy` | public | Read by the Electron main process before every update check. Absolute URL: `<PUBLIC_ORIGIN>/api/v1/desktop/update-policy` — it is an `/api/v1` route (`ProblemDetails` on error, §1.4), unlike the two static surfaces below |
...
| GET | `/desktop/updates/*` | public static | `electron-updater` generic feed |
...
GET /api/v1/desktop/update-policy → 200 {
  ...
  latest: z.strictObject({ version: z.string(), publishedAt: Timestamp, notesUrl: z.url().nullable() }).nullable(),
...
Response 200 { items: z.array(z.strictObject({ version: z.string(), channel: z.enum(['stable','beta']), publishedAt: Timestamp,
                ...
                files: z.array(z.strictObject({ platform: Platform, arch: z.enum(['x64','arm64']),
                  name: z.string(), sha512: z.string(), size: z.int(), blockmap: z.boolean() })) })) }
...
POST /admin/releases
Content-Type: multipart/form-data
  metadata (required)  JSON part: { version, channel, notes?, files:[{platform: Platform, arch, name, sha512, size, blockmap}] }
  artifact (repeated)  one part per file named in metadata (installer, blockmap, latest*.yml)
...
Errors: 403 step_up_required · 409 invalid_state (version already published on that channel)
      · 413 payload_too_large (per-artifact cap 1 GiB) · 415 unsupported_media · 422 validation_failed (sha512_mismatch | missing_artifact)
...
The server verifies each artifact's SHA-512 against the metadata while streaming it to `UPDATES_DIR/<channel>/`, writes the `desktop_releases` row, regenerates `latest.yml`, `latest-mac.yml` and `latest-linux.yml` atomically, and audits `admin.release.published`. `electron-updater` then consumes `/desktop/updates/<channel>/` as a plain generic provider; Windows update code signatures are verified client-side against a fixed `publisherName` (skeleton A53), so the server never becomes the trust anchor. `iridium desktop-updates publish <dir>` performs the same operation from CI without an HTTP round trip.

Withdrawal is a **soft flag, not a delete**: `DELETE /admin/releases/:channel/:version` runs `UPDATE desktop_releases SET withdrawn_at = ?, withdrawn_by = ? WHERE version = ? AND channel = ? AND withdrawn_at IS NULL`, regenerates `latest.yml` / `latest-mac.yml` / `latest-linux.yml` without that version, keeps the row **and** the artifacts (so a client mid-download does not fail hard), and audits `admin.release.withdrawn {version, channel}`. A withdrawn release stays visible to auditors on `GET /admin/releases` with `withdrawnAt` set and is invisible to updaters (`GET /desktop/update-policy` and the generated feed ignore it); a re-publish of the same `(version, channel)` is still refused with `409 invalid_state`, because the published fields of a row are immutable and the primary key is `(version, channel)` — withdrawing and re-publishing is not an edit path (03-data-model.md §13.3, D03-17). The emergency lever above a withdrawal is unchanged: raise `schema_meta['min_client_version']`.
...
Non-`/api/v1` surfaces: `GET /healthz`, `GET /readyz`, `GET /metrics`, `GET /app/*`, `GET /set-password`, `GET /desktop/updates/*`, `GET /desktop/tools/*`, `ALL /mcp` (§4), `GET /collab` (§3).
...
sequenceDiagram
  ...
  H-->>UI: Stateless {"v":1,"t":"persisted","seq":N,"sv":"..."}
  UI->>H: Sync update (edit)
  H->>W: document 'update' listener (svAfter captured synchronously)
  W->>DB: BEGIN … INSERT note_updates … UPDATE note_docs (CAS) … COMMIT
  W-->>UI: Stateless {"v":1,"t":"persisted","seq":N+1,"sv":"..."}  (broadcast)
```
...
| | `sv` | `Base64Sv` = `z.string().min(4).max(87_400).regex(/^[A-Za-z0-9+/]*={0,2}$/)` | Base64 of the **V1 state vector** (`Y.encodeStateVector`) captured synchronously after the last update in the batch was applied. The wire always carries the **full in-memory** vector, even when the writer had to store a zero-length `note_updates.sv_after` / `note_docs.snapshot_sv` because the vector was wider than `VARBINARY(4096)` (03-data-model.md D3-01), so the client's dominance check is unaffected; the bound is therefore the wire size (87 400 characters ≈ 64 KiB raw, one definition in `@iridium/contracts/collab.ts`, 05-collaboration-and-durability.md D05-19) and never the column width. The 4 KiB stateless cap of §3.10 applies to client → server payloads only |
...
| `t` | Field | Type | Meaning |
|---|---|---|---|
| `baseline` | `v`, `t` | `1`, `'baseline'` | No payload. Sent after **every** provider `synced` event (first connect and every reconnect). The server replies on that connection with `persisted` built from the writer's `lastPersisted` (initialised in `afterLoadDocument` from `note_docs.head_seq` plus the last `note_updates.sv_after`, else `snapshot_sv`). This closes the "opened without editing" and "crashed after COMMIT before the broadcast" gaps (skeleton A19 step 5). |
...
Any other `t`, a malformed JSON payload, a missing or unknown `v`, or a stateless payload larger than 4 KiB closes the connection with `protocol-error`.
...
- **Per-document close** — `connection.close({code, reason})` sends `MessageType.CLOSE` for that document. The shared socket and other documents stay up. The Hocuspocus provider surfaces only the **reason string** (its own close event hard-codes a code), so the client state machine keys on reasons, never on codes.
...
| `too-large` | 1009 | A frame exceeded 2 MiB or a single update exceeded 1 MiB | Do not retry the same update; show "This edit is too large" |
...
| `onLoadDocument` | Refuse unknown, trashed, foreign or archived-vault documents; `applyUpdateV2(snapshot)` then `applyUpdate(update_v1)` for `seq > snapshot_through_seq` in order; **return `undefined`** (never bytes) | Throw → document destroyed, connections closed `note-not-found` |
...
| `localSv` | `Y.encodeStateVector(ydoc)`, recomputed on every local update |
| `persisted` | the stateless `persisted {seq, sv}` message (§3.4) — the only "Saved" signal |
...
`dominates(a, b)` checks **every** `(clientId, clock)` pair in `b` against `a` — not just the local client id — so a relayed update from another participant cannot make a client claim "Saved" prematurely, and Hocuspocus issue #845 cannot mislead it. The state machine (`SaveStateMachine` in `@iridium/collab-client`) is a pure function of the inputs above and is property-tested.
...
`/mcp` is a single Streamable-HTTP MCP endpoint on the same origin and port as the REST API, built on `@modelcontextprotocol/server` 2.0.0 + `/node` 2.0.0 + `/fastify` 2.0.0, stateless, serving both protocol eras. This section is the wire reference; the product rationale, client onboarding and the token lifecycle are in 06-mcp-and-agent-access.md.

### 4.1 Transport and request handling

| Item | Value |
|---|---|
| Path and methods | `ALL /mcp`. `POST` is the protocol. `GET` and `DELETE` (legacy session semantics) answer `405`. No `Mcp-Session-Id`, and no server-initiated notifications: `responseMode: 'json'` makes every request/response exchange a single `application/json` JSON-RPC object. The one exception is `subscriptions/listen` (modern era only), which the SDK's own listen router answers over `text/event-stream` regardless of `responseMode` — Iridium advertises no `listChanged` bit, so the stream is acknowledged with an **empty filter** and nothing is ever published on it (06-mcp-and-agent-access.md D06-17) |
| Handler | `const node = toNodeHandler(createMcpHandler(buildIridiumMcpServer, { legacy: 'stateless', responseMode: 'json', keepAliveMs: 15_000, maxSubscriptions: 1024 }))`; the route calls `reply.hijack()` and then `node(Object.assign(req.raw, { auth: req.mcpAuthInfo }), reply.raw, req.body)` |
...
| Cookies | Ignored entirely; `principalKinds: ['token']` |
...
- `vaults.mcp_enabled = false` — the vault disappears from `list_vaults` and from the vault-index resources, and every tool call or resource read naming it answers with the **shared** not-found text of §4.6, byte-identical to a vault outside the token's scope. There is deliberately no distinct "agent access is disabled for this vault" wording: it would confirm to a token that the vault id exists (skeleton F13, `mcp.isolation.contract`).

### 4.3 Authentication and authorization

```
Authorization: Bearer irid_pat_<id16>_<secret43><crc6>
```

| Case | Response |
|---|---|
| Valid, unexpired, unrevoked PAT | `AuthInfo {token, clientId: 'pat:<tokenId>', scopes, expiresAt (always set), resource: new URL(PUBLIC_ORIGIN + '/mcp'), extras: {principal}}` |
| Missing, malformed, unknown, revoked or expired | `401` with `WWW-Authenticate: Bearer realm="iridium", error="invalid_token", error_description="Create an integration token under Settings › Integrations"` and the OAuth-shaped body `{"error":"invalid_token","error_description":"Create an integration token under Settings › Integrations and set it as the Authorization: Bearer header for this MCP server."}` — **not** `ProblemDetails` (§4.6), and one text for every one of those causes (06-mcp-and-agent-access.md, *Storage and verification*, enumerates the checks that all answer it, and *Failing closed and error mapping* pins the header). **No** `resource_metadata` parameter and **no** `/.well-known/oauth-protected-resource` document in MVP (skeleton A33). The PAT-enabled ★ REST routes are the other half of the same hook and answer `ProblemDetails` instead — `401 token_expired` for a well-formed PAT past `expires_at`, `401 unauthenticated` otherwise (§1.5) — so `/mcp` is the one route in the application that is exempt from the error envelope, exactly as it is the one route exempt from the CSRF guard |
...
Effective rights per call are recomputed from the database on every request: `scopes ∩ permissionsOf(live explicit role)` with `vaultId ∈ access_token_vaults` (or `all_vaults` meaning "every vault the owner is an explicit member of right now"). Token principals never inherit server-admin implied access. Revocation is therefore immediate: the next call fails (`unauthenticated`) and there is no principal cache to invalidate.
...
HTTP-level failures are `401` (no or bad credential), `403` (Host/Origin), `405` (legacy `GET`/`DELETE`), `413` (body over 1 MiB), `429` (rate limit) and `500 {"error":"server_error"}`. `ProblemDetails` is **not** used on `/mcp`: MCP clients parse JSON-RPC, not RFC 9457.
...
| `k` | Keyset kind: `notes`, `tree`, `search`, `revisions`, `attachments`, `trash`, `links`, `audit`, `access`, `sessions`, `tokens`, `jobs`, `users`, `vaults` |
| `a` | The after-key tuple for that keyset (§1.6) |
| `f` | SHA-256 of the canonicalised filter object (query, prefix, kinds, vault set …) |
| `t` | Principal key: `pat:<tokenId>` for tokens, `ses:<sessionId>` for user principals |
...
`packages/mcp-bridge` ships a first-party transparent proxy for clients that can only spawn a stdio process (Claude Desktop today). It is not a second implementation: it forwards `tools/list`, `tools/call`, `resources/list`, `resources/templates/list`, `resources/read` and `completion/complete` to the remote `/mcp` over `StreamableHTTPClientTransport` with `requestInit.headers.Authorization`, and lets the SDK negotiate eras.
...
| Flags | `--server <origin>` (required), `--vault <id>` (optional filter), `--token-file`, `--allow-insecure-http` (development only) |
...
| Distribution | Bundled in the desktop app's `resources/bin/` and downloadable from `/desktop/tools/` (npm publication is open question G7) |
...
| Errors | A handler rejection crosses IPC as `{ problem: ProblemDetails }` when it originated from an HTTP response, or `{ problem: { code: 'server_error' \| 'unavailable' \| 'validation_failed', … , requestId } }` for local failures. `IpcTransport` in `@iridium/api-client` rethrows it as the same typed error class the web `FetchTransport` throws, so UI code cannot tell the hosts apart |
| Version skew | None: renderer and main ship in one installer (skeleton A54). The IPC contract therefore has no negotiation and no optional-field tolerance rules |
...
iridium:updates:install    → { ok: true }        // quitAndInstall; only valid in state 'downloaded'
...
UpdateState = z.discriminatedUnion('status', [
  ...
  z.strictObject({ status: z.literal('available'), version: z.string(), notes: z.string().nullable(), mandatory: z.boolean() }),
  z.strictObject({ status: z.literal('downloading'), version: z.string(), percent: z.number().min(0).max(100), bytesPerSecond: z.int() }),
  z.strictObject({ status: z.literal('downloaded'), version: z.string(), mandatory: z.boolean() }),
  z.strictObject({ status: z.literal('error'), message: z.string(), retryInMs: z.int() }),
]);
```

`openExternal` validates the scheme against `https:`/`mailto:` and rejects everything else — this is one of the hard Electron-hardening rules (note content is untrusted). `updates:check` reads `<selected profile origin>/api/v1/desktop/update-policy` first (the absolute form of the §2.16 route): with `policy:'disabled'` it returns `{status:'disabled', reason:'policy'}` without contacting the feed. `mandatory:true` is set when the offered version is at or above the policy's `minVersion` while the running version is below it.
...
| `iridium:event:update-state` | `UpdateState` | Every `electron-updater` state transition |
...
Response-shape enforcement at test time: every integration and end-to-end test asserts its response with the ajv-backed matcher `expect(response).toMatchOpenApi(operationId, status)` (ajv 8.20.0 + swagger-parser 13.0.0). Schemathesis 4.26.1 fuzzes the document (`--stateful=links`) — a light pass per pull request, the full suite nightly. The `rest.route-index.contract.spec` test parses the §2.18 table out of this markdown file and asserts that its operation set, auth column and If-Match column match `openapi.json` and the boot-time route policy, so this document cannot silently drift from the server either.
...
| `tags` | One per domain: `auth`, `meta`, `me`, `tokens`, `vaults`, `members`, `tree`, `nodes`, `notes`, `revisions`, `search`, `attachments`, `imports`, `exports`, `admin`, `desktop`, `ops` |
| `components.schemas` | Named from `z.globalRegistry` (`Vault`, `NoteMeta`, `ProblemDetails`, …); anonymous inline schemas are a lint failure |
| `security` | Two schemes: `sessionCookie` (`apiKey` in cookie `__Host-iridium_session`) and `bearer` (`http`, `bearerFormat: 'irid_ses\|irid_pat'`). Each operation lists what it accepts; `★` operations list both |
...
| Error responses | Every operation documents every `ProblemDetails` code it can produce; the contract test asserts each `ErrorCode` enum member appears in at least one operation |
...
| `minClientVersion` | A semver string in `GET /meta`. Clients send `X-Iridium-Client-Version`; a version below `minClientVersion` gets `426 client_outdated` on every route except `GET /meta` (so the client can always discover why) |
...
| Desktop IPC | No skew is possible — renderer and main ship in one installer — so IPC changes need no version negotiation. Removing a channel still requires an `apiVersion` bump only if the server contract behind it changed |
| Product version | One Changesets version across the server image, the web bundle, the installers and the bridge (skeleton A1), reported as `serverVersion` |
...
| D09-11 | `/readyz` returns the same JSON body with `200` and `503`, and `GET /admin/system` embeds it as `readiness`. Its `status` and per-check vocabulary is `ok`/`warn`/`fail`, and `ReadyzCheckName` is exactly the readiness table of 11-operations-and-deployment.md (fourteen checks, including `access_log_partitions` for 03 D3-03 / I-20) — the same strings the `iridium_readyz_check_status{check}` label and the alert rules use. `/metrics` answers a bad token with a bare `401` (no `ProblemDetails`). | An operator debugging a failing readiness check needs the checklist, not an error document; Prometheus does not parse `application/problem+json`. One vocabulary across the body, the metric label and the alert expressions is what makes a dashboard row, an alert and a `curl /readyz` line refer to provably the same check. |
