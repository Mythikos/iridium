...
| `log_bin_trust_function_creators` | `ON` | Required, not a relaxation. With binary logging on — which the row above makes mandatory — MySQL refuses `CREATE TRIGGER` from any account without `SUPER` (ER_1419), and only `root` holds `SUPER` (§2). Migrations `0028` (the two `audit_events` triggers) and `0029` (the two `audit_events_archive` triggers) and `iridium migrate ensure-guards` create the four audit-immutability triggers as `iridium_migrator`, so without this line the append-only guarantee could never be installed by the role that installs it. The blast radius is exactly those triggers: Iridium defines no stored functions and no procedures, and only `root` and `iridium_migrator` hold any privilege to create a stored program |
...
CREATE TABLE access_tokens (
  ...
  last_used_at            DATETIME(6) NULL,                   -- LastUsedTracker's flush, monotonic (see below)
  last_used_ip            VARBINARY(16) NULL,
  last_client             VARCHAR(120) NULL,                  -- clientLabelOf() in auth/user-agent.ts: 'name/version' (informational)
  rate_limit_per_hour     INT UNSIGNED NULL,                  -- PAT: NULL = pat_policy default; always NULL for kind='oauth'
  created_at              DATETIME(6) NOT NULL,
  created_from_session_id BINARY(16)  NULL,
  created_ip              VARBINARY(16) NULL,
  created_user_agent      VARCHAR(255) NULL,                  -- truncateUserAgent() in auth/user-agent.ts
  ...
  refresh_id              BINARY(16)  NULL,                   -- the refresh row issued in the same token response; no FK, see below
  resource                VARCHAR(255) NULL,                  -- RFC 8707 audience; NULL for kind='pat'
  revoked_at              DATETIME(6) NULL,
  revoked_by              BINARY(16)  NULL,
  revoke_reason           VARCHAR(120) NULL,                  -- written from ACCESS_TOKEN_REVOKE_REASONS (@iridium/contracts); read as a string
  version                 INT UNSIGNED NOT NULL DEFAULT 1,
...
`refresh_id` deliberately carries **no** foreign key, and the reason is a real conflict rather than an oversight: token rows are never deleted (`A31`) while `oauth_refresh_tokens` rows are swept 30 days after the family expires (§4A), so a `RESTRICT` constraint here would make that sweep a dead letter — every refresh row would be pinned forever by the access tokens issued with it. It is a provenance column of exactly the same kind as `rotated_from_id`, which has carried no foreign key since `0008` for the same reason, and it is listed with the other attribution columns in §1.4. It has one meaning: the refresh row issued in the same token response as the access token — the family's first row at code exchange, the newly inserted rotated row at refresh — and it is `NULL` for a PAT and for a client whose `grant_types` exclude `refresh_token` (D03-26). Two readers follow it, and neither can dangle. Reuse detection works forward from a presented refresh row to the access tokens issued with its family. Code replay works backward from a code's `access_token_id` to that token's `refresh_id` and so to its family (§4A); a code row is swept 24 hours after its 60-second expiry, and a refresh row no earlier than 30 days after its revocation or absolute expiry, so a code that can still be replayed never names a swept family. A swept refresh row leaves `refresh_id` pointing at nothing, which is harmless: by then the family is long expired and neither reader can reach it.

Semantics that the schema encodes (the verifier and lifecycle live in `04-auth-and-access-control.md`):

- **Lookup path**: `irid_pat_<token_id>_<secret43><crc6>` (or `irid_oat_…` for an OAuth access token) → CRC check offline → `SELECT … FROM access_tokens WHERE token_id = ?` (unique index, O(1)) → `timingSafeEqual(secret_hash, SHA-256(secret))` → `revoked_at IS NULL` → `rotation_overlap_until IS NULL OR rotation_overlap_until > now` (a rotated-out token) → `expires_at > now` → owner `status='active'` → memberships. Every MCP call performs this fresh; there is no principal cache in the MVP (`A23`). One verification path serves both kinds: the OAuth additions are two primary-key `LEFT JOIN`s on the same statement, not a second round trip.
- **`kind`** is live in two of its three values. `pat` is an integration token a user created in Settings › Integrations; `oauth` is an access token the authorization server of §4A minted for a connector. `scim` is reserved and never issued. The token format is the same family — `irid_pat_…` and `irid_oat_…` — so `tokens.format.unit` and the published leak-scanner regex cover both, and the same regex covers every other `irid_` kind, the OAuth client secret `ocs` of §4A included, which is never accepted as a bearer (D04-36).
...
- **`consent_id` and `client_id`** are what a consent revocation and a client disable sweep on: revoking `oauth_consents.id = X` revokes every `access_tokens` row with `consent_id = X` in the same transaction, and disabling or retiring an `oauth_clients` row does the same through `client_id`. **`refresh_id`** is what a refresh-family revocation sweeps on when refresh-token reuse is detected (§4A). A token minted by a code exchange is inside its family — its `refresh_id` is the family's first row — so a replay of that code revokes the code's access token and, through `refresh_id`, the family and every access token issued with it, and nothing else under the consent (D03-26). `ix_tokens_consent (consent_id, revoked_at)` and `ix_tokens_client (client_id, revoked_at)` exist for exactly those sweeps and for nothing else — the family sweeps filter the consent's live tokens on `refresh_id` — and without them each is a full scan of a table whose rows are never deleted.
- **`name` uniqueness applies among the owner's active PATs only**: `kind='pat'` rows whose derived status (below) is `active`. A rotated predecessor is exempt, because its successor carries the name. No index enforces it, because a unique key cannot express a derived status: the check runs under the owner's `users` row `FOR UPDATE`, in the credential lock order of `02-system-architecture.md` "Lock order" at `READ COMMITTED` (`A46`), and the same check counts the owner's active PATs against `LIMITS.PAT_MAX_ACTIVE_PER_USER` (100), beyond which creation is `422 validation_failed` `token_limit_reached`; a rotation replaces a token and is not counted (`06-mcp-and-agent-access.md` D06-50). An OAuth row's `name` is the client's `client_name`, is not unique, and never produces the `409 name_conflict` that a duplicate PAT name produces — two connectors from the same vendor are two legitimate rows. `display_prefix` for an OAuth access token is `irid_oat_<token_id>_`, the same `CHAR(26)`.
- **`expires_at` stays `NOT NULL`** for an OAuth row and is `now + oauth_policy.accessTokenTtlMinutes` (§13.1, default 60 minutes); the "never expires" sentinel is never used for an OAuth token. `rate_limit_per_hour` is `NULL` and never read: issuance and refresh never write it, and the grant's hourly budget, keyed `ocn:<consentId>` so that it survives refresh, uses `oauth_policy.defaultRateLimitPerHour`. A per-connector override is a grant attribute that M7 adds as a nullable `oauth_consents.rate_limit_per_hour`, and `admin.tokens.update` refuses an OAuth token's budget (D04-37). `admin_owned` and `all_vaults` obey the PAT rules, except that `all_vaults` is refused at the consent step rather than at token creation: for a server administrator with `all_vaults_admin_forbidden`, and for a non-administrator while `pat_policy.allowAllVaultsForNonAdmins` is `false` with `all_vaults_disabled` (§13.1; `06-mcp-and-agent-access.md` D06-51).
...
- **Vault scope** is either the explicit `access_token_vaults` allowlist (each id must be a vault the owner is an explicit member of at creation and is re-checked at use) or `all_vaults=1`, which resolves to "every vault the owner is an explicit member of at call time". `all_vaults` is refused for server admins (`all_vaults_admin_forbidden`, checked first), and for non-admins while `pat_policy.allowAllVaultsForNonAdmins` is `false` (`all_vaults_disabled`, D06-51); `admin_owned=1` records that the owner was an admin when the token was created (audited as `token.created {adminOwned:true}`); token principals never inherit admin-implied access.
- **Rotation**: `POST /me/tokens/:id/rotate` inserts a new row with `rotated_from_id` = the old id and sets the old row's `revoked_at` (immediately) or `rotation_overlap_until` (when an overlap ≤ `pat_policy.rotationOverlapMaxHours` was requested — the grouped form of the skeleton's `pat_rotation_overlap_max_hours`, §13.1); the old row is never deleted, so the chain of rotations is queryable. Only an `active` token rotates: a `revoked`, `expired` or already `rotated` one answers `409 token_not_rotatable`, so a rotation chain cannot fork (D06-01). The successor's `expires_at` is `now + (old.expires_at − old.created_at)`, capped at the effective `pat_policy.maxLifetimeDays`; a predecessor carrying the "never expires" sentinel passes it on only while the effective `allowNoExpiry` holds, and otherwise the successor gets `now + maxLifetimeDays`. Rotation mints a credential with a fresh lifetime, so it applies `all_vaults_admin_forbidden` and `all_vaults_disabled` exactly as creation does (D06-50, D06-51).
- **`last_used_*`** are written only by `LastUsedTracker` (`apps/server/src/auth/tokens/last-used.ts`), an in-memory map bounded by `TOKEN_LAST_USED_PENDING_MAX` that every token-verifying process flushes through its sink (`auth/tokens/last-used-store.ts`) every `TOKEN_LAST_USED_FLUSH_INTERVAL_MS` (10 minutes), independent of `JOBS_ENABLED`, and a final time in the auth plugin's `onClose` under ARCH-06's close-flush deadline; a close flush the deadline abandons destroys its connection and counts its unconfirmed entries in `iridium_token_last_used_dropped_total`. For an OAuth token the tracker also records its `consent_id`, and each flush is one credential-path transaction at `READ COMMITTED` (`auth/credential-transaction.ts`, the credential lock order of A46) that first advances `oauth_consents.last_used_at` with `GREATEST`, in ascending consent id (§4A), and then updates the tokens in ascending id. The token statement `UPDATE … WHERE id = ? AND (last_used_at IS NULL OR last_used_at < ?)` is monotonic, so a late flush never moves a value backwards. Both statements are Kysely `updateTable` calls whose `.set()` is an object literal over `NON_BUMP_COLUMNS` keys only, so they bump no `version` and stand outside the compare-and-set rule of §7.2. The values are informational and never part of authorization; `sessions.last_seen_at` stays on the session verifier's synchronous path (§3), and the `last_used_flush` job type is reserved and never scheduled or runnable (§11.1; `06-mcp-and-agent-access.md` D06-49).
- **Status is derived, never stored.** One function, `deriveTokenStatus(row, now)` in `packages/contracts/src/tokens.ts` beside `TokenStatusSchema`, decides it with the precedence `revoked` (`revoked_at` set) > `expired` (`now ≥ expires_at`, or `rotation_overlap_until` set and `now ≥` it) > `rotated` (`rotation_overlap_until` set) > `active`; **live** means `active` or `rotated`, which in SQL is `revoked_at IS NULL AND expires_at > now AND (rotation_overlap_until IS NULL OR rotation_overlap_until > now)`. The verifier checks revocation, then the overlap, then expiry, then the owner's status, so an elapsed overlap is `401 unauthenticated` even past `expires_at` and only a token that reaches the expiry check reports `token_expired` (D06-50).
- **Rows are never deleted**, for OAuth access tokens exactly as for PATs; `ix_tokens_user (user_id, revoked_at, expires_at)` serves the settings list, whose default is the owner's live PATs. Expiry is derived, never written and never audited: no job sweeps token rows, because auditing expiry would chain the hourly expiry of every OAuth access token, the issuance traffic `04-auth-and-access-control.md` §11.3 keeps out of the chain; presenting an expired token remains the bounded `token.denied {reason:'expired'}`. `ix_tokens_expires (expires_at)` serves the expiry filters (`iridium tokens list --expiring-in-days` and the administrator's status filter) and the `iridium_tokens_active{kind}` gauge's 60-second count of live rows grouped by `kind`, which range-scans only unexpired rows of a table whose rows are never deleted; `ix_tokens_user` cannot serve that server-wide count because its leading column is `user_id`. Keeping the OAuth rows is what keeps every `access_log` row resolvable to the credential that made the call, even after an hour-long access token has expired and the connector has refreshed twice.
...
CREATE TABLE oauth_clients (
  id                         BINARY(16)  NOT NULL PRIMARY KEY,
  client_id                  VARCHAR(512) CHARACTER SET ascii COLLATE ascii_bin NOT NULL,  -- the CIMD https URL, or a 32-char base62 id for DCR/manual; matched byte for byte
  registration_kind          ENUM('cimd','dynamic','manual') NOT NULL,
  client_name                VARCHAR(120) NOT NULL,               -- truncated to 120 code points at ingestion (DCR, CIMD, manual); never a refusal
  client_uri                 VARCHAR(512) NULL,
  logo_uri                   VARCHAR(512) NULL,                   -- stored, never rendered
  application_type           ENUM('native','web') NOT NULL,       -- CIMD: taken from the document, or derived when absent (native iff every redirect URI is an http loopback URI)
  token_endpoint_auth_method ENUM('none','client_secret_basic') NOT NULL DEFAULT 'none',
  client_secret_hash         BINARY(32)  NULL,                    -- SHA-256 of the irid_ocs_ secret43; manual confidential clients only
  client_secret_prefix       CHAR(26)    NULL,                    -- 'irid_ocs_<id16>_'
  ...
  status                     ENUM('active','disabled','deleted') NOT NULL DEFAULT 'active',   -- 'deleted' = a retired tombstone
  created_at                 DATETIME(6) NOT NULL,
  created_by_user_id         BINARY(16)  NULL,                    -- NULL for cimd and dynamic
  first_authorized_at        DATETIME(6) NULL,                    -- set once by the first consent; never moved; NULL = never authorized
  disabled_at                DATETIME(6) NULL,
  disabled_by                BINARY(16)  NULL,
  version                    INT UNSIGNED NOT NULL DEFAULT 1,
  deleted_at                 DATETIME(6) NULL,
  deleted_by                 BINARY(16)  NULL,
  live_client_id             VARCHAR(512) CHARACTER SET ascii COLLATE ascii_bin GENERATED ALWAYS AS
        (IF(status <> 'deleted', client_id, NULL)) VIRTUAL,
  KEY ix_oauth_clients_status (status, created_at),
  KEY ix_oauth_clients_unused (first_authorized_at, created_at)
);
-- own migration, per the 0011 precedent:
ALTER TABLE oauth_clients ADD UNIQUE KEY uq_oauth_clients_live (live_client_id);
...
CREATE TABLE oauth_consents (
  ...
  revoked_at         DATETIME(6) NULL,
  revoked_by         BINARY(16)  NULL,
  revoke_reason      VARCHAR(120) NULL,          -- written from CONSENT_REVOKE_REASONS (@iridium/contracts); read as a string
  version            INT UNSIGNED NOT NULL DEFAULT 1,
  live_consent_key   VARBINARY(32) GENERATED ALWAYS AS
        (IF(revoked_at IS NULL, CONCAT(user_id, client_id), NULL)) VIRTUAL,
  last_used_at       DATETIME(6) NULL,           -- newest access-token use of the grant; NULL = never used; LastUsedTracker only
...
CREATE TABLE oauth_authorization_codes (
  ...
  expires_at            DATETIME(6) NOT NULL,
  consumed_at           DATETIME(6) NULL,              -- set only by the one successful-exchange UPDATE, with access_token_id
  access_token_id       BINARY(16)  NULL,              -- the access token the exchange minted; set with consumed_at by the one successful-exchange UPDATE
  replay_detected_at    DATETIME(6) NULL,              -- first replay detection; set once, under the row lock, by the replay branch
  ...
  CONSTRAINT fk_oauth_codes_token   FOREIGN KEY (access_token_id) REFERENCES access_tokens(id)
);
...
CREATE TABLE oauth_refresh_tokens (
  ...
  revoked_at          DATETIME(6) NULL,
  revoke_reason       VARCHAR(120) NULL,           -- written from REFRESH_TOKEN_REVOKE_REASONS; its subset REUSE_SIGNAL_REVOKE_REASONS is the theft response
  reuse_detected_at   DATETIME(6) NULL,            -- first reuse detection of this row; set once, under the row lock
...
The block above is the **resulting** shape. The tables were created by the immutable M0 migrations `0035`–`0045`; M3's forward migrations `0060`–`0064` and `0066` (§14.1) append `access_token_id`, `replay_detected_at` and `fk_oauth_codes_token` to the codes and `reuse_detected_at` to the refresh rows (D03-26), `last_used_at` to the consents (`06-mcp-and-agent-access.md` D06-49, D06-50), and give `oauth_clients` its retire-in-place shape — the `ascii_bin` `client_id`, the `'deleted'` status, `deleted_at`, `deleted_by`, `first_authorized_at` renamed from `last_authorized_at`, and `live_client_id` with `uq_oauth_clients_live` in place of the former `uq_oauth_clients_client_id (client_id(191))` (D03-27).
...
**Lookup paths.** Every OAuth credential is the same `irid_<kind>_<id16>_<secret43><crc6>` string the rest of the plan uses, with four new kinds — `oac` (authorization code), `oat` (access token), `ort` (refresh token) and `ocs` (a manual confidential client's secret) — so one format, one CRC check and one comparison rule cover them all. An authorization code is found by `uq_oauth_codes_code_id` and a refresh token by `uq_oauth_refresh_token_id`, in both cases a single unique-index lookup followed by `timingSafeEqual(secret_hash, SHA-256(secret))`; nothing is ever found by scanning a secret. That comparison precedes every other check on a code or refresh row, so a public id16 alone can neither exchange a code nor trigger a revocation. A client secret is found by its client row, which a presented `client_id` resolves through `WHERE live_client_id = ?` (D03-27), and is compared in constant time with `client_secret_hash`; it is never accepted as a bearer (D04-36). Access tokens are **not** here: they are `access_tokens` rows with `kind='oauth'` (§4), which is what makes revocation a next-call property for a connector exactly as it is for a PAT, with no second mechanism and no signing key to distribute or rotate.

**The refresh chain, and what reuse detection revokes.** Every refresh token belongs to a `family_id` — the first token of a grant starts a family and every rotation stays in it. Using a refresh token rotates it: the presented row gets `rotated_at = now`, a new row is inserted with the same `family_id` and `rotated_from_id` pointing at the presented row, and the new secret is returned. Presenting a row that has `rotated_at` set — a superseded token, including one a client retries after a lost response — or that was revoked as a theft response (`revoke_reason` in `REUSE_SIGNAL_REVOKE_REASONS`: `refresh_reuse`, `code_replayed`) is the theft signal, and the response is one transaction: every live row of that `family_id` and every live `access_tokens` row issued with the family (its `refresh_id` belongs to the family) are revoked with `revoke_reason='refresh_reuse'`, the presented row is stamped `reuse_detected_at = now` under `reuse_detected_at IS NULL`, and `400 invalid_grant` is returned. Only the transaction that makes that stamp's NULL→now transition writes the chained `oauth.refresh.reuse_detected`; every detection is logged and counted. A row revoked for any other reason — by a person, an administrator, the client through RFC 7009, a revoke-all, a client cascade or user deletion — is refused as an ordinary `invalid_grant` and is never reuse. `ix_oauth_refresh_family (family_id, revoked_at)` is what makes the refresh half of that a single indexed sweep; the access-token half is driven from `ix_tokens_consent (consent_id, revoked_at)` — the family's rows all share one `consent_id`, so the candidate set is the consent's live tokens and the `refresh_id IN (…family…)` filter is applied to a handful of rows. That is why `access_tokens.refresh_id` needs no index of its own. `expires_at` slides forward to `now + oauth_policy.refreshIdleDays` on each rotation but never past `absolute_expires_at`, which is fixed when the family starts at `now + oauth_policy.refreshAbsoluteDays`; `ix_oauth_refresh_expires` drives the sweep over that column.

**Code-replay provenance and first-detection stamps.** `consumed_at` has exactly one writer: the `UPDATE` of a successful exchange, which sets `consumed_at` and `access_token_id` together after the access token and, for a refresh-capable client, the family's first refresh row have been inserted in the same transaction. No failed validation, error branch or replay branch writes it, so a failure never burns a code. The exchange validates in a fixed order. Before its transaction, on plain reads: the client authenticates — a `client_secret_basic` client must present verified Basic credentials and a `none` client none, and any other presentation, or a `client_id` that does not resolve (a retired client's included), answers `401 invalid_client`; the code parses as kind `oac` and its row is read by `code_id`, an absent row still paying one constant-time comparison against a fixed digest; and the secret is compared, a mismatch answering `400 invalid_grant` with nothing written, revoked or audited. One transaction then takes, at `READ COMMITTED` through the credential-transaction helper `apps/server/src/auth/credential-transaction.ts`, the credential lock order of `02-system-architecture.md` "Lock order" (A46): the owner-generation fence; the consent's selected vaults `FOR SHARE` in ascending id; the owner's `users` row, the client row and the consent row `FOR SHARE`, with a locking re-read of the vault selection, a changed selection restarting the transaction under ARCH-24's retry budget; and the code row `FOR UPDATE`. Under those locks it checks `consumed_at IS NULL`, and otherwise takes the replay branch, before anything else, so a replay is detected whichever client presents it. Then, the first failure answering `400 invalid_grant`: the code's `client_id` is the authenticated client, `expires_at > now`, the client is `active`, `redirect_uri` is byte-equal, PKCE S256 passes, `resource` is equal, and the authorizing session, the consent and the owner are live. The client binding is checked first so a mismatch is always observed; it leaves the code unconsumed and redeemable by its own client, and it is a log line and a counter, never a chained row.

The replay branch, still under the same locks, revokes exactly what the first exchange produced: the access token named by `access_token_id` and, when that token's `refresh_id` is set, every live refresh row of its family and every live access token issued with the family, all with `revoke_reason='code_replayed'`. Nothing else under the consent is touched. It then runs `UPDATE oauth_authorization_codes SET replay_detected_at = :now WHERE id = :id AND replay_detected_at IS NULL` on the row it already holds `FOR UPDATE`, so exactly one transaction across all processes sees the NULL→now transition, and only that transaction writes the chained `oauth.code.replayed` (zero counts included, attributed to the code row as the acting credential). Every detection is logged and counted, and the answer is `400 invalid_grant`; a replay after the consent was revoked or the client disabled is therefore chained once with zero counts, and every later replay during the code row's 24-hour life is logged and counted only. The refresh grant has the same shape: client authentication and the secret comparison before the transaction, the presented row and its family's rows `FOR UPDATE` under the same lock order, and a classification of the presented row in this order — reuse (above), then revocation for any other reason (an ordinary `invalid_grant`), then the sliding and absolute expiry, client-status, consent and owner checks before rotation.

One provenance column suffices: the access token's `refresh_id` leads to the family, so the code row needs no `family_id` copy that could disagree with it. The `RESTRICT` on `fk_oauth_codes_token` never fires, because `access_tokens` rows are never deleted and a code row is swept 24 hours after it expires. I-27 checks all three columns (D03-26).
...
**Attribution and foreign keys.** `oauth_authorization_codes.session_id`, `oauth_consents.granted_session_id`, `oauth_consents.revoked_by`, `oauth_clients.created_by_user_id`, `oauth_clients.disabled_by` and `oauth_clients.deleted_by` deliberately carry no foreign key, for the reason §1.4 gives for every other attribution column: sessions are swept on their own schedule and users are anonymised rather than deleted, so a constraint here would either block housekeeping or force attribution to be nulled. `session_id` in particular is a live re-check, not a reference: at code exchange the server looks the session up and refuses the exchange if it is gone, which is a stronger property than a constraint would give.

**Retiring a client.** `iridium oauth clients delete` and, from M7, `DELETE /admin/oauth-clients/:clientId` retire a client rather than delete its row. In one transaction the delete runs the disable cascade with `revoke_reason='client_deleted'` — every consent, refresh row and access token of the client with `revoked_at IS NULL` is revoked — writes one chained `oauth.client.deleted` whose `targets` list the revoked consents and access tokens (§12.2), and turns the row into a tombstone: `status='deleted'` (an appended `ENUM` value), `deleted_at`, `deleted_by` and a `version` bump. A retired client is excluded from client resolution and can never be re-enabled: a route that names it answers `404 not_found` and the CLI exits `3`. It also frees its `client_id`. Uniqueness holds only among non-retired rows, through `live_client_id` — `client_id` while `status <> 'deleted'`, `NULL` once retired — and `uq_oauth_clients_live` over it, the `live_consent_key` pattern above; every lookup of a presented `client_id` is `WHERE live_client_id = ?`, so a tombstone never resolves and a later registration of the same identifier is a new row. `client_id` is `ascii_bin`, so the match is exact and the whole value is in the key, and a CIMD document must name the requested `client_id` byte for byte (`06-mcp-and-agent-access.md` D06-41). The tombstone keeps every consent, token, refresh row, `access_log` row and audit target that names it resolvable. Disabling is the reversible control: a disabled client keeps its row and its `client_id` — which is what keeps a CIMD URL blocked — while its consents, refresh rows and access tokens are revoked with `client_disabled`. After a disable the connector's next token request is `400 invalid_grant`, because the client still resolves and fails the locked status check; after a retirement it is `401 invalid_client`, because the `client_id` no longer resolves (`06-mcp-and-agent-access.md` D06-43).

`first_authorized_at` is set once, by the first transaction that creates a consent for the client, and never moved afterwards, so the client row is locked `FOR UPDATE` once per client lifetime rather than on every authorization, and the stamp does not bump `version`, because it is not an administrator-editable field. The per-grant time is `oauth_consents.last_authorized_at`, written by the consent screen's Allow and by every silent re-authorization; the client DTO's `lastAuthorizedAt` is derived as its maximum over the client's consents through `ix_oauth_consents_client`, and `firstAuthorizedAt` exposes the column. "Never authorized" means `first_authorized_at IS NULL`. `oauth_consents.last_used_at` is the grant's newest access-token use, `NULL` when no token of the grant was ever used: only `LastUsedTracker`'s flush writes it, advancing it with `GREATEST` from each flushed OAuth token's `consent_id` (§4), it does not bump `version`, and it is informational and never part of authorization. `OAuthConsent.lastUsedAt` serves it (`06-mcp-and-agent-access.md` D06-49, D06-50). A CIMD row is created only by the first Allow for its `client_id`, inside the consent transaction and with `first_authorized_at` already set, so a CIMD row is never unused (D06-41). Two invariants hold the rest: I-28, a disabled or retired client holds nothing unrevoked, and I-29, a never-authorized client has no dependents and no CIMD row is never-authorized (§16.1; D03-27).

**Both engines.** Every statement above runs unchanged on MySQL 8.4.11 and 9.7.2 under the dialect rule of §1.1. The only features used are `ENUM`, `JSON`, a `VIRTUAL` generated column with a unique key over it, foreign keys added to an existing table (`0046`, and M3's `0060`), `ALGORITHM=INSTANT` `ADD COLUMN` at the end of a partitioned table (§12.5) and of an unpartitioned one (M3's `0064` and `0066`), and, in M3's `0061` (§14.1), a column charset conversion, an appended `ENUM` value and a column rename on `oauth_clients`, a table no release before M3 has written. All of them are inside the 8.4.11 floor, and all are asserted by `migrations.parity.integration` on both images rather than by this paragraph.
...
| `oauth_clients` with `registration_kind='dynamic'`, `first_authorized_at IS NULL` and `status <> 'disabled'` (retired rows included) | `OAUTH_UNUSED_CLIENT_TTL_DAYS` (7) after `created_at`; audited `oauth.client.expired`, except that a retired row's removal is not audited again, because its retirement is already chained as `oauth.client.deleted` |
...
The sweep runs as `sweepOAuth` in `apps/server/src/auth/oauth/retention.ts`, which the `session_ticket_sweep` handler calls after the session and set-password-link sweep, in the phase order codes, refresh rows, clients; the clients phase is `expireUnusedClients` in `auth/oauth/clients.ts`, the owner of `oauth_clients` persistence. The client sweep is the only one of the three that deletes a row other rows may point at, and it reaches only rows that cannot have dependents: a dynamically registered client that never completed an authorization (`first_authorized_at IS NULL`) and is not disabled, because disabling is the reversible control the sweep must never undo. A CIMD row is created only by an Allow, with `first_authorized_at` set, so the sweep never reaches it, and a manual client is never a candidate. The sweep takes each candidate `FOR UPDATE` and re-checks the predicate before its `DELETE`, and I-29 says such a row has no consent, code, refresh or access-token row. The `RESTRICT` constraints on `oauth_authorization_codes.client_id`, `oauth_refresh_tokens.client_id`, `oauth_consents.client_id` and `access_tokens.client_id` are what make that reasoning checkable rather than assumed: if a change ever lets a never-authorized client acquire a dependent, the sweep fails loudly instead of leaving an orphan, and `oauth.sweep.integration` covers the ordering.
...
- `INSERT` of the root row (`id = parent_id` in the same row) is valid under InnoDB's immediate foreign-key check; `tree.root-row.integration` asserts it on both MySQL lanes so a driver or server change that breaks it fails CI.
...
|---|---|---|
| Row `version` compare-and-set | every mutable metadata row, except `server_settings`, whose rows are written only under the record lock of `schema_meta` `server_settings_version`, the settings document's one CAS validator (§7.1, §13.1; `A13` amended) | `UPDATE … SET version = version + 1 WHERE id = ? AND version = ?`, assert `numUpdatedRows === 1n` |
...
| `access_tokens` | revocation and rotation; `rate_limit_per_hour` through `PATCH /admin/tokens/:tokenId` (PAT rows; `06-mcp-and-agent-access.md` D06-02). `name` is fixed at creation — no route changes it. The last-used flush of `last_used_at`, `last_used_ip` and `last_client` is not a bump (§4) | `version` in the `Token` body of `GET /me/tokens/:tokenId` and `GET /admin/tokens/:tokenId`; token rows carry no `ETag` |
| `oauth_clients` | registration metadata change (a re-fetched CIMD document, an administrator's edit), `status` flipped to `disabled` or back, retirement to `deleted` (§4A); stamping `first_authorized_at` is not a bump | `version` in the client body of `GET /admin/oauth-clients/:clientId`, and the `If-Match` validator for `PATCH`/`DELETE /admin/oauth-clients/:clientId` |
| `oauth_consents` | a widened or narrowed scope set, a changed vault selection, revocation; stamping `last_authorized_at` or advancing `last_used_at` is not a bump (§4A) | `version` in the grant body of `GET /me/oauth-consents`; consent rows carry no `ETag` |
| `server_settings` | any policy change to that group's row (§13.1) | `GET /admin/settings` publishes the grouped document with `ETag: "<version>"` = `schema_meta` `server_settings_version`, incremented once per effective `PUT` under that row's record lock and never below the maximum row version; a row's `version` column records the document version that last changed it and is not a CAS validator (`A13`; 09 D09-10). There is no per-key route |
...
| `schema_meta`, `desktop_releases` | `schema_meta` is operator/migration state written under the migration lock, except `server_settings_version`, which the settings `PUT` increments under that row's record lock and which is itself the settings document's validator (§13.1), and the key versions, which `iridium keys promote` changes in its audited transaction (§13.2); a release row's published fields are immutable (`(version, channel)` is the primary key), and the single mutation it admits — withdrawal — carries its own CAS predicate (`UPDATE desktop_releases SET withdrawn_at = ?, withdrawn_by = ? WHERE version = ? AND channel = ? AND withdrawn_at IS NULL`, §13.3) |
...
Rule: **no service may use an unqualified `UPDATE … WHERE id = ?`** on a table listed in §7.1. The rule is fail-closed, and three frozen `@internal` constants of `apps/server/src/db/cas.ts` define it; they change only together with this paragraph:

| Constant | Members |
|---|---|
| `VERSIONED_TABLES` | the nine tables of §7.1 |
| `CAS_PREDICATE_COLUMNS` | `version`, `status`, `head_seq`, `snapshot_through_seq`, `revision`, `revoked_at`, `deleted_at`, `consumed_at`, `rotated_at`, `disabled_at`, `withdrawn_at` |
| `NON_BUMP_COLUMNS` | `updated_at` on every table; `users.last_login_at` and `users.authz_version`; `vaults.tree_version` and `vaults.root_node_id`; `access_tokens.last_used_at`, `last_used_ip` and `last_client`; `oauth_clients.first_authorized_at`; `oauth_consents.last_authorized_at` and `last_used_at` |

An `updateTable('<versioned table>')` whose `where` chain carries no predicate on a `CAS_PREDICATE_COLUMNS` column is refused, unless its single `.set()` argument is an object literal with no spread and no computed key and every key of that literal is a `NON_BUMP_COLUMNS` member for that table, the values being any expression; that exemption is how stamps that bump no `version`, such as `LastUsedTracker`'s flush (§4), stand outside the rule. A spread, a computed key or a non-literal `.set()` argument is unknown and needs the predicate. An `updateTable` whose table argument is not a string literal is refused, aliases being normalised first, and a raw `sql` or `sql.raw` `UPDATE` of a versioned table is refused outright. `db.cas-discipline.guard` (`apps/server/test/guards/`), an oxc AST scan of the non-test sources under `apps/server/src`, enforces it and fails with the offending file and line; `db.cas.unit` covers the three statement shapes above.
...
| Key generations | `schema_meta['pepper_version' \| 'audit_key_version' \| 'cursor_key_version']`, recorded per row in `user_credentials.pepper_version` and `audit_events.key_version` | whole install | the `schema_meta` value changes only by `iridium keys promote <kind> --to <n>` (ARCH-09); a row records the version in charge when it was written, and transparent re-hash also rewrites `user_credentials.pepper_version` (`A29`) | never reset; a new version is generated by `iridium keys rotate <kind>` (key files only) and put in charge by `iridium keys promote <kind> --to <n>`. `schema_meta['attachment_key_version']` and `attachments.key_version` are reserved, and no command changes them (`G4`, §10.2, §13.2) |
...
Database-level outcomes and the errors they become. Wire details are in `09-api-reference.md`; codes live in `@iridium/contracts/errors.ts`. Two of the codes below are contributed by this section and must be present in that closed enum (`09-api-reference.md` §1.5, D09-1) or `guards.error-shape.guard` fails: `busy` (503, lock-wait timeout, `Retry-After: 1`) and `node_trashed` (409).
...
- The loader never reconstructs a document from `note_projections.markdown`. `collab.initial-state-only-path.guard` greps for `new Y.Doc(` outside `@iridium/crdt`, `collab/persistence/initial-state.ts` and their tests, and `collab.no-reinit.guard` greps for `getText('content').insert` outside `NoteService.initialize`, the restore path and the repair path.
...
`title` maintenance is the one place a structural change touches a projection: when a note is renamed and its `note_projections.heading_title IS NULL`, the rename transaction also runs `UPDATE note_search SET title = ?, updated_at = ? WHERE note_id = ?` (§6.4 step 6). This does not move `revision`, because the body did not change. `projection.title-after-rename.integration` is the regression test.
...
The pair means one set of bytes can be reachable under only one live path. When an upload's bytes already exist under a different path, the API returns the existing row and its existing `path_hint`; the editor inserts a reference to that path rather than creating a second name for the same image. `attachments.dedupe.integration` covers upload-twice, import-duplicate and upload-after-delete.
...
| `export` | `POST /vaults/:id/exports` | `{scope, options, requestId}` | the vault | extension row in `export_jobs` (§11.4); `requestId` is the id of the enqueueing request, copied onto each `access_log` row the export writes (§12.5). No other job payload carries a request id |
...
| `session_ticket_sweep` | schedule | `{}` | — | deletes `sessions`, `password_setup_tokens`, `oauth_authorization_codes` and `oauth_refresh_tokens` rows past their retention, and `oauth_clients` rows with `registration_kind='dynamic'`, `first_authorized_at IS NULL` and `status` not `disabled` (retired rows included) past theirs (§1.4, §4A) |
| `last_used_flush` | — (reserved) | — | — | reserved `JobType` value, kept so the v0.2.0 wire enum does not narrow; no producer, schedule or handler — `LastUsedTracker` owns the flush of `access_tokens.last_used_*` (§4; `06-mcp-and-agent-access.md` D06-49). It is never in `MAINTENANCE_JOB_TYPES`, so `POST /admin/jobs/:type/run` and `iridium jobs run` refuse it |
...
Every note included in an export is written to `access_log` as `surface='export'` with its `revision`, one row per note carrying the `request_id` of the `POST` that queued the job (from `jobs.payload.requestId`, §11.1), written by the export job through `AccessLogWriter`'s durable batch method on its own connection and committed before the job reports success (D03-25, `06-mcp-and-agent-access.md` D06-11; the producer lands at M6), which is what makes "what did this person take" answerable (`A46`).
...
CREATE TABLE audit_events (                                   -- append-only, HMAC-chained per chain_id
  ...
  vault_id        BINARY(16)   NULL,
  target_type     VARCHAR(32)  NULL,                           -- validated at write against AUDIT_TARGET_TYPES; read as an open string
  target_id       BINARY(16)   NULL,
  targets         JSON         NULL,
  outcome         ENUM('success','failure') NOT NULL,
  reason          VARCHAR(128) NULL,
  context         JSON         NOT NULL,                       -- {ip, user_agent, request_id, client, mcp_client, os_user, host, argv_shape}; the last three only for CLI mutations (OPS-19)
...
| Lock order | Follow the owner fence → optional vault gate → explicit node/note/document parents → derived children order in `02-system-architecture.md`. Projection publication holds vault-S before snapshot reads; raw updates and explicit checkpoints omit it. Structural changes hold vault-X, and purge awaits writer disposal outside that lock before revalidating. Credential writers — consents, codes, refresh rows, access tokens, their revocations and the last-used flush — run at `READ COMMITTED` through `apps/server/src/auth/credential-transaction.ts` and follow the credential order declared in the same section (A46). Audit heads are last in both. `lock-order.integration` and `projection.target-lifecycle.integration` exercise actual InnoDB contention; ordering is a tested protocol, not a claim that foreign-key locks cannot deadlock |
...
| `key_version` | Selects the HMAC key. `iridium keys rotate audit` only generates the new key file; `iridium keys promote audit --to <n>` puts it in charge by changing `schema_meta` `audit_key_version` in one migrator-role transaction with `system.key.rotated` on the `server` chain, the first row signed with the new key (ARCH-09, §13.2); verification picks the key by the row's own `key_version`, so every historical key must stay in the encrypted secrets bundle of the backup set (`A47`) |
| `targets` | Used when one action affects many rows (a recursive trash, a purge, `token.revoked_all`): `[{type, id, path?}]` — element for element the shape `AuditEvent.targets` publishes in `09-api-reference.md`, so a stored row needs no translation on read and the strict DTO cannot reject one; `path` carries the derived path for tree targets and is absent for targets that have none (a token, a session) — capped at `AUDIT_TARGETS_MAX` (1 000) entries, so a bulk action is one verifiable event rather than thousands. When it caps, the writer adds its reserved stored metadata key `targets_truncated: true`, which producers may not set (`record()` refuses it as `AuditMetadataReservedKeyError`) and the read path exposes as `AuditEvent.targetsTruncated`. The bulk revocations — the revoke-alls of `04-auth-and-access-control.md` §3.8 and `06-mcp-and-agent-access.md` (D04-19), the client cascades and user deletion — are one event each, whose `targets` list the revoked consents (`oauth_consent`) and then the revoked access tokens (`token`), each group in id order, and whose `metadata` carries the totals; none writes a per-token `token.revoked` or a per-consent `oauth.consent.revoked`. No operation writes chained rows in proportion to server-wide credential counts, and the server chain head, the last lock in every order, is taken once per operation |
| `metadata` | Before/after values of non-content fields only, camelCase with a fixed shape per action, because it is served verbatim as `AuditEvent.metadata` (ARCH-17; the shapes are in `04-auth-and-access-control.md` §11, D04-35). The one writer-owned key is `targets_truncated` above. Note bodies, Markdown, frontmatter values, passwords and token secrets are never written here; `logging-redaction.integration` greps captured audit rows for fixture markers from the hostile corpus. Rows already written keep what they recorded and are never rewritten; readers treat producer metadata as opaque |
| `context` | `{ip, user_agent, request_id, client, mcp_client, os_user, host, argv_shape}`, every member optional; `os_user`, `host` and `argv_shape` are written only by CLI mutations (OPS-19). `request_id` is the server-generated UUIDv7 of the request (ARCH-14) and ties an audit row to the structured log lines and, for token reads, to `access_log.request_id`. The REST edge translates the stored members once into `AuditEvent.context`'s camelCase, an absent member reading as `null` |
...
2. **Triggers** — migrations `0028_audit_events_triggers` and `0029_audit_events_archive` create `BEFORE UPDATE` and `BEFORE DELETE` triggers on `audit_events` and on `audit_events_archive` respectively, each raising `SIGNAL SQLSTATE '45000'` with `MESSAGE_TEXT` `'<table> is append-only'` naming its own table. These bind even the migrator role, so an operator mistake is caught too. The archive job therefore runs its `DELETE` step with the triggers temporarily dropped and re-created inside the same migration-style unit (§12.4), which is the only sanctioned path and is itself audited.
...
`db-grants.integration` asserts all three: the denied privilege, the firing trigger, and a detected tamper after a privileged row rewrite in a scratch database.
...
```sql
CREATE TABLE access_log (                                     -- every token-authenticated read (MCP + REST), every authenticated REST refusal, and the OAuth grant steps; high volume
  ...
  action       VARCHAR(64)  NOT NULL,                          -- mcp.get_note | mcp.resources.read | rest.notes.getMarkdown | oauth.token.issue | …
  vault_id     BINARY(16)   NULL,
  ...
  client_name  VARCHAR(64)  NULL,                              -- declared identity, else the first User-Agent product token (untrusted)
  client_version VARCHAR(32) NULL,
  oauth_client_id BINARY(16) NULL,                             -- oauth_clients.id: bound client on token rows, resolved requested client on authorize/consent rows
  ip           VARBINARY(16) NULL,
  request_id   BINARY(16)   NULL,                              -- the server-generated UUIDv7 request id (ARCH-14); set on every row from M3
...
| `token_id` nullable, `surface` includes `oauth` | The OAuth grant steps of §4A are per-call traffic with a client, a latency and a status, and they belong in the same log as the calls they lead to — an operator reading one connector's story wants authorize → consent → token issue → reads → refresh in one indexed place. Two of the four have no token yet, so `token_id` is `NULL` exactly for `oauth.authorize` and `oauth.consent` and non-`NULL` for everything else, including `oauth.token.issue` and `oauth.token.refresh`, which log the token they just minted. `user_id` is always present: both pre-token actions run behind a live session and are not logged at all if there is no user to attribute them to. `oauth.authorize` writes one row per `GET /oauth/authorize` that carries a live session — `ok` when a code was minted or a consent request created, `denied` for an error redirect or error page (an unresolvable `client_id` included), `rate_limited` for the per-session `429`. `oauth.consent` writes one row per `POST /oauth/consent` that reaches a live session — `ok` for Allow, `denied` for Cancel and for a wrong step-up password, `not_found` for an unknown, expired or other-session request id, `rate_limited` when the login limiter refuses the step-up — and `GET /oauth/consent` writes none; a silent re-authorization at `/oauth/authorize` additionally writes an `ok` `oauth.consent` row from the same request, while only the page's Allow is chained as `oauth.consent.granted`. `oauth.token.issue` and `oauth.token.refresh` write one row per successful grant, with `token_id` the newly minted access token; a refused grant writes none, because its evidence is the `oauth.code.replayed` or `oauth.refresh.reuse_detected` audit row and the metrics. Every `surface='oauth'` row has `vault_id` and `note_ids` `NULL`, and the four rows are appended by `onSend` hooks on the OAuth routes, which see limiter `429`s, error pages, redirects and JSON answers alike. The `NULL`s cost nothing in `ix_access_token_time (token_id, occurred_at)`, which is only ever probed with a concrete token id |
...
| Writes | Two producers, in the order they arrive. The M2 REST observer (`apps/server/src/ops/access-log.ts`) runs in the `onResponse` hook and inserts **one row per logged response** with a single `INSERT`, after the response has been sent and outside the read's own transaction, so no client's latency includes the log write; a failed insert is logged as `access_log.write_failed` and swallowed, because an operational log may neither change an already-sent response nor manufacture a transactional one. From M3, when per-tool-call MCP traffic and the OAuth grant steps multiply the row rate, `AccessLogWriter` (`apps/server/src/audit/access-log-writer.ts`, constructed by the audit plugin) replaces that observer. Every producer appends each row exactly once, as an in-memory push — the `INSERT` never runs on the request path — no later than the hand-off of the response's final byte. REST and the `/oauth` steps append in one `onSend` producer that an `onRoute` hook adds to the end of every logging route's own `onSend` array, so it is the last `onSend` hook of that route and none after it can throw; a per-request flag makes it append at most once, so when an earlier `onSend` hook throws and the error handler's replacement reply re-runs the chain, the one row carries the replacement's status, and the producer itself never throws (a row it cannot build is counted as dropped). There the status and the serialised payload are known: `bytes_out` is the payload length or an explicit `Content-Length`, `NULL` only for a stream without one, and `latency_ms` is measured to the append. MCP responses are hijacked, so `onSend` never sees them: MCP appends in `writeWebResponse` immediately before the `reply.raw.end()` that completes the response — for a stream the server ends, at the stream's end, with `latency_ms` and `bytes_out` measured then — and an MCP reply sent before the hijack appends in `onSend`. A response whose socket closes first — a stream the client or a proxy tears down — appends in its `close` handler, so a request whose connection died still has its row. A `flush()` awaited after a client received a complete response therefore always sees that row and never waits on an open stream; `streamsClosed()` resolves when the writer's count of open MCP streams reaches zero, so a caller that closed its own streams can wait for their rows without polling (`app.accessLog.flush()` is the decorated seam). `append()` truncates `note_ids` to `ACCESS_LOG_MAX_NOTE_IDS` and sets `note_ids_truncated`. The writer inserts with one multi-row `INSERT` on `dbApp`, outside any read transaction, every `ACCESS_LOG_FLUSH_INTERVAL_MS` (2 000) or `ACCESS_LOG_FLUSH_ROWS` (200) rows, whichever comes first, with one flush in flight at a time. Its queue is bounded by `ACCESS_LOG_QUEUE_MAX_ROWS` (10 000) and `ACCESS_LOG_QUEUE_MAX_BYTES` (16 MiB, charged on the serialised row) and drops oldest until both hold; a batch that fails is retried once and then dropped; every lost row increments `iridium_access_log_dropped_total{reason='overflow'\|'write_failed'}`, with a warning at most once per 60 s. The final flush runs in the audit plugin's `onClose`, after every in-flight request has appended its row and before the pools close, under ARCH-06's close-flush deadline, `min(DB_QUERY_TIMEOUT_MS, remaining drain budget − SHUTDOWN_FLUSH_MARGIN_MS)`: a flush whose deadline is already spent acquires no connection, one whose deadline fires is cancelled by destroying its connection, and either way the rows not confirmed written are counted as dropped and `shutdown.flush_abandoned` is logged, so an informational log never turns a clean stop into exit `1`. These are `06-mcp-and-agent-access.md` D06-11's parameters. The writer accepts `surface='export'` rows from M3; the export producer lands at M6 and writes one row per exported note, durably, in batches of `ACCESS_LOG_FLUSH_ROWS` on the export job's own connection before the job reports success, carrying the request id of the `POST` that queued the job, so a large export neither loses its rows nor evicts request rows. Silently blocking reads on an operational log would be worse, and the security-relevant subset (denials) is additionally in the tamper-evident audit chain, which is never dropped. Amended 2026-09-21 (D03-25): the staging replaces an unconditional `AccessLogWriter`, because `12-milestones.md` §7.2 already places the batched writer at M3 while M2 needs a row per refusal for `authz.rest-viewer.integration` — a rate one insert per response carries without a queue. Amended 2026-09-25 (D03-25): the M3 append point is the in-memory push no later than the response's final byte described above — the last `onSend` hook of each route, once per request; `writeWebResponse` for MCP; the `close` handler for a stream the client closes — so a flush covers every response a caller has already received |
| Who is logged | Every token-authenticated read on MCP and REST, the four OAuth grant steps, and — from M2 — every **authenticated refusal** on REST: `401`, `403`, `404` and `429` on any non-`system` principal, a session `user` included. The refusal is the row an operator and `authz.rest-viewer.integration` need most, and outcome rather than principal kind is what decides that an attempt is observable. A refused row resolves nothing about the target it was refused: `vault_id`, `note_ids` and `revision` stay `NULL`, so the log cannot reopen the disclosure the refusal just closed (`04-authentication-authorization-security.md` §5.4). On MCP it is one row per HTTP request on either mount that `patAuth` or `oauthAuth` authenticated, whatever happens afterwards — the kill switch, a rate limit, the handler's outcome or a socket that closed early — and none for an unauthenticated refusal. An MCP row's `status` is the one declared beside the canonical `mcp/errors.ts` text the caller received, so no outcome is resolved twice: `denied` only for the server-wide kill switch, a scope refusal and a cursor refusal; `not_found` for every outcome that answers with the shared not-found text (a note or vault that is absent, trashed, outside the token's vaults or in an MCP-disabled vault), exactly as a REST `404` logs `not_found`; `rate_limited` for the burst or hourly budget; `error` for an ambiguous path, the deadline, a handler failure, a JSON-RPC protocol error or a socket that closed before an outcome was set; `ok` otherwise. A successful **write** is not logged here at all — it belongs to the audit chain of §12.4, which records decisions rather than per-call traffic (D03-25) |
...
| `client_name`, `client_version` | Derived by one module, `apps/server/src/auth/user-agent.ts` (`clientIdentityFrom`), which also serves `access_tokens.last_client` and the audit `context.mcp_client` through `clientLabelOf`: the declared identity (MCP `clientInfo`, or on REST the `X-Iridium-Client` / `X-Iridium-Client-Version` pair), else the first RFC 9110 product token of the `User-Agent`, else `NULL` for both; OAuth rows pass the `User-Agent`. Each value is cut on a code-point boundary to its column width (64 and 32 code points; 120 for the label) with a bounded scan. Untrusted, informational, never used for authorization |
| `oauth_client_id` | The client identity in two strengths — the first column in this plan that has one. On a call authenticated by an OAuth access token, and on `oauth.token.issue` and `oauth.token.refresh`, it is the client the credential is bound to: `access_tokens.client_id`, or the client the code or refresh token was issued to, which the authorization server wrote and nothing a caller sends can influence — the **verified** identity. On `oauth.authorize` and `oauth.consent` it is the registered client the authorization server resolved and validated `redirect_uri` against (for consent, the client held in the pending consent request), `NULL` when the `client_id` did not resolve — a **requested** identity, not yet proven by a credential. It is `NULL` on a PAT call. `client_name`/`client_version` stay the untrusted self-report, and the activity views label the three differently ("Claude · verified connector", "requested client", "self-reported"), because presenting a self-declared name beside a verified one with no distinction is how a log stops being evidence. The column carries **no** foreign key for the same reason the rest of the table does not (partitioned table, and the row must outlive the client): a retired client still resolves, as a tombstone with `status='deleted'` (§4A), while a client removed by the unused-client sweep leaves the id unresolvable, and readers render it as a removed client with the short id, never as an error. It arrived by migration `0048` as `ALTER TABLE access_log ADD COLUMN oauth_client_id BINARY(16) NULL, ALGORITHM=INSTANT` — an `ADD COLUMN` at the end of the row is `INSTANT` on both 8.4.11 and 9.7.2, so a populated partitioned log is not rewritten, and §14.1 records why the statement names no `LOCK` clause |
| `request_id` | Set on every row from M3: the server-generated UUIDv7 of the request that produced the row — Fastify's request id, echoed as `X-Request-Id` and logged as `requestId` — or, for an export row (M6), of the request that queued the export, which the job carries as `requestId` in its payload (§11.1). An inbound `X-Request-Id` is never adopted: a trusted proxy's value is reachable only as the `upstreamRequestId` log field, the join to the proxy's own log (ARCH-14). Correlates with `audit_events.context.request_id` and the structured logs |
...
`audit_events.action` values come from a closed list in `@iridium/contracts/audit.ts` (`AUDIT_ACTIONS`); a zod enum validates every write, and `audit.vocabulary.unit` gives every listed action an accepted row shape — chain scope, credential type, outcome and target. Those cases are row shapes, not producer evidence, so a member joins `AUDIT_ACTIONS` in the change that lands its first producer; the table below lists the one action specified ahead of its producer, marked (M7). `target_type` is validated at write against the closed `AUDIT_TARGET_TYPES` (`record()` refuses any other value with `AuditTargetTypeError`), while stored rows are read as open strings; `token` is the spelling for an `access_tokens` row, as deployed chains already hold it (D04-35).
...
| OAuth | `oauth.client.registered`, `oauth.client.disabled`, `oauth.client.enabled` (M7, the re-enable of `PATCH /admin/oauth-clients/:clientId`), `oauth.client.deleted`, `oauth.client.expired`, `oauth.consent.granted`, `oauth.consent.updated`, `oauth.consent.revoked`, `oauth.refresh.reuse_detected`, `oauth.code.replayed`, `oauth.authorize.denied` |
...
Chain assignment: every action carrying a `vault_id` goes to `vault:<hex>`; everything else goes to `server`. A vault manager may read their vault's chain, including the administrative actions that touched it (`A46`), which is why membership and settings events are vault-scoped rather than server-scoped. Every `oauth.*` action is server-scoped: a grant is made against an account and a client, not inside a vault, even when the vault selection it records names several. The same holds for `token.revoked_all` with scope `vault`: it is a server-chain credential action whose vault is its target, so its `vault_id` is `NULL`. `collab.connection.rejected` is written only for a connection whose ticket identified a live session and whose document resolved to an active vault, on that vault's chain. `mcp.access.denied` alone has the scope `vault_or_server`: it goes to the `vault:<hex>` chain if and only if the call named a vault id that `authorize()`'s vault lookup resolved, and otherwise to `server`, and its `vault_id` and its dedup key `(token_id, resolved vault_id or NULL, reason)` carry the resolved id or `NULL`, so no caller-supplied id ever reaches a vault chain (`06-mcp-and-agent-access.md` D06-03).

`access_log.action` has its own, separate vocabulary — the same closed-list discipline, a different list, because the two tables record different things (§12). Its values are `<surface>.<operation>`: `mcp.<operation>` for MCP calls, over the closed `MCP_OPERATIONS` list in `@iridium/contracts` (the six tool names, the protocol methods, `notifications` for any `notifications/*` method, `batch` for a legacy JSON-array body, and `other` for any other method or unknown tool, never the caller's string), `rest.` followed by the route's OpenAPI `operationId` for REST rows (`rest.notes.getMarkdown`), `export.<operation>` for export (one row per exported note from M6, written durably by the export job), and four values for the authorization server itself — `oauth.authorize`, `oauth.consent`, `oauth.token.issue` and `oauth.token.refresh`. Binding the REST half to the generated operation id is what keeps it closed without a second hand-maintained list: `pnpm gen` exports those ids into `packages/contracts/openapi/openapi.json` and the route set is asserted equal to them, so an action cannot name an operation the specification does not document. The `rest.` prefix is kept even though `surface` already carries `rest`, because a mixed page of `mcp.get_note`, `rest.notes.getMarkdown` and `oauth.token.issue` should read as one vocabulary without a second column (D03-25). The OAuth four are logged here rather than only in `audit_events` because they are per-call operational traffic with a latency, a status and a client, which is exactly what this table is shaped for; the decisions those calls represent (a consent granted, a client registered, a code replayed) are separately and permanently in the audit chain above.
...
One row per policy group, each holding a zod-validated JSON object. A group is the unit of the `admin.settings.changed` audit event, whose `metadata` records the before/after of the group's stored values with secrets elided, and of the per-group write of `PUT /admin/settings`, which serialises on `schema_meta` `server_settings_version` (§13.2); the REST surface presents all groups as **one** document whose `ETag` is that validator, so `If-Match` is taken once for the whole set (09 D09-10). The validator is incremented once per effective `PUT`, under its row's record lock at `READ COMMITTED`, and is never below the maximum row `version`; a row's own `version` records the document version that last changed it and is not a CAS validator (`A13`).

`pat_policy`, `mcp_enabled` and `oauth_policy` are written from M3, the three groups with M3 consumers (AG12). `session_policy`, `password_policy`, `retention` and `desktop_update_policy` are written from M7, each in the change that rewires its consumer; until then those consumers read `EnvSchema`, a `PUT` naming any of them is `422 validation_failed`, and a row of one of those keys, or of the reserved `smtp` key, is ignored with one warning. Settings rows are database state, so a row that fails its group schema, or a key outside this table, is never fatal: before the store's first load the fail-closed `server_settings` readiness check fails, naming the key and the remedy, and the server stays `not_ready` until the row is corrected; once loaded, the server keeps its installed policy and the check warns. A direct change to `server_settings` reaches a running process only when `server_settings_version` is raised above the installed version, which the next readiness evaluation observes, or when a `PUT` reads the table; the remedy for an invalid row is therefore to correct or delete it and increment the validator in the same transaction.

This table is the single definition of the settings vocabulary and the single spelling authority: `ServerSettings` in `09-api-reference.md` §2.15.3 is a projection of it, group for group and field for field (camelCase keys, same names, no additions), and `settings-store.contract` (the shape and the merge) and `admin.settings.integration` (the route and its wire shape) assert that the zod objects of `@iridium/contracts/settings.ts` are the only shape either surface accepts.
...
| `pat_policy` | `{defaultLifetimeDays, maxLifetimeDays, allowNoExpiry: false, rotationOverlapMaxHours, defaultRateLimitPerHour, allowAllVaultsForNonAdmins: boolean}` | token creation and rotation (`A31`), and the OAuth consent and authorization steps (`allowAllVaultsForNonAdmins`, D06-51) |
...
| `pat_rotation_overlap_max_hours` (range 0–24, baseline `PAT_ROTATION_OVERLAP_MAX_HOURS` 24; a rotate request's `overlapHours` defaults to 0) | `pat_policy.rotationOverlapMaxHours` |
| `server_settings.desktop.require_secure_storage` | `desktop_update_policy.requireSecureStorage` — the desktop group, never `session_policy`: `09-api-reference.md` §2.15.3 publishes it as `desktopUpdatePolicy.requireSecureStorage`, a projection of this section, which is authoritative for field spelling |
...
Resolution against the environment: every member has exactly one `EnvSchema` baseline, one range and one merge direction, declared once as `SERVER_SETTING_RULES` in `@iridium/contracts/settings.ts`, beside `VAULT_SETTING_RULES` and in its `SettingStrictness` vocabulary: `'lower'` where the smaller value is stricter (TTLs, lifetimes, the rotation overlap, rate limits), `'higher'` where the larger is (`minLength`, from M7), and `'false'` for a permissive flag, merged by logical AND. The zod group schemas, the environment keys' bounds and the merge all read that one declaration, and each baseline key accepts exactly its member's range, so an effective value can never fail the strict response schema; an out-of-range environment value exits `2` naming the key, the value and the range, and is never clamped. `settings/merge.ts` (`apps/server/src/settings/`, ARCH-31) maps the typed configuration onto the baselines — it never reads the environment — and merges member by member with the shared `stricterOf` and `isWeakerThanBaseline`, taking whichever value is **stricter**. An administrator can therefore tighten policy from the console but never loosen what the operator pinned in the deployment, and the console shows both values with the effective one highlighted. A `PUT` validates only the members it writes, so an environment tightening never makes an unchanged stored member refuse a later `PUT`; the merge clamps such a member, and `GET` shows it beside the floors so the console can offer to clear it. Related members clamp to the stricter value in the environment baselines as in stored values — `defaultLifetimeDays ≤ maxLifetimeDays`, `refreshIdleDays ≤ refreshAbsoluteDays`. An environment pair is refused only when the operator set both keys in contradiction; otherwise the dependent key takes the bound's value with a warning. A `PUT` that writes a member of a related pair and leaves both stored out of order is `422 exceeds_related_field`, and with only one of them stored the effective clamp applies. The baseline of `pat_policy.defaultRateLimitPerHour` is `PAT_DEFAULT_RATE_LIMIT_PER_HOUR`, renamed from `MCP_RATE_LIMIT_PER_HOUR`, which is still accepted as a renamed alias, and that of `pat_policy.allowAllVaultsForNonAdmins` is `PAT_ALLOW_ALL_VAULTS_FOR_NON_ADMINS` (D03-28). A missing row means "use the environment baseline", so a fresh install needs no seeding.

`oauth_policy` follows that rule without extending it. `accessTokenTtlMinutes`, `refreshIdleDays`, `refreshAbsoluteDays` and `defaultRateLimitPerHour` merge as `'lower'`. All three booleans are **permissive** flags (`'false'`) and merge by logical AND, which is why `allowConsentWithoutStepUp` is spelled as a permission to skip step-up rather than as a requirement to perform it: with the existing AND rule, an operator who pins `false` in the deployment cannot have it turned back on from the console, and no new merge mode has to exist. There is deliberately no `allowAllVaultsForNonAdmins` member — the OAuth consent screen reuses `pat_policy.allowAllVaultsForNonAdmins`, because one policy about who may scope a credential to every vault is better than two that can disagree. That flag is permissive, default `true`, and AND-merged with `PAT_ALLOW_ALL_VAULTS_FOR_NON_ADMINS`. While it is `false`, a non-administrator's `all_vaults` is refused wherever a credential's lifetime would restart, with `all_vaults_disabled`: `POST /me/tokens`, the rotation of an `all_vaults` token, and the consent step, which then offers no all-vaults choice; and a client whose live consent has `all_vaults = 1` is re-prompted rather than silently re-authorized. Existing tokens run to their own `expires_at` and existing refresh families to their `absolute_expires_at`. A server administrator is refused `all_vaults` whatever the flag says, with `all_vaults_admin_forbidden`, checked first (`06-mcp-and-agent-access.md` D06-51).
...
| `server_settings_version` | seeded by `0065_server_settings_version` as `COALESCE(MAX(server_settings.version), 0)` (`0` on every existing install, because nothing writes `server_settings` before M3), with `0049`'s idempotent `ON DUPLICATE KEY UPDATE` form; then only by `PUT /admin/settings`, `+1` per effective write under this row's record lock, never lowered and never re-created by the server, because a re-created or lowered validator could reuse an `ETag`. Invariant: never below `MAX(server_settings.version)` | the settings store's load and refresh on every readiness evaluation, and the `PUT` (§13.1); a missing or regressed value fails the `server_settings` readiness check before the first load, warns after it, and answers a `PUT` `503 unavailable`. Its presence and floor are `restore --verify` invariants |
| `pepper_version` | seeded `1` by `0032_schema_meta`; changed only by `iridium keys promote pepper --to <n>` (migrator role, one transaction with `system.key.rotated`, ARCH-09) | credential verification and transparent re-hash (`A29`) |
| `audit_key_version` | seeded `1` by `0032_schema_meta`; changed only by `iridium keys promote audit --to <n>` (migrator role, one transaction with `system.key.rotated`, ARCH-09) | `AuditWriter` (new rows), `verify-chain` (per-row `key_version`) |
| `cursor_key_version` | seeded `1` by `0032_schema_meta`; changed only by `iridium keys promote cursor --to <n>` (migrator role, one transaction with `system.key.rotated`, ARCH-09) | MCP/REST cursor signing (`A35`) |
| `attachment_key_version` | seeded `1` by `0032_schema_meta`; no command changes it — `iridium keys rotate attachment` exits `3` with `not_implemented` and `keys promote` does not accept `attachment` (`G4`) | nothing in the product: seeded and kept so the reserved envelope-encryption seam of §10.2 needs no migration, but `G4` was answered "volume and database encryption only" on 2026-09-12, so no reader exists |
...
`apps/server/migrations/NNNN_<name>.ts`, applied in this order. Every file contains **one** DDL statement, except `0028_audit_events_triggers` and `0029_audit_events_archive`, whose statements are each `IF NOT EXISTS`-guarded and which `migrations.one-ddl.guard` admits by content hash (D03-29); no further exception is admitted. The rule exists because MySQL performs an implicit commit for DDL: a file with two statements can leave the schema half-changed with nothing recorded, whereas one statement per file means the applied set and the schema can only ever disagree about a single object, which an idempotent guard then resolves on re-run.

Every table under this heading lists its migrations as ``| NNNN | `name` | … |`` rows, and `migrations.plan-parity.guard` holds their concatenation, `NNNN_name` in table order, equal to `MIGRATION_NAMES` in `apps/server/src/db/migrations.ts`, in order and with no number repeated; `migrations.admission.guard` holds that registry equal to the migrations directory. This list is therefore the authoritative ordered record of the migration set: the repository guide restates no migration number, range or content, and any section that names a migration (12 §7.2's M3 row, a decision entry) follows this list's numbering.
...
Every later table gets its own `NNNN_<table>` migration plus a companion `NNNN_<table>_grants`, and `db-grants.integration` fails if a table exists in `information_schema.TABLES` without a matching grant (§2), so the pair cannot be forgotten.
...
| 0048 | `access_log_oauth_client` | `ALTER TABLE access_log ADD COLUMN oauth_client_id BINARY(16) NULL, ALGORITHM=INSTANT` (§12.5) — no `LOCK` clause, because `ALGORITHM=INSTANT` admits only `LOCK=DEFAULT` (MySQL refuses any other `LOCK` with ERROR 1221), so naming the algorithm is the no-rebuild guarantee: a change the engine could not make instantly fails the statement instead of silently rebuilding the table. No grants companion, because `access_log`'s grants are table-level and already in place |

Every one of the fourteen OAuth statements, `0035`–`0048`, runs unchanged on MySQL 8.4.11 and 9.7.2 under the dialect rule of §1.1: the only features they use are `ENUM`, `JSON`, a `VIRTUAL` generated column with a unique key over it, foreign keys added to an existing table (`0046`), and `INSTANT` `ADD COLUMN` at the end of a partitioned table. `credential_type`'s `'oauth'` value is **not** among them — `audit_events` is created by `0026` with the value already in its `ENUM` (§12.1), because the whole authorization server ships in the same release and an `ALTER` to a table created eight migrations earlier in the same set would be ceremony, not safety.
...
M3 adds forward migrations after the immutable `0001`–`0059` set, numbered in landing order:

| ID | Name | Contents |
|---|---|---|
| 0060 | `oauth_codes_access_token` | One `ALTER TABLE oauth_authorization_codes` adding `access_token_id`, `replay_detected_at` and `fk_oauth_codes_token` (§4A, D03-26); no `ALGORITHM` clause, because a foreign key cannot be added instantly and the table holds no row on any deployment before M3 |
| 0061 | `oauth_clients_retire` | One `ALTER TABLE oauth_clients`: `client_id` to `VARCHAR(512) CHARACTER SET ascii COLLATE ascii_bin`, the `status` `ENUM` appends `'deleted'`, `ADD deleted_at`, `ADD deleted_by`, `RENAME COLUMN last_authorized_at TO first_authorized_at` (which `ix_oauth_clients_unused` follows), `DROP INDEX uq_oauth_clients_client_id` (D03-27) |
| 0062 | `oauth_clients_live_client_id` | `ADD COLUMN live_client_id`, the `VIRTUAL` generated column `IF(status <> 'deleted', client_id, NULL)` |
| 0063 | `oauth_clients_uq_live` | `UNIQUE uq_oauth_clients_live (live_client_id)` — its own file for the reason `0011` and `0038` are |
| 0064 | `oauth_refresh_reuse_stamp` | `ALTER TABLE oauth_refresh_tokens ADD COLUMN reuse_detected_at DATETIME(6) NULL, ALGORITHM=INSTANT` (D03-26) — its own file under the one-DDL rule |
| 0065 | `server_settings_version` | Data migration: one idempotent `INSERT` of the `schema_meta` validator `server_settings_version` = `COALESCE(MAX(server_settings.version), 0)`, in `0049`'s `ON DUPLICATE KEY UPDATE` form (§13.1, §13.2) |
| 0066 | `oauth_consents_last_used_at` | `ALTER TABLE oauth_consents ADD COLUMN last_used_at DATETIME(6) NULL, ALGORITHM=INSTANT` |

None of them needs a `_grants` companion, because every table they touch already carries its table-level `iridium_app` grants (`0034`, `0036`, `0039`, `0043`, `0045`). If another change lands a migration first, the numbers follow the landing order; nothing edits `0001`–`0059`. Each file holds one statement under §14.2's rule — one DDL statement, or for the data migration `0065` one `INSERT` — and runs unchanged on both lines: the set stays inside the 8.4.11 floor, and `migrations.parity.integration` asserts it on both images. It uses a foreign key added to an existing table (`0060`); a column charset conversion, an appended `ENUM` value and a column rename on `oauth_clients`, a table no release before M3 has written (`0061`); a `VIRTUAL` generated column with a unique key over it (`0062`, `0063`); and `INSTANT` `ADD COLUMN` (`0064`, `0066`). `0061` renames and converts in place rather than by §14.4's add, backfill, switch and drop, because that rule protects code that reads the column across a rolling release, and no release before M3 writes `oauth_clients` or reads `last_authorized_at`.
...
| Rule | Detail |
|---|---|
| One DDL statement per file | Above, with exactly two exceptions, the M0 files `0028_audit_events_triggers` (2 statements) and `0029_audit_events_archive` (3), each statement `IF NOT EXISTS`-guarded, pinned by the closed allowlist `apps/server/test/guards/migrations.one-ddl.allowlist.json` of `{file, sha256, ddlStatements, reason}`; a hash mismatch, a count mismatch or a stale entry fails, and no further exception is admitted (D03-29). `migrations.one-ddl.guard` is a fail-closed oxc AST scan of `apps/server/migrations/*.ts`. A statement root is the receiver of an `.execute(` call: a `sql` tagged template; `sql.raw` of a string or template literal; a `db.schema` builder chain (DDL); a `selectFrom`, `insertInto`, `updateTable`, `deleteFrom` or `replaceInto` chain (DML); or `db.transaction().execute(callback)`, whose callback is scanned in place. Any other receiver — an identifier holding a query, a call result — is refused as an unknown statement form, and every root outside the `down` export counts toward `up`. The leading keyword is read from the literal or the template's first quasi, and a statement that begins with an interpolation is refused; `CREATE`, `ALTER`, `DROP`, `RENAME` and `TRUNCATE` are DDL, while DML and `GRANT`/`REVOKE` are not. A `sql` fragment that is not a statement root (interpolated into another template, passed to `sql.join`) is not classified. A call to an imported function must name one of `applyGrants`, `indexExists`, `columnExists` and `currentSchema`, and a DDL root inside a loop or an iteration callback (`map`, `forEach`, `flatMap`, `reduce`, `some`, `every`, `filter`) is refused |
...
| Rename a column | never in place: add, backfill, switch, drop — except where no earlier release writes the table or reads the column, which is why `0061` renames `oauth_clients.last_authorized_at` to `first_authorized_at` and converts `client_id` in place (§14.1, D03-27) |
...
erDiagram
  ...
  access_tokens ||--o| oauth_authorization_codes : minted_by_exchange
...
| `oauth_consents` / `oauth_clients` → `access_tokens` | `consent_id`, `client_id` | `fk_tokens_oauth_consent`, `fk_tokens_oauth_client`, RESTRICT | Both `NULL` for `kind='pat'`, both set for `kind='oauth'`, together with `resource` (invariant I-24). RESTRICT costs nothing here: a consent is never deleted, a retired client keeps its row, and the unused-client sweep reaches only clients with no access token (I-29) |
| `oauth_refresh_tokens` → `access_tokens` | `refresh_id` | **no** foreign key | Provenance only: the refresh row issued with the token (the family's first row at code exchange, the new rotated row at refresh), `NULL` for a PAT and for an OAuth token issued to a client whose `grant_types` exclude `refresh_token`. A constraint would pin every refresh row forever, because token rows are never deleted (§4) |
| `access_tokens` → `oauth_authorization_codes` | `access_token_id` | `fk_oauth_codes_token`, RESTRICT | `NULL` until the successful exchange, which sets it with `consumed_at` (invariant I-27). The constraint never fires: token rows are never deleted, and a code row is swept 24 hours after it expires |
...
`iridium doctor` runs every register check, I-01 … I-29, in order, prints one line per check with a count, and exits non-zero on any violation. Each check is selectable as `--invariant I-nn` (repeatable), and the named flags of `11-operations-and-deployment.md` select groups of them: `--heads` is I-07 and I-08, `--checkpoint-stale` I-10, `--stale-projections` I-11, `--credentials` I-21 and I-24–I-29, `--db-roles` I-23, and `--attachments` I-17's sampled store walk. A register check ships with the milestone that ships the code producing the rows it checks (`12-milestones.md` D12-24): the M0–M2 backlog shipped at M3, and `doctor.integration` proves each check against a named deliberate corruption wherever the schema admits the violation and holds the implemented id set equal to this register, so a missing check fails a test. `restore --verify` (M8) runs the complete register plus the blocking content and attachment comparisons of `A47`, and refuses the restore on violation.
...
| I-04 | `notes.vault_id = nodes.vault_id` for every note | protocol (both immutable) | `doctor` | reported; OPS-17 keeps mutations under `iridium repair`, and this repair's subcommand is decided with the M8 repair group |
...
| I-14 | `trash_entries` has a row exactly when `nodes.deleted_at IS NOT NULL`, and every row's `cascade_root_id` is itself a trashed node of the same vault | database (FK) + protocol | `doctor` | reported; OPS-17 keeps mutations under `iridium repair`, and this repair's subcommand is decided with the M8 repair group |
...
| I-19 | Every `audit_events.action` is in the closed vocabulary and every `chain_id` matches `^(server\|vault:[0-9a-f]{32})$` | database (`VARCHAR` + zod at write) | `doctor`, `audit.vocabulary.unit` | none |
...
| I-21 | Token consistency: `expires_at IS NOT NULL` for every row; `admin_owned = 1` implies `all_vaults = 0`; `rotated_from_id` chains are acyclic and reference rows of the same `user_id`; a revoked row has `revoked_at` and, when the revocation had an actor, `revoked_by` | database (`NOT NULL`) + checker | `doctor --credentials` (M3), `token.effective-permissions.prop` | reported |
...
| I-24 | Credential-kind consistency: every `access_tokens` row with `kind='oauth'` has non-`NULL` `client_id`, `consent_id` and `resource`, and every row with `kind='pat'` has all three `NULL` | checker (the columns are individually nullable because one table serves both kinds) | `doctor --credentials` (M3), `oauth.principal-parity.prop` | reported per token; a row in this state is refused at verification anyway, so the repair is revocation, not a backfill |
| I-25 | Refresh-chain consistency: within one `family_id`, at most one live row (`revoked_at IS NULL AND rotated_at IS NULL`) exists, every `rotated_from_id` names a row of the same family, the chain is acyclic, and no row's `expires_at` exceeds its family's `absolute_expires_at` | protocol (rotation in one transaction) + checker | `doctor --credentials` (M3), `oauth.refresh-rotation.integration` | none automatic: two live rows in a family is the reuse signal the rotation path exists to detect, so `doctor` reports the family and the operator revokes it |
| I-26 | `oauth_consent_vaults` is empty for every consent with `all_vaults = 1`; and `admin_owned = 1` implies `all_vaults = 0`, the same pairing I-21 asserts for a token | checker | `doctor --credentials` (M3) | delete the stray rows — `all_vaults` wins, exactly as it does for a token's `access_token_vaults` |
| I-27 | OAuth grant-credential provenance: (i) on every `oauth_authorization_codes` row, `consumed_at IS NOT NULL` ⟺ `access_token_id IS NOT NULL`, and the referenced token has `kind='oauth'` and the code's `user_id`, `client_id` and `consent_id`; (ii) `replay_detected_at IS NOT NULL` ⟹ `consumed_at IS NOT NULL`; (iii) on every `oauth_refresh_tokens` row, `reuse_detected_at IS NOT NULL` ⟹ `rotated_at IS NOT NULL OR revoke_reason IN ('refresh_reuse','code_replayed')` (`REUSE_SIGNAL_REVOKE_REASONS`) | protocol (the one successful-exchange `UPDATE`; the first-detection stamps under the row lock, §4A) | `doctor --credentials` (M3), which the default `doctor` run also executes; `oauth.authorization-code.integration`, `oauth.refresh-rotation.integration` | reported |
| I-28 | A disabled or retired client holds nothing unrevoked: no `oauth_consents`, `oauth_refresh_tokens` or `access_tokens` row of a client whose `status` is `disabled` or `deleted` has `revoked_at IS NULL` | protocol (the client cascades revoke in the transaction that changes the status, §4A) | `doctor --credentials` (M3) | reported |
| I-29 | A never-authorized client has no dependents: a client with `first_authorized_at IS NULL` has no consent, authorization-code, refresh or access-token row; every client with a consent has `first_authorized_at` set; and no row with `registration_kind='cimd'` has `first_authorized_at IS NULL` | protocol (the first consent stamps `first_authorized_at`; a CIMD row is created by that Allow, §4A) | `doctor --credentials` (M3), `oauth.sweep.integration` | reported |

`doctor` is designed to be safe on a live system: every check is a `SELECT`, reads are chunked by note id with `LIMIT`, and the expensive ones (I-12's re-projection, I-17's store walk) run only when explicitly requested or as part of `restore --verify`. `iridium doctor --db-roles` is the check added for I-23 (decision D03-02). `A57`'s own breadth list covers `--argon2`, `--stale-projections`, `--yjs-instances`, `--repair-heads` and `--repair-content`; the remaining flags this register cites — `--invariant`, `--credentials`, `--db-roles`, `--attachments`, `--oversize`, `--checkpoint-stale`, `--sizes` — and the mutating twins they prescribe are specified in `11-operations-and-deployment.md`, under OPS-17 (a mutation lives in the `iridium repair …` group, requires `--yes`, is audited, and refuses to run unless `doctor` currently reports the matching finding) and OPS-39 (the commands added beyond `A57`). I-04 and I-14 name no `doctor` mutation flag, because OPS-17 keeps every mutation under `iridium repair`: their findings are reported, and whether their repairs become `iridium repair` subcommands or stay manual is decided with the M8 repair group.
...
`restore --verify` and `doctor` share one implementation of each register check: the M2 content checks (I-07, I-08 and I-11's stale projections) in `apps/server/src/cli/doctor-content.ts`, the `--credentials` group in `apps/server/src/cli/doctor-credentials.ts` and every other check in `apps/server/src/cli/doctor-register.ts` (`12-milestones.md` D12-24). `doctor.integration` proves each against a named deliberate corruption wherever the schema admits the violation and holds the implemented id set equal to §16.1, so a missing or divergent check fails a test. The three suites above assert their oracles against their own models, not through the checker.
...
| D03-09 | The settings merge combines the `EnvSchema` baseline with the `server_settings` row field by field, taking whichever value is **stricter**, with the direction (`min`/`max`/logical AND) declared per field; a missing row means "use the baseline". **Amended 2026-09-25.** The direction is declared once per member in `SERVER_SETTING_RULES` with the `SettingStrictness` values `'lower'` (min), `'higher'` (max) and `'false'` (logical AND of a permissive flag), and the merge is `settings/merge.ts`; related members clamp to the stricter value both in the environment baselines and in stored values (§13.1, D03-28) | `A26` says "env values are floors" without defining the direction for each field; a TTL and a minimum password length are strict in opposite directions, and an administrator must never be able to loosen what the operator pinned |
...
| D03-15 | One invariant register (`I-01` … `I-29`) implemented once in `apps/server/src/db/invariants.ts` and consumed by `iridium doctor`, `iridium restore --verify` and the three model property suites. **Amended 2026-09-25.** The register checks are implemented in `apps/server/src/cli/doctor-content.ts` (the M2 content checks I-07, I-08 and I-11), `apps/server/src/cli/doctor-credentials.ts` (the `--credentials` group) and `apps/server/src/cli/doctor-register.ts` (every other check), which `doctor` and `restore --verify` both run; the model property suites assert their oracles against their own models, and `doctor.integration` holds the implemented id set equal to §16.1 (§16.3; `12-milestones.md` D12-24) | `A47` and `C.5` name overlapping invariant sets for different tools; a single implementation is the only way the checker and the tests cannot drift. As amended, the single implementation is per check and shared by the two tools, and what stops the checker drifting from the register is a test that fails on a missing or undetecting check |
...
| D03-21 | `iridium_backup` additionally holds `REPLICATION SLAVE`, `BACKUP_ADMIN` and `SHOW_ROUTINE` globally, and the grant verification — `db-grants.integration` assertion (e) and `iridium doctor --backup-role` — executes the shipped dump command from one exported constant (`MYSQLDUMP_ARGV`) rather than a shorter hand-written command | `A8`'s privilege list predates the dump command the plan now ships: `--single-transaction` with `--source-data` takes `LOCK INSTANCE FOR BACKUP` (needs `BACKUP_ADMIN`) on MySQL 8.0.21+, `--routines` needs `SHOW_ROUTINE` without global `SELECT`, and `mysqlbinlog --read-from-remote-server` needs `REPLICATION SLAVE`. A test that exercises a shorter command would pass while `iridium backup` failed in production or in a drill, which is the one failure mode a backup role must not have |
| D03-22 | `system.audit.archived` is part of the closed audit vocabulary (§12.6), written on the `server` chain by the archive path with `{chain_id, from_id, to_id, rows, export_path, export_sha256}` | `11-operations-and-deployment.md`'s archive procedure emits it, and the vocabulary is enforced by a zod enum plus `audit.vocabulary.unit`: without the value, step 5 of the documented procedure throws and `iridium audit archive` cannot complete. Recording where a chain's earlier rows went is also what keeps a prefix-archived chain self-explanatory to an auditor |
...
| D03-24 | Two columns the OAuth design (§4A) touches deviate from the shape it would otherwise imply, and both deviations are deliberate: `access_tokens.refresh_id` carries **no** foreign key to `oauth_refresh_tokens`, and `access_log.token_id` becomes nullable while `access_log.surface` gains the value `oauth`. **Amended 2026-09-25.** `refresh_id` has one meaning, the refresh row issued in the same token response as the access token, and code replay reads it backward from a code's `access_token_id` (D03-26) | Each resolves a contradiction between the OAuth tables and a rule this section already owns, and resolving it in the schema is the only place it can be resolved once. A `RESTRICT` foreign key on `refresh_id` would make §4A's refresh-token retention rule unreachable, because token rows are never deleted (`A31`): every refresh row would be pinned by the access tokens issued with it, forever. `refresh_id` is provenance of exactly the kind `rotated_from_id` already is, and §1.4 lists both. For the log, two of the four OAuth actions (`oauth.authorize`, `oauth.consent`) happen before any token exists, so a `NOT NULL` `token_id` would have forced them into a separate table or out of the log entirely; keeping them here is what lets an operator read one connector's whole story — authorize, consent, issue, read, refresh — from one partitioned, indexed place, and the `NULL` is confined by a stated rule (§12.5) rather than left as a general nullable column |
| D03-25 | `access_log` has two producers in sequence, and a wider REST scope than "every token-authenticated read": the M2 REST observer (`apps/server/src/ops/access-log.ts`) inserts one row per response in `onResponse`, synchronously and outside the read's transaction, and covers every authenticated refusal — `401`, `403`, `404`, `429` on any non-`system` principal — while resolving nothing about the refused target; `AccessLogWriter`'s batching, bounded queue and drop metric arrive with M3's MCP, export and OAuth producers. REST `action` values are `rest.` followed by the route's OpenAPI `operationId`. **Amended 2026-09-25.** From M3 the batched `AccessLogWriter` replaces the M2 observer, and every producer appends each row exactly once, as an in-memory push, no later than the hand-off of the response's final byte: REST and the `/oauth` steps in an `onSend` producer that an `onRoute` hook makes the last `onSend` hook of each route and a per-request flag limits to one append, so a chain Fastify re-runs after a throwing hook still yields one row with the final status; MCP in `writeWebResponse` immediately before the `reply.raw.end()` that completes the response; and a stream the client closes, in its `close` handler (§12.5). A flush therefore covers every response a caller has already received. The writer accepts `surface='export'` from M3; the export producer, one row per exported note written durably by the job with the queuing request's id, lands at M6 and supersedes `08-markdown-pipeline-import-export.md`'s single aggregate export row. `oauth_client_id` is the client bound to the credential on token-authenticated and token-issuing rows, and the resolved requested client on `oauth.authorize` and `oauth.consent` rows | M2 ships REST refusals before any MCP traffic exists, and `authz.rest-viewer.integration` depends on exactly one observable denied attempt per refused route, so the producer has to exist at M2 while the batching parameters answer a volume that does not yet — `12-milestones.md` §7.2 already assigns the batched writer to M3, and writing §12.5 as one unconditional batched writer made the milestone that needs the rows the milestone that cannot have them. Scoping the refusal case by principal kind rather than by outcome would hide a session user's denied attempt from the operator view that exists to show denied attempts, and the audit chain deliberately carries decisions rather than per-call traffic (`A46`). Binding REST actions to the generated operation id keeps the vocabulary closed by construction rather than by a parallel list nobody regenerates |
| D03-26 | Code-replay provenance and first-detection stamps (§4A). Migration `0060` adds `oauth_authorization_codes.access_token_id` (nullable, foreign key `fk_oauth_codes_token` to `access_tokens`, no `ALGORITHM` clause) and `replay_detected_at`; migration `0064` adds `oauth_refresh_tokens.reuse_detected_at`. The one `UPDATE` of a successful exchange sets `access_token_id` together with `consumed_at`, and nothing else ever sets `consumed_at`; `access_tokens.refresh_id` is uniformly the refresh row issued in the same token response. The exchange authenticates the client (by `live_client_id`, D03-27) and checks the code's secret on unlocked reads first, then locks in the credential order of `02-system-architecture.md` "Lock order" (A46) and re-validates, checking the client binding first; a mismatch leaves the code unconsumed and is logged and counted, never chained. A replay, possible only with the secret, revokes exactly the code's access token and, through its `refresh_id`, the family and every access token issued with it; it is chained once per code, by the transaction that performs the `replay_detected_at` NULL→now transition, with zero counts included, and every detection is logged and counted. Refresh reuse is a rotated row, or a row of a family already revoked as a theft response (`REUSE_SIGNAL_REVOKE_REASONS`: `refresh_reuse`, `code_replayed`), and uses the same stamp on the presented row; a refresh row revoked by a person, an administrator, the client or a cascade is an ordinary `invalid_grant`, never a reuse event. I-27 checks all three columns through `doctor --credentials` | `consumed_at` alone says that an exchange happened, not what it produced, so a replayed code could not revoke its own issuance without a provenance column; one column suffices, because the access token's `refresh_id` leads to the family. Revoking the whole consent would also cut the user's unrelated sessions of the same connector, and a `family_id` copy on the code row would be a second provenance that can disagree with the first. Comparing the secret before any other check, and never writing `consumed_at` on a failure, keeps a holder of the public id16 alone from burning a legitimate code or triggering a revocation. Stamping first detection under the row lock bounds the chained rows to one per presented credential across every process and restart, which an in-process window cannot, while each later replay still reaches the log and the counter. A client mismatch and a refresh token revoked for an ordinary reason are routine misuse or retries, and chaining them would put caller-driven volume into the tamper-evident chain |
| D03-27 | `oauth_clients.client_id` is `VARCHAR(512) CHARACTER SET ascii COLLATE ascii_bin`, unique among non-retired rows through the `VIRTUAL` `live_client_id` (`IF(status <> 'deleted', client_id, NULL)`) and `uq_oauth_clients_live` (migrations `0061`–`0063`), which replace `uq_oauth_clients_client_id (client_id(191))`; every lookup of a presented client_id is `WHERE live_client_id = ?`. Deleting a client retires it — the disable cascade with `client_deleted` in the same transaction, then `status='deleted'`, `deleted_at` and `deleted_by` — and a disabled client keeps its row and its `client_id` (`06-mcp-and-agent-access.md` D06-43). `last_authorized_at` is renamed `first_authorized_at` and set once by the first consent, so the client row is exclusively locked once per client lifetime; the per-grant time stays on `oauth_consents.last_authorized_at`, from which the client DTO's `lastAuthorizedAt` is derived. Credential writers lock in the credential order that `02-system-architecture.md` "Lock order" and A46 declare. The unused-client sweep reaches only dynamic rows that were never authorized and are not disabled, retired rows included; a CIMD row is created authorized (D06-41). Invariants I-28 (a disabled or retired client holds nothing unrevoked) and I-29 (a never-authorized client has no dependents) | A hard delete would either fail on the client's `RESTRICT` keys or orphan every consent, token, `access_log` row and audit target that names it, and allowing deletion only of never-authorized clients would leave an administrator no way to remove a connector that was ever used; a tombstone keeps every reference resolvable and still frees the identifier for a new registration. The former prefix key over a case- and accent-insensitive collation could equate two distinct client identifiers, while `ascii_bin` makes the match exact and puts the whole value in the key. Stamping the client row on every authorization would serialise every user's silent grant for a popular connector on one exclusive row lock; a once-set `first_authorized_at` answers the only question the schema asks of the client row — was it ever authorized — and the per-grant time already lives on the consent. The rename and the charset conversion are made in place, against §14.4's add, backfill, switch and drop, because no release before M3 writes `oauth_clients` or reads `last_authorized_at` |
| D03-28 | One declaration per server-settings member. Every member of `pat_policy`, `mcp_enabled` and `oauth_policy` (and, at M7, of the other groups) has exactly one `EnvSchema` baseline, one range and one merge direction, declared once as `SERVER_SETTING_RULES` in `@iridium/contracts/settings.ts` with `VAULT_SETTING_RULES`'s `SettingStrictness` vocabulary and one shared weaker-than-baseline comparison, and read by the zod schemas, the environment bounds and `settings/merge.ts`, which maps typed configuration onto the baselines and never reads the environment. §13.1 is the single spelling authority. Environment keys accept exactly the member range; a value that v0.2.0 accepted outside it exits `2` naming the key and the range, announced by the M3 changeset's `[config]` flag (OPS-29). Related members clamp to the stricter value: in the environment a pair is refused only when the operator set both keys in contradiction, and otherwise the dependent key takes the bound's value with a warning; in stored settings a `PUT` that writes a member of a pair and leaves both stored out of order is `422 exceeds_related_field`. `rotationOverlapMaxHours` is 0–24 with baseline 24 (a rotate request's `overlapHours` defaults to 0); `accessTokenTtlMinutes` 5–1 440; the refresh windows 1–366; both hourly defaults 60–100 000, with the baselines `PAT_DEFAULT_RATE_LIMIT_PER_HOUR` (renamed from `MCP_RATE_LIMIT_PER_HOUR`, still accepted as a renamed alias) and `OAUTH_DEFAULT_RATE_LIMIT_PER_HOUR`; `PAT_ALLOW_ALL_VAULTS_FOR_NON_ADMINS` (true) is the new baseline of `allowAllVaultsForNonAdmins`. The five shared bounds — `PAT_LIFETIME_DAYS_MAX` (366), `PAT_ROTATION_OVERLAP_HOURS_MAX` (24), `OAUTH_ACCESS_TOKEN_TTL_MINUTES_MIN` (5), `OAUTH_ACCESS_TOKEN_TTL_MINUTES_MAX` (1 440) and `OAUTH_REFRESH_DAYS_MAX` (366) — are `LIMITS` members | A member with no environment form would need a second merge path, and a range declared in three places drifts until an effective value fails the strict response schema. Keeping the `MCP_` spelling for a PAT-only default reads as covering both MCP mounts. Refusing a pair when only one side was set would make hardening one key fatal, and clamping an out-of-range environment value could silently lengthen a lifetime or a TTL. Naming 09's DTO or any other text as the spelling authority left two sources that had already disagreed |
| D03-29 | §14.2's one-DDL-per-file rule admits exactly two exceptions, `0028_audit_events_triggers` (2 statements) and `0029_audit_events_archive` (3), immutable M0 files whose statements are each `IF NOT EXISTS`-guarded. `migrations.one-ddl.guard` pins them by content hash in its closed allowlist, and no further exception is admitted. Accepted 2026-09-25; supersedes `A7` in part (its one-DDL-per-file clause) | The two M0 files shipped immutable with more than one guarded statement; the rule is kept for every other file and the exception set is closed and content-pinned |
