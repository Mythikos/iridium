...
`innodb_lock_wait_timeout` keeps the server default for the pools as configured, while `withVaultLock` issues `SET SESSION innodb_lock_wait_timeout = 5` for its transaction so a queued structural request fails fast instead of holding a request slot, and the persistence writer uses `10` on `dbPersist`. `lock-order.integration.test.ts` (M2 exit) runs concurrent trash, rename, edit and audit traffic against one vault and asserts no deadlock is observed over the run and that every conflict surfaces as one of the explicit errors above.
...
| `sv_after` | `Y.encodeStateVector(doc)` captured synchronously when the update was applied — the value the `persisted` message carries and the value `afterLoadDocument` reads to initialise `lastPersisted` without decoding the document. |
...
| `min_client_version` | release migration / admin | `/meta`, the desktop "update required" screen |
...
| `users` → `password_setup_tokens` | `user_id` | `fk_spl_user`, RESTRICT | At most one unconsumed row per purpose; older rows are marked consumed |
