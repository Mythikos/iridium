...
| Writes | Two producers, in the order they arrive. The M2 REST observer (`apps/server/src/ops/access-log.ts`) runs in the `onResponse` hook and inserts **one row per logged response** with a single `INSERT`, after the response has been sent and outside the read's own transaction, so no client's latency includes the log write; a failed insert is logged as `access_log.write_failed` and swallowed, because an operational log may neither change an already-sent response nor manufacture a transactional one. From M3, when per-tool-call MCP traffic, export streaming and the OAuth grant steps multiply the row rate, `AccessLogWriter` takes that hook over: it batches rows in memory and inserts them with a multi-row `INSERT` every 500 ms or every 500 rows, the queue is bounded at 10 000 rows and drops oldest on overflow, a batch that fails is retried once and then dropped, either drop increments `iridium_access_log_dropped_total` with a throttled `WARN`, and the queue is flushed during the shutdown drain. These are the same parameters as `06-mcp-and-agent-access.md` D06-11. Silently blocking reads on an operational log would be worse, and the security-relevant subset (denials) is additionally in the tamper-evident audit chain, which is never dropped. Amended 2026-09-21 (D03-25): the staging replaces an unconditional `AccessLogWriter`, because `12-milestones.md` §7.2 already places the batched writer at M3 while M2 needs a row per refusal for `authz.rest-viewer.integration` — a rate one insert per response carries without a queue |
| Who is logged | Every token-authenticated read on MCP and REST, the four OAuth grant steps, and — from M2 — every **authenticated refusal** on REST: `401`, `403`, `404` and `429` on any non-`system` principal, a session `user` included. The refusal is the row an operator and `authz.rest-viewer.integration` need most, and outcome rather than principal kind is what decides that an attempt is observable. A refused row resolves nothing about the target it was refused: `vault_id`, `note_ids` and `revision` stay `NULL`, so the log cannot reopen the disclosure the refusal just closed (`04-authentication-authorization-security.md` §5.4). A successful **write** is not logged here at all — it belongs to the audit chain of §12.4, which records decisions rather than per-call traffic (D03-25) |
