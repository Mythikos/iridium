...
| `/admin/agent-activity` | the server-wide `access_log` view: filter bar (token, user, vault, surface, status, time range, OAuth client — the `oauthClientId` chip, whose options come from `/admin/oauth-clients` and whose labels distinguish a **verified connector** from a **self-reported client**, the same distinction the client name column carries everywhere else because `clientInfo` is untrusted display text and a verified `client_id` is not), counters for the selected range (calls, notes read, denials, rate-limited), virtualised table, row-detail drawer (action, vault, note ids resolved to paths and titles at read time, revision, latency, bytes, the client rendered as "<name> · verified connector" from `access_log.oauth_client_id` or "<name> · self-reported" from `client_name`/`client_version` — never merged into one column — IP, request id) and "Export" behind step-up | `/admin/agent-activity`, `/admin/agent-activity/export` |
...
| Tests | `collab.ipc-websocket.spec` (unit, against a stub main), `desktop.host-contract.e2e` parameterised over both modes, and the `three-instances` E2E run in fallback mode in the nightly matrix so the path never rots even if it is not the default |
...
| Blocking case | When the server raises `minClientVersion`, `CompatibilityGate` reaches `client-too-old` on the next `426 client_outdated` and shows its blocking screen (8.2). At 1.0 the desktop branch of that screen shows the download link, the file name and the SHA-256 for this platform and architecture, plus the copy controls; under `policy: 'disabled'` it shows the contact-an-administrator text unchanged; "Export my text" stays reachable, unchanged |
...
| `a11y-keyboard-only` (E2E) | the keyboard-only walk of 9.1 executed in the shell — create, rename, move, trash and restore a note with no pointer, including a keyboard drag — plus the assertion that every native-menu item resolves to the same `CommandId` the palette resolves to | ubuntu (xvfb), windows, macos |
