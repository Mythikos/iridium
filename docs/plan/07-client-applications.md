...
| Release-feed hosting, checksum publication, fleet rollout, deployment | 11-operations-and-deployment.md |
...
One codebase, two hosts, two different commitments. The **desktop application is Iridium's supported client at 1.0** and the **web host is a development and internal surface** (01-vision-scope-and-principles.md §4.6). Nothing in this section's mechanisms follows from that: the seam, the three host implementations, the contract suite and the boundary tags are identical for both hosts, and `apps/web` ships in the server image exactly as specified in section 6. What follows from it is which lanes block a merge (10-testing-and-quality.md), which proof gates a spec §9 acceptance row (12-milestones.md §2.3), and the wording of section 9.3.

One exception to "every user-facing screen is a `@iridium/ui` route" exists and it is deliberate: the OAuth consent screen at `/oauth/consent` is rendered by the server (06-mcp-and-agent-access.md D06-31). It must work before any application bundle has loaded, must carry no application JavaScript under the nonce CSP, and must not put the single-use `request_id` into client state or a history entry the router manages. It inlines its own stylesheet, `apps/server/src/oauth/consent.css`, under the response's style nonce and references nothing under `/app/`; from M4 `pnpm gen` writes that stylesheet's custom properties from `packages/ui/src/theme/tokens.css`, so it does not look foreign. Its permission lines are `READ_PERMISSION_LABELS` from `@iridium/contracts` (06 D06-40), the strings the token dialog shows through `en.ts` (9.2). It is not a route of `@iridium/ui` (D07-47).
...
| `updates` | `null` on the web and in the desktop `IRIDIUM_E2E=1` / `--allow-insecure-server` modes; otherwise IPC-backed (see 7.15). At 1.0 `check()` resolves to `disabled`, `idle` or `manual-download` and `install()` always rejects with `updates_manual_only`; the method stays on the seam so the post-1.0 epic re-enables a handler rather than re-adding a channel (D07-44). |
...
|---|---|---|---|
| `/login` | signed-out only, except that a live session with a valid `return_to` leaves at once by `location.assign(return_to)` instead of routing to the vault selector | `redirect?: string` (same-origin route path only); `return_to?: string` (honoured only when the raw value begins with the literal `/oauth/authorize?` and `new URL(value, location.origin)` has the page's origin and the pathname `/oauth/authorize`; left by `location.assign`, a full-document navigation, because the target is a server route; anything else is ignored). When both are present, a valid `return_to` takes precedence and `redirect` is ignored; an invalid `return_to` is ignored and `redirect` applies by its own rule, exactly as if `return_to` were absent (AG9) | Electron shows the server step first when no profile is active |
...
| `/download` | public | `platform?: 'win32' \| 'darwin' \| 'linux'`, `arch?: 'x64' \| 'arm64'` (both defaulted from the running host, both droppable) | `GET /desktop/update-policy`; renders `latest.artifacts[]` — file name, size, SHA-256, download link and the per-OS verification command — with the viewer's own platform and architecture first |
...
| `/settings/profile` · `/settings/sessions` · `/settings/integrations` · `/settings/appearance` · `/settings/about` | signed-in | — | `/me`, `/me/sessions`, `/me/tokens`, `/me/oauth-consents`, `/me/connector-setup`; `/settings/about` additionally reads `host.updates` and `iridium:app:info` |
| `/admin/users` · `/admin/vaults` · `/admin/tokens` · `/admin/oauth-clients` · `/admin/agent-activity` · `/admin/audit` · `/admin/settings` · `/admin/releases` · `/admin/system` | server admin | per page (filters, cursor) | `/admin/*` (see 11-operations-and-deployment.md for the console's operational content) |
...
- `/download` is public because a user who has no desktop client yet has no session, and because an administrator hands the URL `https://<origin>/app/download` to a new starter. It renders the same data in both hosts; on Electron a click on an artefact link goes through `shell.openExternal` like every other external link (5.12), so the bundle is downloaded by the user's browser and the shell never writes an executable to disk.
...
The OAuth authorize bounce (09 §2.19.2, step 4) is the only producer of `return_to`: a browser that reaches `/oauth/authorize` with no live session, on a server whose entry document declares the sign-in route (6.2), is answered `302` to the relative `/app/login?return_to=<the authorize request's own path and query, percent-encoded>`, never an absolute URL. The rule of 3.2 is the SPA half of threat T18's control, and `login.return-to.component` covers it: the rule itself, the precedence over `redirect` in both directions, the live-session exception and the refusals. The query is not inspected further, because the server re-validates every authorize parameter. An ignored `return_to` never changes the existing behaviour: the user lands wherever `redirect` or the guard would have sent them.
...
| Virtualisation | fixed row height 28 px, `overscan: 12`; the 10 000-node fixture scrolls at ≥ 55 fps (`perf.workspace.e2e`, 9.4). |
...
| Empty and error states | "No matches in this vault" with the parsed query echoed; `422 validation_failed` shows the operator help inline; `429 rate_limited` shows a countdown. |
...
| Remote carets | `@iridium/editor`'s own `iridiumRemoteSelections` plugin (same decorations, class names and awareness contract as `y-codemirror.next`'s, with a caret that writes its colour through the CSSOM so a nonce policy accepts it — spike S4), themed through CSS variables; the name flag fades after 3 s of caret inactivity and reappears on movement |
...
The acknowledgement contains both insertion clocks and deleted-item identities: `persisted.sv` must dominate `localSv`, and `persisted.ds` must equal `localDs`. The latter is a fixed 64-character lowercase SHA-256 fingerprint of the canonical Yjs delete set, captured with the vector on every local or relayed update. A deletion can leave the vector unchanged, so an older vector alone must never make deleted text look saved. A stale or unrelated acknowledgement keeps the pending-work warning until both witnesses match; reconnecting on the same `Y.Doc` preserves the previous acknowledgement and local-edit deadline. The encoding and mandatory wire fields are defined in 09 §3.4 and the rule order remains owned by 05.
...
| `saved` | `saved` (rule 14) ← `unsynced === 0 ∧ dominates(persisted.sv, localSv) ∧ persisted.ds === localDs ∧ closeReason === null` | "Saved" | neutral, check glyph | editable | tooltip: "Committed to the server database at <time>" |
...
| `syncing` | `syncing` (rule 13) ← `unsynced > 0 ∨ persisted === null ∨ ¬dominates(persisted.sv, localSv) ∨ persisted.ds !== localDs` | "Syncing…" | subtle, animated dot (static under reduced motion) | editable | — |
| `save-failed` | `save-failed` (rule 12) ← `persistFailed` newer than `persisted`, or `(¬dominates ∨ persisted.ds !== localDs) ∧ now − lastLocalEditAt > 15 000` | "Not saved — retrying" | danger, alert glyph | editable | "Export my text", "Details" (reason from `persistFailed.reason`: `db_unavailable`, `db_error`, `too_large`, `backpressure`, `content_invalid`), retry countdown |
...
1. `saved` requires vector dominance and an equal delete-set fingerprint; a fake provider that reports `synced` with no `persisted` message must keep the pill in `syncing` (test `saved-requires-dominance`). A delete-only edit with unchanged clocks must also remain pending until the acknowledgement carries its deletion witness.
...
### 4.15 Integrations: tokens and authorized applications

This page is the product surface of MCP access (A31, A34, F14) and is specified in full here because it is where a user hands an agent its credential — either an integration token the user copies into a client, or an OAuth grant the user approves on the consent screen (06-mcp-and-agent-access.md, "MCP authentication and the two credential channels"). The two audiences use two different URLs and the page never lets them blur: clients that sign in use `<origin>/mcp/connect`, clients that send a token header use `<origin>/mcp`.

| Element | Detail |
|---|---|
| Token list | `GET /me/tokens`, paged with `nextCursor`, lists integration tokens only: an OAuth access token is not addressable on `/me/tokens`, and an application's access is managed per grant under **Authorized applications** below (06 D06-50). The default page holds live tokens (`active` and `rotated`), and `includeInactive` adds expired and revoked ones. Each row: name, display prefix `irid_pat_<id16>`, scopes ("Read" in MVP), vault scope ("3 vaults" or "All vaults I am a member of"), created, expires (with a warning chip inside 14 days), last used (`lastUsedAt` and `lastClient`, which the last-used flush writes on the token row), rate limit, status (`active`, `expired`, `revoked`, `rotation overlap until <time>`). |
| Create | `TokenCreateDialog` (step-up required): name, scope bundle (only "Read" is selectable, and the dialog lists the six permissions it grants with `en.ts`'s `permission.<scope>` strings, 9.2; reserved write scopes are not listed), vault selection (multi-select of the user's memberships, or the "All vaults I am a member of" switch), expiry (30 / 90 / 180 / 365 days, filtered by `policies.patMaxLifetimeDays` from `GET /meta`; "no expiry" is offered only when `policies.patAllowNoExpiry` is true). The switch is hidden while `policies.patAllowAllVaultsForNonAdmins` is false, because the server then refuses `vaults: 'all'` from a non-admin with `422 validation_failed` `all_vaults_disabled` (06 D06-51). Server admins see the switch, where it is shown, disabled with the explanation from F4 ("integration tokens never inherit administrator access; select the vaults you are an explicit member of") and a warning banner that the token will be audited as admin-owned. |
...
| Snippets | `SnippetsPanel` renders per-client configuration from `GET /me/tokens/:tokenId/snippets` (omitting `client` fetches every client), whose response is `{serverOrigin, mcpUrl, snippets: Snippet[]}` (09 §2.4). The tab set is **two** enums from `@iridium/contracts/src/tokens.ts`, rendered as two groups: `SnippetClientSchema.options` for the **token audience** (every tab points at `<origin>/mcp`) and `ConnectorClientSchema.options` for the **connector audience** (every tab points at `<origin>/mcp/connect` and carries no secret, rendered by the "Connect a connector" panel of this table). Each is the one definition of its client enum (06-mcp-and-agent-access.md D06-22) and each is iterated at render time rather than copied into the UI, so a client added to either enum gets a tab with no client change and the `?client=` value can never be one the server rejects. The token-audience group renders as cards: "Claude Code" (`claude-code`, the `claude mcp add --transport http` command, plus `claude-code-mcp-json`, a committable `.mcp.json` using an environment variable), "Claude Desktop" (`claude-desktop` through the bundled bridge, with `mcp-remote` beneath it as the pinned documented alternative), "Editors" (`cursor`, `vscode`, `windsurf`), "API and scripts" (`messages-api`, `curl`, `custom-mcp-client`) and "claude.ai connector (request header)" (`claude-ai-connector-header`, shown with the reachability note the response carries, since it needs a public origin, and with the caveat the response also carries: per-connector request headers are in beta and available to a limited set of organisations, the value is sent verbatim so `Bearer ` including its trailing space must be typed, and the authentication choice cannot be edited after the connector is created). The connector-audience `claude-ai-connector` tab is the OAuth one and is the route a claude.ai or Claude Desktop custom connector should normally take. Each snippet renders `Snippet.title`, its `format` (`shell` / `json` / `text`), `Snippet.file` as the one-line "where this file lives" hint (`null` for a shell command or prose), and every string in `Snippet.notes`; the secret is substituted **client-side** for the `{{IRIDIUM_MCP_TOKEN}}` placeholder in `Snippet.template` by the copy-with-secret action inside the reveal dialog, so the server never returns it again. The stdio tabs use the bundled bridge: the desktop substitutes `<bridge-path>` with the absolute `resources/bin/iridium-mcp.mjs` path from `iridium:app:info.bridgePath`, and the web links to `GET /desktop/tools/iridium-mcp-<version>.mjs` (with its `iridium-mcp-latest.mjs` redirect and `GET /desktop/tools/SHA256SUMS`, 11 OPS-65) and substitutes the downloaded file's path. The `.mjs` extension is part of the contract in both places, because the generated snippet runs the file as `node <bridge-path>` (06 D06-14); `iridium-mcp` is the package's `bin` name, never a filename. |
| Rotate | step-up; optional overlap hours bounded by `policies.patRotationOverlapMaxHours` from `GET /meta` (the published effective value of `pat_policy.rotationOverlapMaxHours`, 8.1), with the dialog stating exactly when the old secret stops working; the new secret goes through the same reveal component. |
| Revoke | step-up; confirmation states that agents using it lose access immediately (A23), and states in the same sentence that revoking a token does **not** revoke a connector's authorization — an application that signed in with OAuth holds a separate grant, and **Authorized applications** below is where that is revoked. |
| Authorized applications | A second card below the token list, from `GET /me/oauth-consents`: client name, a verification badge (`Verified identity` for a Client ID Metadata Document, with the `client_id` URL shown on hover; `Unverified — this application registered itself` for a dynamically registered client; `Registered by an administrator` for a manually registered one), the vaults the grant covers, the scopes, granted date, last used (`OAuthConsent.lastUsedAt`, the grant's newest access-token use; per-grant read history is available to administrators only, in the admin agent-activity view of M7, 06 D06-50), and a **Revoke** action (step-up) whose confirmation states that the application loses access immediately and that its next refresh will fail. Component test `authorized-apps.component`. |
| Connect a connector | A panel rendering `GET /me/connector-setup` per client from `ConnectorClientSchema` (`claude-ai-connector`, `claude-desktop-connector`, `claude-code-oauth`, `vscode-oauth`, `cursor-oauth`, `mcp-remote-oauth`), with no secret and no `{{IRIDIUM_MCP_TOKEN}}` placeholder anywhere, and a prominent line saying which URL each audience uses: "Clients that sign in use `<origin>/mcp/connect`. Clients that send a token header use `<origin>/mcp`. Do not mix them." The panel is reachable without an existing token, because an OAuth connector needs none. It renders only while the authorization server is mounted — `GET /meta`'s `features[]` carries `oauth` and the response's own `oauthEnabled` is true (06-mcp-and-agent-access.md `MCP_OAUTH_ENABLED`); on a site that has unmounted the OAuth surface the page shows the token half alone, while **Authorized applications** above keeps rendering so a grant made before the switch can still be revoked. |
...
| `/settings/about` | client version, server version and the versions from `iridium:app:info` (Electron, Chromium, Node, platform, architecture, packaged, `secureStorage`); licence and third-party notices; and, on the desktop host, the update card of 7.15. At 1.0 the card is accompanied by a permanent line stating that this build has no in-application updater and naming `/app/download` as where new versions come from. In the web host the About page shows the same versions and links to `/app/download`, and renders no update card |
...
| `/admin/oauth-clients` | every registered OAuth client: client name, registration kind with the verification badge (`Verified identity` for a Client ID Metadata Document, `Unverified — registered itself` for a dynamically registered client, `Registered by an administrator` for a manual one), `client_uri`, redirect URIs, `application_type`, status, created, last authorized, and live consent and token counts, and the actions **Disable** (`PATCH /admin/oauth-clients/:clientId {status:'disabled'}` with `If-Match`) and **Delete**, each behind step-up and a typed confirmation that states every consent, refresh token and access token for that client is revoked in the same transaction, plus **Register a client** (the manual confidential-client form, step-up, whose secret is shown once through the same `TokenSecretReveal` component the PAT flow uses). A row opens the consent list for that client; a second tab lists every live consent server-wide with owner, client, vaults, scopes, granted and last used, each revocable. `logo_uri` is never rendered anywhere on this page. Component test `oauth-client-list.component` | `/admin/oauth-clients*`, `/admin/oauth-consents/:consentId` |
| `/admin/agent-activity` | the server-wide `access_log` view: filter bar (token, user, vault, surface, status, time range, OAuth client — the `oauthClientId` chip, whose options come from `/admin/oauth-clients` and which matches `access_log.oauth_client_id`, so it selects both the client a credential is bound to on token-authenticated, `oauth.token.issue` and `oauth.token.refresh` rows (**verified connector**) and the client a request named on `oauth.authorize` and `oauth.consent` rows (**requested client**), and never the **self-reported** `client_name`; these are the three labels the client column carries everywhere else, because `clientInfo` is untrusted display text, a requested client is unproven and only a bound `client_id` is verified (06 D06-23)), counters for the selected range (calls, notes read, denials, rate-limited), virtualised table, row-detail drawer (action, vault, note ids resolved to paths and titles at read time, revision, latency, bytes, the client rendered as "<name> · verified connector" from `access_log.oauth_client_id` on a token-authenticated row and on `oauth.token.issue` and `oauth.token.refresh` rows, where it is the client the credential is bound to; as "<name> · requested client" from the same column on `oauth.authorize` and `oauth.consent` rows, where it is the registered client the authorization server resolved and validated `redirect_uri` against, which the request named but no credential proves (06 D06-11); or as "<name> · self-reported" from `client_name`/`client_version` — never merged into one column — IP, request id) and "Export" behind step-up | `/admin/agent-activity`, `/admin/agent-activity/export` |
...
| `/admin/settings` | grouped policy forms (session TTLs, PAT policy, password policy, retention, MCP kill switch, desktop update policy, and the **OAuth** card beside the MCP card, carrying the whole `oauth_policy` group — `accessTokenTtlMinutes`, `refreshIdleDays`, `refreshAbsoluteDays`, `defaultRateLimitPerHour`, `allowDynamicClientRegistration`, `allowClientIdMetadataDocuments`, `allowConsentWithoutStepUp` — whose registration switch states that turning it off also removes `registration_endpoint` from the published authorization-server metadata on the next request) with `If-Match`, step-up, and an explicit note on each field whose environment value is a floor | `/admin/settings` |
...
| Cost control | rebuilding a view for a 100 KB note must stay inside the 300 ms "note open" budget (9.4); the measurement is part of `perf.workspace.e2e`. |
...
export function buildExtensions(opts: EditorOptions): Extension[] {
  return [
    ...
    yCollab(opts.ytext, null, { undoManager: opts.undoManager }),  // 4 sync + undo only; a null
    yRemoteSelectionsTheme,                            //   awareness switches upstream's
    opts.awarenessCompartment.of(                      //   yRemoteSelections off entirely (S4)
      iridiumRemoteSelections(opts.ytext, opts.isAwarenessOwner ? opts.awareness : null)
...
- **`EditorView.cspNonce`** is fed from the page nonce (web: injected into `index.html`, 6.2; Electron: generated per window load, 7.3). M0 spike S4 (`docs/spikes/S04-editor-csp-nonce.md`, 12-milestones.md §4.4) measured CodeMirror's generated `<style>`, the remote-selection theme, `rehype-highlight` classes and Base UI positioning under `style-src 'self' 'nonce-<n>'` with zero `securitypolicyviolation` events, so the facet is what this relies on and no stylesheet is extracted. The one component that failed was `y-codemirror.next`'s remote-caret widget, which sets `style` as an attribute; the row above is the executed remedy. Two consequences of the same measurement land outside this file: the web host substitutes **`reply.cspNonce.style`** into the placeholder, because `@fastify/helmet` with `enableCSPNonces: true` emits a different nonce for `script-src` and for `style-src` (6.2); and `@iridium/ui` mounts `<CSPProvider nonce={…}>` at its root, because `@base-ui/react`'s `Select` and `ScrollArea` render a React 19 hoisted `<style>` that is refused without one, and the provider takes the same page nonce that reaches this facet.
...
|---|---|---|
| Note open (tree click → editor ready) | `performance.mark('iridium:note-open:start')` in the tab-open command and `performance.mark('iridium:note-open:ready')` when the view's first `updateListener` fires after the initial state, measured between them | `perf.workspace.e2e` Playwright test on the 100 KB fixture (budget < 300 ms, median of 10) |
| Preview render | `PreviewResult.timing` carries `prescanMs`, `parseMs`, `hastMs` and `sanitizeMs` from the worker; the client adds the React commit duration measured with a `PerformanceObserver` on long tasks | `perf.workspace.e2e` (p95 < 100 ms for a 100 KB note) and, on regression, the parser switch criterion recorded in A42 |
| Tree scroll | `requestAnimationFrame` timestamps collected in the page over a 2 s programmatic scroll of the 10 000-node fixture, asserted on the 5th-percentile frame interval | `perf.workspace.e2e` (≥ 55 fps) |
...
**Status at 1.0.** The web host is a development and internal surface, not a supported product surface (01-vision-scope-and-principles.md §4.6). Everything specified in this section is still built, still shipped inside the server image and still tested on every pull request — the `chromium` end-to-end lane and the Vitest Browser Mode component project are merge-blocking, because this host is how `@iridium/ui` is developed and how most of the UI suite runs. It runs in current Chrome and Edge (9.3). Firefox and WebKit are out of scope, so this section states no behaviour for them, and the security controls below are non-negotiable regardless of the support commitment: a surface served to anybody is attack surface.
...
The nonce is injected by substitution, not by templating engine: at boot the server reads `index.html` once, splits it on the literal placeholder `__IRIDIUM_CSP_NONCE__` (emitted by the Vite HTML transform into the `<meta name="iridium-csp-nonce" content="…">` tag and into any `<style>`/`<script>` attribute that needs it), and per request writes the pieces with `@fastify/helmet`'s generated nonce (decision D07-10). The nonce written is **`reply.cspNonce.style`**, not `reply.cspNonce.script`: with `enableCSPNonces: true` the plugin generates a different nonce for each directive and exposes both, and the page's one placeholder feeds `EditorView.cspNonce`, so substituting the script nonce would refuse every CodeMirror style under a policy that reads as correct (spike S4). The application reads the nonce from the meta tag and passes it to `EditorView.cspNonce` (5.4). A boot assertion fails if the placeholder is absent from the built HTML, so a build-configuration change cannot silently ship a nonce-less page.

From M4, under G13's working default, the entry document also carries `<meta name="iridium-sign-in" content="/app/login">`, added in the same change that adds the `/login` route's `return_to` handling (3.2, 4.2); the `iridium-csp-nonce` meta is its precedent. The server reads the entry document once at boot, with the same read that checks the nonce placeholder, and bounces a sessionless OAuth authorize request to `/app/login` only when the marker is present; without it, the authorize endpoint answers the `access_denied` error redirect instead of redirecting to a route it does not serve. A marker naming any other path refuses boot with exit 2, because a bundle and a server that disagree about the sign-in route are a build defect (AG9).

Headers on `/app/*`. The policy string below is the **single normative** web CSP (decision D07-41): 02-system-architecture.md §B, 10-testing-and-quality.md's `security.headers.integration` row and 12-milestones.md's M4 row point at it rather than restating it, and that test compares the response against the one committed fixture rendered from it — the nonce matched by pattern and `<PUBLIC_HOST>` substituted from the test environment — so a policy change is a one-place edit and cannot pass a test that asserts a different string:
...
| `files.uploadImport(jobId, source, onProgress)` | a `fetch` `PUT` with a `ReadableStream` body built from the selected files (multipart frames generated incrementally) and `duplex: 'half'`; progress from bytes enqueued. A browser below the feature floor of 9.3 falls back to a `FormData` upload without progress |
...
The shell is deliberately thin: it supplies platform capabilities the browser cannot (native pickers, a secret store, a protocol handler, an update check, deep links, a native menu) and it holds the only reusable credential in the product. Every hardening item below is mandatory and has a named test; nothing in this section is optional configuration. It is also Iridium's supported client at 1.0 (01-vision-scope-and-principles.md §4.6): every hardening item and every acceptance proof named in this section is a release gate rather than a parity nicety, and the Electron end-to-end specs that discharge a spec §9 row run on every pull request across all three operating systems (10-testing-and-quality.md, "`ci.yml`").
...
apps/desktop/
  src/main/
    ...
    updater.ts         version comparison + policy from the server (no electron-updater at 1.0)
...
|---|---|---|---|
| H1 | Sandbox everywhere | `app.enableSandbox()` at module top level; `webPreferences.sandbox: true` | `desktop.web-preferences.guard` |
| H2 | Context isolation | `contextIsolation: true`, `nodeIntegration: false`, `nodeIntegrationInWorker: false` | `desktop.web-preferences.guard` |
| H3 | No webview, no drag-drop navigation | `webviewTag: false`, `navigateOnDragDrop: false` | `desktop.web-preferences.guard` |
| H4 | DevTools only unpackaged | `devTools: !app.isPackaged` | `desktop.web-preferences.guard` |
| H5 | Safe dialogs | `safeDialogs: true` (blocks dialog spam from content) | `desktop.web-preferences.guard` |
...
| H8 | CSP | per-load header from the active profile; nonce for `style-src` | `desktop.csp.spec`, `desktop.hardening.e2e` |
| H9 | Navigation lock | `will-navigate` → `preventDefault()` unless the target origin is `app://iridium` (parsed with `new URL`) | `desktop.hardening.e2e` (a hostile link cannot navigate) |
| H10 | Window opening | `setWindowOpenHandler` → `{ action: 'deny' }`; `https:`/`mailto:` routed to the validated `shell.openExternal` | `desktop.hardening.e2e` |
...
| H13 | IPC origin | every `ipcMain.handle` reads `event.senderFrame?.origin` synchronously at the top and requires `app://iridium` (plus the Vite dev origin when `!app.isPackaged`) | `ipc.origin.guard` |
| H14 | IPC payloads | zod parse from `@iridium/contracts/desktop-ipc.ts` before any service call; failures return a `ProblemDetails` with `validation_failed` and are logged | `contracts.desktop-ipc.unit`, `desktop.ipc-contract.e2e` |
| H15 | Preload surface | fixed wrappers only; no `ipcRenderer` pass-through, no channel parameter reaching `invoke`, `event` stripped from every callback | `desktop.preload-surface.guard` (snapshot of exposed keys + a grep assertion) |
| H16 | Fuses | `runAsNode:false`, `enableCookieEncryption:true`, `enableNodeOptionsEnvironmentVariable:false`, `enableNodeCliInspectArguments:false`, `enableEmbeddedAsarIntegrityValidation:true`, `onlyLoadAppFromAsar:true`, `grantFileProtocolExtraPrivileges:false` | `desktop.fuses.guard` reads the packaged binary's fuse wire |
| H17 | No credential in the renderer | `window.iridium` exposes no token getter; `iridium-attachment://` and `IpcTransport` keep the bearer in main; the scheme handler re-emits `FORWARDED_ATTACHMENT_HEADERS` verbatim and omits `Cross-Origin-Resource-Policy` (7.7) | `desktop.attachments-no-token-in-renderer.e2e` (including the `Range` → `206` case); `desktop.hardening.e2e` asserts the forwarded header set |
...
| H19 | Deep links | zod-validated, never navigate the window, unknown server prompts | `desktop.deep-link-fuzz.e2e` |
| H20 | Content untrusted | hostile-markdown corpus rendered in the packaged app | `desktop.hostile-markdown.e2e` |
...
| `iridium:updates:check\|install` | `{}` → `{ state: UpdateState }` · `{}` → `{ ok: true }` | 7.15; at 1.0 `install` is registered and always rejects with `409 updates_manual_only`, so the channel and its contract test survive the deferral of in-application updates (D07-44) |
...
There is no synchronous IPC (`ipcRenderer.sendSync`) anywhere, and `contracts.desktop-ipc.unit` and `desktop.ipc-contract.e2e` assert that the set of registered `ipcMain` channels equals the set in `@iridium/contracts/desktop-ipc.ts` — neither side can add a channel unilaterally.
...
| macOS at 1.0 | Keychain access (and the `enableCookieEncryption` fuse) require a code-signed application. The 1.0 macOS bundle is **ad-hoc** signed (7.14), which is enough for the operating system to execute an arm64 binary but is not a stable identity a Keychain item can be bound to, so `isEncryptionAvailable()` is `false`, `iridium:app:info.secureStorage` reports `'unavailable'` and **every macOS installation runs in memory-only mode**: the secret lives in the main process for the life of the process, `secrets.bin` is never written, and the user signs in again after every quit. The login screen states this as product behaviour, not as a fault. The administrative consequence is stated with the setting rather than left to be discovered: `requireSecureStorage = true` makes the macOS client unable to sign in at all until the post-1.0 signing epic ships, so the setting stays `false` by default and `docs/ops/desktop-distribution.md` says so in the same sentence that describes it. **Assumption:** that an ad-hoc signature does not satisfy the Keychain is this plan's reading of the digest's statement that `EnableCookieEncryption` and Keychain access require a code-signed app. `desktop.macos-secure-storage.e2e` asserts the behaviour the product depends on — whatever `isEncryptionAvailable()` returns, `app.info`, the login screen and the on-disk state agree, and no secret is written when it is false — plus the expected `false`, so if an ad-hoc build does reach the Keychain the test records it and this row is corrected rather than the product being wrong |
...
| Tests | `collab.ipc-websocket.spec` (unit, against a stub main), `desktop.host-contract.e2e` parameterised over both modes, and the `desktop.three-instances.e2e` run in fallback mode in the nightly matrix so the path never rots even if it is not the default |
...
`--allow-insecure-server` is the only way to reach an `http://` origin. It is a developer affordance with teeth: it disables credential persistence (memory-only, 7.6), disables the update check entirely (7.15), so `host.updates.check()` resolves to `{status:'disabled', reason:'insecure-server'}`, and paints a permanent warning strip in the title bar. `desktop.insecure-mode.spec` asserts all three effects.

Certificate handling:

| Layer | Behaviour |
|---|---|
| Default trust | Chromium's verifier plus the OS trust store. Enterprise roots are picked up from the Windows certificate store (GPO-deployed), the macOS Keychain (MDM profile) and — on Linux — the NSS shared database `~/.pki/nssdb` (`certutil -d sql:$HOME/.pki/nssdb -A -t "C,," -n "Corp Root" -i corp-root.crt`), not `/etc/ssl/certs`. `docs/ops/deployment.md` carries the per-OS instructions. M0 spike S7 (`docs/spikes/S07-electron-enterprise-ca.md`, 12-milestones.md §4.4) ran on Windows only and **deferred this leg to a standing test**: a `@smoke` Playwright electron case per operating system installs a throwaway CA by the platform mechanism and asserts `net::OK` with `isIssuedByKnownRoot === false`, then asserts the handshake fails once the CA is removed. Until that case is green the OS-store route is documented but unverified, which is why the pin below — not this row — is the route the deployment documentation recommends |
| Fingerprint pin (the recommended private-CA route) | S7 measured this path accepting exactly the pinned (hostname, leaf SHA-256) pair for renderer `fetch`, renderer WebSocket, renderer subresources and main `session.fetch`, while leaving public TLS intact, so it is the primary documented route for a private-CA deployment on all three operating systems and needs no change to the machine's certificate stores. `session.setCertificateVerifyProc((request, callback) => …)` returns `0` **only** when `request.hostname === new URL(activeProfile().origin).hostname` **and** the SHA-256 fingerprint of `request.certificate` equals `activeProfile().pinnedCertSha256`; every other case returns `-3` (defer to Chromium). Returning `0` also disables Certificate Transparency checks and the result is cached by the network service, which is precisely why the exception is scoped to one host and one fingerprint. The proc is registered on the profile's session and its verdict is cached there, so **every main-process request must issue on that session** (`session.fetch`, never a bare `net.fetch`) or it bypasses the pin; the CI greps of 7.4 carry that rule beside `webSecurity: false` and `ignore-certificate-errors` |
...
| Update check and transfers | the update-policy fetch and `net.fetch` use the same Chromium stack, so a pinned or OS-trusted certificate covers the update check, uploads and attachment fetches with no second configuration surface. At 1.0 nothing downloads a bundle through this stack: the download is handed to the user's browser by `shell.openExternal` (7.15), so a pinned certificate does not apply to it and the artefact's SHA-256 is what the user checks instead |
### 7.12 Deep links
...
| Testing | `desktop.deep-link-fuzz.e2e` (Playwright electron) drives several hundred generated URLs — wrong scheme, missing parameters, `note=../`, huge values, embedded newlines, `javascript:` inside parameters, duplicated keys, percent-encoding tricks — and asserts that the window never navigates, no profile is created, no unhandled rejection occurs, and exactly the valid links produce a route change |
...
### 7.14 Packaging, fuses and 1.0 distribution

#### 7.14.1 What 1.0 produces

At 1.0 the desktop client is distributed as an unsigned application bundle per operating system and architecture, downloaded over HTTPS from the site's own Iridium server, with a published SHA-256 checksum per artefact. There is no installer, no code signature, no notarisation and no in-application updater. Code signing (Apple Developer ID with notarisation, Azure Trusted Signing), installer formats (NSIS, MSI) and in-application automatic updates are a single post-1.0 epic (7.14.6); every seam they need is kept, so the epic is a change to `apps/desktop/electron-builder.yml`, `apps/desktop/src/main/updater.ts`, `apps/desktop/package.json`, `.github/workflows/release.yml` and four documentation files — not a redesign.

| Platform | electron-builder target | Architectures | Artefact name |
|---|---|---|---|
| Windows 10/11 | `zip` | `x64`, `arm64` | `Iridium-<version>-win32-<arch>.zip` |
| macOS 13+ | `zip`, ad-hoc signed (`mac.identity: "-"`) | `x64`, `arm64` | `Iridium-<version>-darwin-<arch>.zip` |
| Linux | `tar.gz` | `x64`, `arm64` | `Iridium-<version>-linux-<arch>.tar.gz` |

Six artefacts per release. `<version>` is the single Changesets product version (A1, A54, D12-1) — the same string the server image tag, `GET /meta.serverVersion`, `X-Iridium-Client-Version` and `iridium:app:info.appVersion` carry.

The platform segment of every file name uses the **`process.platform` vocabulary** — `win32`, `darwin`, `linux`, never `win`/`mac` — because `desktop_releases.files[].platform` and the `Platform` enum of 09-api-reference.md §2.16 already fix that vocabulary and the plan carries exactly one spelling for it. The names are produced by an explicit per-platform `artifactName` template rather than by electron-builder's `${os}` macro, so the vocabulary cannot drift with a macro's definition.

**Why `tar.gz` on Linux and `zip` on Windows and macOS.** A zip archive does not reliably carry the Unix executable bit or symlinks, so an extracted Linux bundle would not be runnable; `tar.gz` carries both. On macOS electron-builder's `zip` target is produced with `ditto`, which preserves the `.app` bundle structure and its permissions, and it is also the format Squirrel.Mac consumes, so the epic inherits it unchanged. On Windows there is no executable bit and `zip` is the format Explorer opens without extra software.

The checksum published beside each artefact is SHA-256, lowercase hex, in three places generated from one computation: `GET /desktop/updates/<channel>/SHA256SUMS` (GNU coreutils `sha256sum` format), `GET /api/v1/desktop/update-policy` → `latest.artifacts[].sha256`, and `GET /admin/releases` → `files[].sha256`. The publication rules, the generation step and the administrator's out-of-band check belong to 11-operations-and-deployment.md; the wire shapes belong to 09-api-reference.md §2.16.

#### 7.14.2 `apps/desktop/electron-builder.yml`
...
extraResources:
  # Non-release builds package the tsdown output at this path. release.yml › desktop puts the bridge
  # artefact it extracted from the release image in place of this source before electron-builder runs,
  # and asserts byte identity (7.14.3, 06 D06-14, 11 OPS-65).
...
win:
  ...
  # No win.azureSignOptions at 1.0. publisherName is recorded in the installed app's app-update.yml and is
  # verified against every future update, so it is chosen once by the post-1.0 signing epic (D07-45) and is
  # never set provisionally.
mac:
  target: [{ target: zip, arch: [x64, arm64] }]
...
linux:
  ...
  category: Office
publish: [{ provider: generic, url: "https://updates.invalid/desktop/updates/${channel}/", channel: stable }]
# placeholder, deliberately kept: it is what bakes app-update.yml into the package, which is the file the
# post-1.0 updater overrides at runtime with setFeedURL (D07-35). Nothing reads it at 1.0.
```

#### 7.14.3 The detail table
...
| Windows | `zip` only. No NSIS, no MSI, no MSIX, no `.blockmap`. There is no installer, no Start-menu entry, no uninstaller and no per-machine installation; the user extracts the archive and runs `Iridium.exe` from wherever they put it. `app.setAsDefaultProtocolClient('iridium')` runs at first launch and writes under `HKCU`, so `iridium://` deep links work per user and only after the application has been run once |
| macOS | `zip` only, **ad-hoc signed** (`identity: "-"`). No dmg, no Developer ID, no notarisation, no hardened runtime. The ad-hoc signature conveys no identity and no Gatekeeper trust; it exists because macOS on Apple Silicon refuses to execute an arm64 Mach-O with no signature at all. **Assumption:** that electron-builder 26.16.1 applies the ad-hoc identity when `mac.identity: "-"` is set and no `CSC_*` secret is present. `release.bundle-integrity` runs `codesign -dv --verbose=4` on the built `.app` and asserts a signature is present and ad-hoc, and the packaged smoke on the macOS runner proves the extracted bundle launches, so a false assumption fails the release rather than shipping an unrunnable arm64 bundle |
| Linux | `tar.gz` only. No AppImage, no deb, no rpm, no OS-level signing (Linux never had any). Two consequences are documented rather than left to the user: the archive carries `iridium.desktop` at its root, because without it there is no `x-scheme-handler/iridium` registration and `iridium://` deep links are inert on Linux at 1.0; and `chrome-sandbox` cannot arrive setuid-root through an extraction performed by an ordinary user, so a host that restricts unprivileged user namespaces needs one repair command. Both are in 7.14.5 and in `docs/ops/desktop-distribution.md`, and `release.packaged-smoke.e2e` executes them from that document rather than from a copy |
| Signing | **None at 1.0.** Not Windows Authenticode, not Apple Developer ID, not notarisation. The post-1.0 desktop distribution epic adds all of it (7.14.6). The absence is asserted, not merely omitted: `release.bundle-integrity` fails if `Get-AuthenticodeSignature` reports anything but `NotSigned` on Windows, so a stray signing secret in CI cannot silently change what is shipped |
| Fuses | Unchanged — all seven values of A53, flipped by electron-builder at package time. Two of them cannot be honoured by an unsigned build and stay set anyway, because unsetting them would be a second difference the epic has to undo: `enableCookieEncryption` needs the macOS Keychain (7.6), and `enableEmbeddedAsarIntegrityValidation` validates the ASAR against a hash embedded in a signed binary. On Windows and Linux the fuse set behaves exactly as designed |
| E2E fuse variant | A second electron-builder configuration identical except `enableNodeCliInspectArguments: true`, which Playwright's `_electron` launcher needs. It was called the "test-signed variant"; it is renamed the **E2E fuse variant** because nothing is signed at 1.0 and a name that asserts a signature would be false. The final artefacts are still smoke-tested unmodified in `release.yml`, so the fuse difference never ships |
| ASAR | `asar: true`; `asar.disableIntegrity` is never set. `desktop.fuses.guard` reads the packaged binary to prove the wire, exactly as before |
| Bundled bridge | Unchanged: `resources/bin/iridium-mcp.mjs` — the stdio bridge from `packages/mcp-bridge`, a self-contained single file with every runtime dependency inlined (02-system-architecture.md). Release builds copy the release artefact `release.yml` extracted from the server image, which `release.yml › desktop` puts in place of the 7.14.2 `extraResources` source before electron-builder runs and which is byte-identical to the server download `GET /desktop/tools/iridium-mcp-<version>.mjs` (06 D06-14, 11 OPS-65); non-release builds, the pull-request and local builds that the `e2e-electron` packaged smoke uses, copy `packages/mcp-bridge/dist/iridium-mcp.mjs`, which the same tsdown configuration builds. The `.mjs` extension travels with the desktop copy because the generated Claude Desktop snippet invokes it as `node <bridge-path>`, so the module kind must not depend on Node's syntax detection; `iridium-mcp` is the package's `bin` name (the npm/PATH alias) and never a filename. The absolute path is reported by `iridium:app:info.bridgePath` so the Integrations page can print a ready-to-paste stdio configuration (4.15). Only `release.yml` asserts byte identity, between the release copy and the artefact it extracted |
| Build manifest | After building on each runner, `tooling/release/write-bundle-manifest.ts` writes `release/bundles.json` — `[{platform, arch, name, sizeBytes, sha256, sha512}]` — and `release/SHA256SUMS`. This replaces `latest*.yml` as the publish-time input, because electron-builder does not produce channel files for a `zip`/`tar.gz`-only target set; `iridium desktop-updates publish <dir>` and `iridium desktop-updates verify <dir>` both read `bundles.json`. The epic adds the installer rows to the same manifest and changes nothing else about the publish path |
| Supply chain | Unchanged: SBOM and provenance are produced in `release.yml` (11-operations-and-deployment.md) for the bundles as they were for the installers; the desktop build consumes only workspace packages plus the pinned dependency set |
| electron-builder 27 | Still an explicit post-MVP migration (`migrate-schema`, `win.sign`, `mac.sign`, nested `asar`, `files[]`-only feeds, `quitAndInstall({})`), not attempted during MVP (A53), and now sequenced **after** the signing epic so the migration is done once against the final target set |

#### 7.14.4 What unsigned actually costs

At 1.0 the integrity of a desktop bundle rests on exactly three things: TLS between the client and the site's own server, the SHA-256 the site publishes beside the artefact, and the file permissions of `DESKTOP_UPDATES_DIR`. It does not rest on a code signature, and nothing in the product verifies one. The consequence is direct: anyone who can write into `DESKTOP_UPDATES_DIR`, or who can terminate TLS in front of the server, can serve a modified bundle together with a `SHA256SUMS` that matches it, and no check anywhere in Iridium would notice. The published checksum protects against a truncated download, a corrupted mirror and an artefact that is not the one the administrator approved; it does not protect against a compromised server, because the checksum comes from the same server as the file. What a security-conscious site does about that in the meantime is 11-operations-and-deployment.md OPS-60.

**Windows.** A zip downloaded with a browser carries the Mark of the Web, and Windows File Explorer copies that mark onto every extracted file. Launching `Iridium.exe` then shows Microsoft Defender SmartScreen — "Windows protected your PC — Microsoft Defender SmartScreen prevented an unrecognised app from starting" — and the user must choose **More info → Run anyway**. The documented first step avoids it: right-click the downloaded `.zip` → **Properties** → tick **Unblock** → **OK**, *then* extract; the extracted files inherit no mark and the prompt does not appear. There is no way to remove the prompt for an unsigned executable, and an unsigned executable can never accrue SmartScreen reputation, so it does not fade with use. Some endpoint-protection products quarantine unsigned executables outright. A site running WDAC or AppLocker must add a rule for the bundle, and without a publisher certificate the only rule kinds available are path and hash — which is why `iridium desktop-updates verify --print-inner` prints the digest of the executable inside each archive (11-operations-and-deployment.md, the CLI inventory).

**macOS.** A zip downloaded with a browser is quarantined (`com.apple.quarantine`). On first launch Gatekeeper refuses to open the application because the developer cannot be verified. On macOS 13 and 14 the user can Control-click the application and choose **Open**, then confirm. From macOS 15 that gesture no longer works: the user must open **System Settings → Privacy & Security**, find the blocked-application notice for Iridium, choose **Open Anyway**, and confirm. `docs/ops/desktop-distribution.md` gives the System Settings path first and names the Control-click gesture only as the older-OS variant, because the folklore version leaves macOS 15 users stuck. A managed fleet removes the attribute before distributing — `xattr -d com.apple.quarantine /Applications/Iridium.app` — and the document names that for what it is: the deliberate removal of a protection. The second macOS consequence is the credential one in 7.6: every macOS installation runs in memory-only mode and the user signs in again after every quit.

**Linux.** Linux artefacts were never OS-signed, so nothing changes in kind; what changes is the shape. A `tar.gz` is not a package, so there is no `.desktop` entry, no menu item and — the consequence that matters — no `x-scheme-handler/iridium` registration, which is what the deb and rpm targets provided. `iridium://` deep links are therefore inert on Linux at 1.0 until the bundle's own `iridium.desktop` is installed: `desktop-file-install --dir="$HOME/.local/share/applications" iridium.desktop && update-desktop-database "$HOME/.local/share/applications"`. Separately, Chromium's sandbox needs either unprivileged user namespaces or a setuid-root `chrome-sandbox`, and no archive format can carry a setuid bit through an extraction performed by an ordinary user; on a host that restricts unprivileged user namespaces the extracted bundle is repaired once with `sudo chown root:root ./chrome-sandbox && sudo chmod 4755 ./chrome-sandbox`. `--no-sandbox` is never the documented answer and appears in no runbook.

#### 7.14.5 The Linux bundle's `iridium.desktop`

The Linux archive carries `iridium.desktop` at its root, built from `apps/desktop/build/linux/iridium.desktop` and copied in by the `extraResources` entry of 7.14.2. It declares `Exec=%k/iridium %u` and `MimeType=x-scheme-handler/iridium;`, so installing it with `desktop-file-install` is what makes `iridium://` links reach the shell (7.12) on Linux at 1.0. The post-1.0 epic's deb and rpm targets install the same file through the package manager, at which point the manual step disappears; the file itself does not change.

#### 7.14.6 The post-1.0 desktop distribution epic

| What the epic adds | Detail |
|---|---|
| Windows identity | **Azure Trusted Signing**, which requires organisation validation. Owned by the organisation as a legal entity, never by an individual. Configured as `win.azureSignOptions {publisherName, endpoint, codeSigningAccountName, certificateProfileName}` |
| macOS identity | **Apple Developer Program membership (Organization)** with a **Developer ID Application** certificate and notarisation credentials. CI secrets, in the spellings the digest verifies: `CSC_LINK`, `CSC_KEY_PASSWORD`, `APPLE_ID`, `APPLE_APP_SPECIFIC_PASSWORD`, `APPLE_TEAM_ID`. The private key lives in the release secret store, never on a laptop |
| Installer formats | Windows NSIS assisted (`oneClick: false`, `perMachine: true`, `allowElevation: true`, `differentialPackage: true`) plus MSI for SCCM/Intune; macOS `dmg` plus the `zip` Squirrel.Mac consumes; Linux AppImage (auto-updating) plus deb and rpm, which is what restores the `.desktop` and MIME registration and therefore Linux deep links. **MSIX is not in the epic**: it needs a separate Store identity and a packaging model that fights both the `iridium://` handler and per-machine installation, and G8's answer did not ask for it. It stays a named non-goal |
| In-application updates | `electron-updater` 6.8.9 re-added; `updater.ts` restored to `setFeedURL` from `GET /desktop/update-policy` with the origin check (D07-35, unchanged and already designed), `autoDownload: false`, the three policies, `verifyUpdateCodeSignature: true` with the fixed `publisherName`, the `.blockmap` differential path, and `quitAndInstall` behind the unsaved-work guard. `UpdateState` regains `downloading`, `downloaded` and `installing`; `manual-download` stays in the union permanently, because a site on `policy: 'disabled'` and a deb/rpm installation still need it |
| Restored settings | `mac.hardenedRuntime: true`, `mac.notarize: true`, `mac.identity` to the Developer ID; macOS credential custody returns to `safeStorage`, which is what makes `requireSecureStorage` usable on macOS |
| **The irreversible part** | `publisherName` is recorded in the installed application's `app-update.yml`, and `electron-updater` compares every future update's Authenticode subject against it. Once one externally distributed **installed** client exists, changing the certificate subject breaks updates for it, and the only recovery is an intermediate release that accepts both subjects — which itself has to be signed with the old subject and distributed before the change. The epic therefore fixes the legal-entity string **before** its first external build, records it in `docs/adr/0053-electron-shell.md` and in 13-decision-log.md as a single-valued immutable configuration item, and `desktop.publisher-name.guard` asserts that the value in `electron-builder.yml` equals the one committed in `apps/desktop/build/publisher.json` so it cannot drift in a pull request (D07-45). One thing shipping unsigned at 1.0 makes cheaper: there is no installed base carrying a `publisherName` yet, so the epic gets one clean chance to choose |
| What the epic does **not** change | No route, no table, no IPC channel name, no `desktop_releases` shape, no `GET /desktop/update-policy` shape, no `@iridium/ui` component. It touches `apps/desktop/electron-builder.yml`, `apps/desktop/src/main/updater.ts`, `apps/desktop/package.json`, `.github/workflows/release.yml`, `tooling/release/write-bundle-manifest.ts` and four documents. That is the whole point of keeping the seams |
| Entry condition | Both identities provisioned. This is an external procurement dependency the project does not control, and 14-risks-and-open-questions.md ASM-07 states it as the epic's assumption rather than as a 1.0 one |

### 7.15 How a user learns about a new version (1.0)

| Aspect | Decision |
|---|---|
| Mechanism | **There is no in-application updater at 1.0.** `electron-updater` is not a dependency (02-system-architecture.md, package dependencies), `updater.ts` imports nothing from it, and no code path downloads or installs an executable. What remains is a version comparison: `main` fetches `GET <active profile origin>/api/v1/desktop/update-policy` at startup *before* sign-in and every 6 hours — unchanged timing, unchanged route, unchanged `Cache-Control: public, max-age=300` — and compares `latest.version` with `app.getVersion()` using the same semantic-version comparison `CompatibilityGate` uses for `minClientVersion` (8.2), so two places in the client cannot disagree about which version is newer |
| Policy response | `GET /desktop/update-policy` returns `{ policy: 'disabled' \| 'prompt' \| 'silent', channel: 'stable' \| 'beta', minVersion, latest: { version, publishedAt, notesUrl, artifacts } \| null, feedUrl, requireSecureStorage }` (09 §2.16). `feedUrl` is still returned and still validated against the active profile's origin before use; at 1.0 it is used to build download URLs rather than to point an updater, and the origin check is unchanged so a compromised or misconfigured policy response still cannot send a fleet to a foreign host |
| Resulting state | `policy: 'disabled'` → `{status:'disabled', reason:'policy'}`, and the feed is not contacted. `--allow-insecure-server` → `{status:'disabled', reason:'insecure-server'}`. An unpackaged build (and `IRIDIUM_E2E=1`) → `{status:'disabled', reason:'unpackaged'}`. `latest` null, or `latest.version` not newer → `{status:'idle', currentVersion, checkedAt}`. Newer → `{status:'manual-download', version, notes, mandatory, artifacts}`. `mandatory` keeps its existing meaning exactly: the offered version is at or above `minVersion` while the running version is below it |
| `silent` cannot be honoured | With no updater, `silent` cannot silently do anything. The shell presents `silent` identically to `prompt` and the card says so in one line, so an administrator who set `silent` is not misled into believing a fleet is updating itself. The setting is not rejected and not rewritten — the server-side value is what the epic will honour |
| UI | `UpdateState` is exposed through `host.updates` and rendered as a card in Settings › About (4.16). In `manual-download` the card shows: the running version and the offered version; the release notes from the policy response, rendered through the sanitizing preview pipeline and never as raw HTML (unchanged); the artefact for this platform and architecture by **file name, size and SHA-256**; a **Download** link, a **Copy download link** control and a **Copy checksum** control; and the verification command for this operating system as selectable text (`certutil -hashfile <file> SHA256`, `shasum -a 256 <file>`, `sha256sum -c SHA256SUMS --ignore-missing`). There is no Install control at 1.0, in any state. The card links to `<PUBLIC_ORIGIN>/app/download` for the other platforms |
| Downloading | The Download link is an ordinary external link: `shell.openExternal(<https URL>)`, already restricted to `https:` and `mailto:` (7.4). The shell never fetches, writes or executes the bundle. That is deliberate — a shell that downloaded and launched an unsigned executable would be a worse attack surface than the prompt it was avoiding |
| Blocking case | When the server raises `minClientVersion`, `CompatibilityGate` reaches `client-too-old` on the next `426 client_outdated` and shows its blocking screen (8.2). At 1.0 the desktop branch of that screen shows the download link, the file name and the SHA-256 for this platform and architecture, plus the copy controls, which it can still obtain because `GET /desktop/update-policy` is exempt from the version gate (8.2); under `policy: 'disabled'` it shows the contact-an-administrator text unchanged; "Export my text" stays reachable, unchanged |
| `install()` | `iridium:updates:install` stays declared in `@iridium/contracts/desktop-ipc.ts` and stays registered, so `contracts.desktop-ipc.unit` and `desktop.ipc-contract.e2e` keep passing and the epic re-enables a handler rather than re-adding a channel. At 1.0 it always rejects with `ProblemDetails` `{ status: 409, code: 'updates_manual_only', title: 'This build installs updates manually' }`. No UI calls it |
| Tests | `desktop.update-manual.e2e` (7.16) and `desktop.update-policy.spec` |

D07-35 is unchanged and dormant. The packaged `app-update.yml` still carries the placeholder feed URL from the `publish` block, the runtime `setFeedURL` + origin-check design still stands, and `publisherName` and `verifyUpdateCodeSignature` are still the settings that must never be overridable from a policy response. None of it executes at 1.0, and none of it has to be re-decided when the epic switches it on.

### 7.16 Desktop test matrix

| Test | Kind | Runs on |
|---|---|---|
| `desktop.web-preferences.guard` | guard: file snapshot of the options object | ubuntu, windows, macos |
| `desktop.fuses.guard` | packaged-binary fuse wire | the release matrix |
| `desktop.preload-surface.guard` | snapshot of `window.iridium` keys + grep for channel pass-through | ubuntu |
| `ipc.origin.guard` | rejects a handler call from a foreign origin and from a detached frame | ubuntu |
| `contracts.desktop-ipc.unit`, `desktop.ipc-contract.e2e` | every channel validates payloads; registered channels equal the contract set | ubuntu |
...
| `desktop.hardening.e2e` | navigation and window-open denial, CSP present, no Node in the renderer, and an in-page `fetch('iridium-attachment://…')` whose response carries `Content-Security-Policy: sandbox`, `X-Content-Type-Options: nosniff`, `Referrer-Policy: no-referrer` and `X-Permitted-Cross-Domain-Policies: none`, carries **no** `Cross-Origin-Resource-Policy`, and whose `<img>` in the rendered preview actually decodes (the header name set is read from `FORWARDED_ATTACHMENT_HEADERS`, so a new hardening header without a forward fails the suite) | ubuntu (xvfb), windows, macos |
| `desktop.sign-in.e2e`, `desktop.open-note.e2e`, `desktop.three-instances.e2e` | real flows against a real server (`@iridium/testkit` child-process mode); `desktop.sign-in.e2e` additionally asserts that main's pre-login `POST /auth/sessions` and `POST /auth/set-password` carry `X-Iridium-Client: desktop`, no `Cookie` and no `Origin`, and are accepted by the CSRF guard (04-auth-and-access-control.md §4.4) | ubuntu, windows, macos |
| `desktop.hostile-markdown.e2e` | the shared corpus renders inert in the packaged shell | ubuntu, windows, macos |
| `desktop.a11y-keyboard-only.e2e` | the keyboard-only walk of 9.1 executed in the shell — create, rename, move, trash and restore a note with no pointer, including a keyboard drag — plus the assertion that every native-menu item resolves to the same `CommandId` the palette resolves to | ubuntu (xvfb), windows, macos |
...
| `desktop.attachments-no-token-in-renderer.e2e` | the renderer's network log shows no `Authorization` header and no token string anywhere in `window`; a `Range: bytes=0-1023` fetch through the scheme returns `206` with `Content-Range` | ubuntu |
| `desktop.deep-link-fuzz.e2e` | 7.12 | ubuntu, windows, macos |
| `desktop.update-manual.e2e` | against a stubbed `/api/v1/desktop/update-policy`: the About card in `manual-download` shows the offered version, the file name, the size and the SHA-256 for the runner's platform and architecture; the copy controls place the URL and the digest on the clipboard; **no** Install control exists in any state; `iridium:updates:install` invoked directly over IPC rejects with `updates_manual_only`; `policy:'disabled'` renders no card and contacts no feed; `policy:'silent'` renders exactly as `prompt` and says so | ubuntu, windows, macos |
| `desktop.update-policy.spec` | the four `disabled` reasons, the `idle` / `manual-download` boundary including equal and newer running versions, the `mandatory` derivation against `minVersion`, and the `feedUrl`-origin refusal | ubuntu |
| `desktop.macos-secure-storage.e2e` | on the ad-hoc-signed packaged build: `iridium:app:info.secureStorage` is `'unavailable'`, the login screen shows the memory-only warning, `secrets.bin` is never created, a relaunch returns to the sign-in screen, and with `requireSecureStorage: true` in the policy the shell shows the administrator-refusal screen instead of a login form | macos |
| `desktop.artifact-names.guard` | parses `apps/desktop/electron-builder.yml`: the target set is exactly `{win: [zip], mac: [zip], linux: [tar.gz]}` over `[x64, arm64]`; none of `nsis`, `msi`, `msix`, `dmg`, `appimage`, `deb`, `rpm`, `snap` is configured; `win.azureSignOptions` is absent, `mac.notarize` is `false` and `mac.identity` is `"-"`; and the three `artifactName` templates render to the six expected names for a sample version | ubuntu (`static`) |
| `desktop.host-contract.e2e` | the parameterised host suite (2.4), including the `IpcWebSocket` mode | ubuntu, windows, macos |
| packaged smoke (`release.packaged-smoke.e2e`) | extracts the produced archive with the commands in `docs/ops/desktop-distribution.md` — including, on Linux, installing `iridium.desktop` and repairing `chrome-sandbox` — then launches it, signs in against a disposable server, opens a note and quits, and runs the packaged `resources/bin/iridium-mcp.mjs`. The documented commands are the tested commands, so a runbook step that stops working fails the release | the release matrix |

Playwright's electron project runs with `IRIDIUM_E2E=1` (no single-instance lock, no update check) against a server started by `@iridium/testkit`; macOS runners provision MySQL with `shogo82148/actions-setup-mysql@v1` at `mysql-version: '8.4'` (A51). This lane pins **one** line deliberately, and it pins the floor: 8.4 LTS and 9.7 LTS are equal required targets (03-data-model.md §1.1, D03-23), but the desktop suite exercises the client against a server rather than the engine, so running it on both lines would double the slowest matrix in the build to prove nothing the database lanes do not already prove. Two-line coverage lives where it belongs — `migrations.integration`, `migrations.parity.integration`, `db-grants.integration` and the restore drills, each merge-blocking on both entries of `ci.yml › integration` (10-testing-and-quality.md) — and pinning the floor here means any server behaviour the desktop suite depends on is proven on the line with the smaller feature set.

The specs in this matrix that discharge a spec §9 acceptance row carry the Playwright tag `@smoke`, which is the selector the merge-blocking `e2e-electron` job uses; the rest run nightly in `e2e-electron-full` (10-testing-and-quality.md).
...
{ "apiVersion": 1, "minClientVersion": "1.0.0", "serverVersion": "1.2.3",
  "features": ["mcp", "oauth", "desktop-updates", "attachments", "import", "export", "search"],
  ...
  "policies": { "passwordMinLength": 15, "passwordMaxLength": 128, "patMaxLifetimeDays": 366,
                ...
                "patAllowAllVaultsForNonAdmins": true } }
```

The field spellings are 09-api-reference.md §2.2's and no other: that schema is the single definition of the body (01 §7, 09 D09-19), the OpenAPI document is generated from it, and the `static` job's `pnpm gen:check` step fails CI on a divergence. `limits` is the client-visible subset of `@iridium/contracts/limits.ts` (A.1, 02 ARCH-16) — the wire names deliberately differ from the constant names, so a form reads `limits.noteSoftChars` while the editor's guard reads `NOTE_SOFT_MAX_UTF16` — and `policies` is the client-visible projection of `server_settings` (A26, A31, A29). Its four `pat*` members are the effective `patPolicy`, read per request from the settings store: the environment baseline merged with whatever an administrator has tightened through `/admin/settings`, so the dialog offers exactly what the server will accept. `passwordMinLength` stays the environment value `PASSWORD_MIN_LENGTH` until M7 adds the `passwordPolicy` group (AG12), and `passwordMaxLength` is the `Password` schema's own maximum (`PASSWORD_MAX_CODE_POINTS`, 09 §2.2). Both objects exist so forms can enforce the same rules the server enforces instead of guessing them or discovering them through a `validation_failed` response (decision D07-31): the login and set-password forms read `policies.passwordMinLength` / `passwordMaxLength`, `TokenCreateDialog` reads `policies.patMaxLifetimeDays`, `patAllowNoExpiry`, `patRotationOverlapMaxHours` and `patAllowAllVaultsForNonAdmins` (4.15), and the upload and paste paths pre-flight against `limits.uploadBytes` and `limits.noteSoftChars`. Neither object carries a secret or an administrative value, and both are additive, so publishing more of them later is a non-breaking change.

Every client request carries `X-Iridium-Client: web|desktop` and `X-Iridium-Client-Version: <product version>`; collaboration stateless messages carry `v: 1`; the desktop IPC contract has no skew at all, because the renderer and main ship inside one bundle.
...
| Product versioning | Changesets keeps one product version across the server image, the web bundle, the desktop bundles and the bridge (A1) | "server 1.2.3 with desktop 1.2.1" is a comparison of the same number line |
...
| `client-too-old` | `__IRIDIUM_VERSION__ < minClientVersion`, or `server.apiVersion > max(SUPPORTED_API_VERSIONS)` | a full-screen "Update required" screen naming both versions. Desktop at 1.0: the manual download affordance of 7.15 — the artefact's file name, size, SHA-256 and download link for this platform and architecture, with the copy controls and the verification command, under both `prompt` and `silent` (which is presented identically because no updater exists), and instructions to contact an administrator under `disabled`. No Install control appears, and "Export my text" stays reachable on the blocking screen exactly as before. Web: "Reload" (6.5), because reloading fetches the matching bundle |
...
Rules: the verdict is recomputed on every window focus after 10 minutes idle, on every reconnect of the vault channel, after a profile switch, and — without waiting for any of those — the moment a request answers `426 client_outdated`, whose `ProblemDetails.detail` is exactly the new `minClientVersion` string (4.18). The server accepts an absent version header for existing unversioned callers, but an explicit malformed SemVer header is `422 validation_failed`; the hosts always send their actual version. That last trigger is the one that matters in practice: an administrator upgrading the server raises `minClientVersion` under a long-running session, and every gated route starts answering 426 immediately for an explicitly versioned client below the committed floor, so a client that only re-probed on focus would show generic server-error toasts until the user happened to switch windows. The gate is scoped positively (09 §1.2, the A54 amendment): it evaluates the version header, both the 422 and the 426, only on `/api/v1/*` and on the `/collab` upgrade, and no other path evaluates it (not the MCP mounts, `/oauth/*`, `/.well-known/*`, `/desktop/*`, `/app/*`, the root, `/login`, `/set-password` or the operational probes). `GET /api/v1/meta` and `GET /api/v1/desktop/update-policy` are exempt from it, so a client below the floor can still read the floor and the artefact list its blocking screen shows (7.15, "Blocking case"). A `client-too-old` verdict reached while a note has unsaved text keeps "Export my text" reachable on the blocking screen — the gate never traps text. Feature strings gate affordances rather than requests: with `mcp` absent, Settings › Integrations renders "Integration tokens are disabled on this server" instead of letting a user create a token that cannot be used; with `oauth` absent, the same page hides the "Connect a connector" panel and the connector-audience snippet group while keeping the Authorized applications card, so a grant made before the surface was unmounted stays revocable (4.15); with `import` absent, the import entry points are hidden.

Tests: `compat.gate.spec` (component, all four verdicts with a stubbed `/meta`), `meta.apiversion.integration` (integration, asserting the shape and that `apiVersion` is an integer), and `compat-blocked` (Playwright, web and electron) which serves a `/meta` with a raised `minClientVersion` and asserts the blocking screen plus a working "Export my text", and additionally raises `minClientVersion` *mid-session* so the next mutation answers `426` — asserting that the blocking screen appears on that response alone, with no generic error toast and no retried mutation, and, in the electron project, that the blocking screen carries the artefact name and SHA-256 for the running platform and architecture, that the copy controls work, and that no Install control is rendered. These are cases of `compat-blocked` rather than a new spec, because the screen and its export affordance are already that file's subject.

## 9. Accessibility, internationalisation, client support and budgets
...
| Verification | axe-core (pin at M0) in component tests with **zero** serious or critical findings allowed; per-route axe scans in the web E2E project; `a11y.keyboard-only.e2e` in the web host and `desktop.a11y-keyboard-only.e2e` in the supported client; manual passes with NVDA on Windows and VoiceOver on macOS at M4 and again at M8, recorded in `docs/ops/accessibility.md`. The desktop walk is the one that carries the commitment at 1.0 (01 §4.6); axe itself stays in the component and web lanes because the rendered DOM is the same in both hosts and the only desktop-specific chrome — the native menu — is not reachable by an in-page scanner |

### 9.2 Internationalisation

| Decision | Detail |
|---|---|
| String table | every user-visible string lives in `packages/ui/src/i18n/en.ts` as a typed record (`as const`), so `t('workspace.status.saved')` is checked at compile time and an unused or missing key is a build error (A55). One group is sourced rather than written: the six Read-permission labels, `permission.<scope>`, are derived from `READ_PERMISSION_LABELS` of `@iridium/contracts` (`packages/contracts/src/permission-labels.ts`, 06 D06-40), imported and never copied, through a typed mapped-key helper that keeps literal key types, so `t('permission.note:read')` stays compile-checked and `guards.i18n.guard` still holds. The server-rendered consent page imports the same constant, so the token dialog and the consent screen cannot word a permission differently (D07-47) |
...
### 9.3 Supported clients, browsers and platforms

The commitment is stated once in 01-vision-scope-and-principles.md §4.6 and rendered here as the matrix an implementer works against.

| Target | Commitment at 1.0 |
|---|---|
| **Desktop application** (Electron 44.3.0, Chromium 152) on macOS 13+, Windows 10/11 (x64, arm64), Linux x64/arm64 | **Supported client at 1.0.** Merge-blocking `electron` E2E on ubuntu, windows and macos; every spec §9 row proven in both hosts is gated on its Electron proof. Distributed at 1.0 as unsigned `zip` (Windows, macOS) and `tar.gz` (Linux) bundles, x64 and arm64 (7.14) |
| Web host in Chrome and Edge (current stable and the two preceding releases) | Development and internal surface; not a supported product surface at 1.0. Merge-blocking `chromium` E2E (4 shards) and Vitest Browser Mode component project, because this is how the shared UI is developed and tested |
| Firefox and WebKit | Out of scope. No Playwright project, no nightly smoke run, no best-effort claim; `guards.non-goals.guard` (case *Cross-browser support*) fails the build if a lane appears |
| Mobile browsers | Out of scope (spec §10 defers mobile clients); the layout degrades gracefully but is not tested or supported |
| Build targets | web bundle `build.target: 'chrome150'`, desktop renderer `'chrome152'` (decision D07-18) — unchanged, because the web host still runs on current Chrome and Edge |
| Feature floor (web host) | ES2024, `structuredClone`, module workers, `BroadcastChannel`, `ResizeObserver`, `IntersectionObserver`, `Intl.PluralRules`/`ListFormat`/`RelativeTimeFormat`, `:focus-visible`, CSS nesting and container queries, `AbortController`. `requestIdleCallback` and request-body streams are used with fallbacks (5.9, 6.4) |
| Below the feature floor | Detection is by **feature test**, never by user-agent string (decision D07-18): a boot check renders a static, dependency-free page that names Chrome and Edge as the browsers the web host runs in and states that the desktop application is Iridium's supported client, and never attempts to run the SPA. Asserted by `ui.unsupported-browser.component` |

Nothing here narrows the code: the web host is built, served and hardened exactly as section 6 specifies, and the `IridiumHost` seam is identical in both hosts. What the table fixes is which claim the project makes and which lane proves it.
...
The renderer bundle budget blocks the pull request, because it is deterministic. The timing budgets are measured on every pull request and reported as warning annotations, and they block the nightly `perf` job, where the runner class is fixed (decision D07-19). A timing gate on shared PR runners would be a flakiness generator and would end up disabled, which is strictly worse than an honest nightly gate; A42's parser-switch criterion is tied to the nightly measurement for the same reason. The harness runs against the fixed demo vault fixture plus the generated 10 000-node tree from `@iridium/testkit`, on the CI runner class named in 10-testing-and-quality.md, so numbers are comparable across commits. Which budget is a 1.0 commitment follows the support commitment: the desktop cold-start and memory budgets are measured in the supported client and are release gates, while the workspace, note-open, preview, tree-scroll, typing and switcher budgets are measured in the web host because it is the automatable surface and the same bundle runs in both — they gate the nightly `perf` job as development signals. The renderer-bundle budget is shared by both hosts and blocks the pull request, as before.
...
| D07-16 | The desktop shell runs exactly one `BrowserWindow`; profile switching reloads it; additional windows are post-MVP. | One window keeps per-profile CSP generation, credential scope, deep-link routing and the connection budget unambiguous; the `desktop.three-instances.e2e` scenario uses three app instances instead. | 7.2 |
...
| D07-18 | Build targets are `chrome150` (web) and `chrome152` (desktop renderer); unsupported browsers are detected by feature test, never by user-agent, and are shown a static dependency-free page. | AG6 fixes the web host's browser floor at current Chrome and Edge and Electron 44 pins Chromium 152; feature tests do not rot as UA strings change and cannot mis-block a compliant browser. | 6.1, 9.3 |
...
| D07-31 | `GET /meta` additionally publishes `policies` (`passwordMinLength`, `passwordMaxLength`, `patMaxLifetimeDays`, `patAllowNoExpiry`, `patRotationOverlapMaxHours`) alongside the `limits` object (`uploadBytes`, `importBytes`, `importFiles`, `importDepth`, `noteSoftChars`, `noteHardChars`, `bodyBytes`, `wsMaxPayloadBytes`), in exactly the spellings of 09-api-reference.md §2.2, and both clients read them to validate forms locally. **Amended 2026-09-25:** `policies` gains `patAllowAllVaultsForNonAdmins` (06 D06-51), which `TokenCreateDialog` reads to hide its all-vaults switch. The four `pat*` members publish the effective `patPolicy`, read per request from the settings store (the environment baseline merged with an administrator's stricter stored value), while `passwordMinLength` stays the environment value `PASSWORD_MIN_LENGTH` until M7 adds the `passwordPolicy` group (AG12) and `passwordMaxLength` is the `Password` schema's own maximum (`PASSWORD_MAX_CODE_POINTS`). | Login, set-password, token-creation and upload forms must enforce exactly the server's rules; without a published projection they would hard-code values that drift from `server_settings` or rely on a rejected request to teach the user the rule. One zod schema in 09 is the only definition, so the generated client, the OpenAPI document and the forms cannot disagree; the wire names stay distinct from the `limits.ts` constant names (02 ARCH-16). Additive, so it is a non-breaking change under A54. The 2026-09-25 amendment keeps that promise once an administrator can tighten the PAT policy at M3: publishing the environment baseline instead of the effective value would offer choices the server then refuses, and hiding a switch whose use the server would answer with `422 all_vaults_disabled` is truthful where a refusal after submission is not. The new member is additive like the rest. | 4.2, 4.15, 8.1 |
...
| D07-34 | When one note is mounted in both panes, exactly one `EditorView` owns awareness — the focused one — through an `awarenessCompartment`; the other is built with `awareness: null`, which makes `iridiumRemoteSelections` publish nothing and render nothing. (After spike S4 the compartment holds that plugin rather than `yCollab`, which is always constructed with a null awareness; 5.4.) | Every instance that owns awareness writes the local selection into the same awareness local state, so two views over one session overwrite each other's `cursor` field and remote participants see the caret flip between panes. Focus is the only meaningful owner, and compartment reconfiguration avoids rebuilding a view on every focus change. | 5.2, 5.4 |
| D07-35 | The packaged `app-update.yml` carries a placeholder feed URL; `updater.ts` calls `autoUpdater.setFeedURL` with `feedUrl` from `GET /desktop/update-policy` before every check and refuses a `feedUrl` whose origin differs from the active profile's. `publisherName` and `verifyUpdateCodeSignature` are never overridden at runtime. **Dormant at 1.0 and kept as the post-1.0 seam:** nothing calls `setFeedURL` in the 1.0 build, because there is no `electron-updater` (7.15); the `publish` block, the baked `app-update.yml` and the origin-check design all ship and are switched on unchanged by the desktop distribution epic (7.14.6). | `electron-builder` bakes `publish.url` at build time, but a self-hosted product only learns its server origin at runtime from the active profile, so a baked URL points at a host that does not exist. The origin check keeps the runtime feed from becoming a redirection primitive, and leaving the signature settings baked keeps update verification out of the policy response's reach. Keeping the decision live and dormant rather than deleting it is what makes the epic a configuration change instead of a re-decision. | 7.14, 7.14.6, 7.15 |
...
| D07-43 | The 1.0 desktop client is distributed as six unsigned bundles — `zip` on Windows and macOS, `tar.gz` on Linux, x64 and arm64, named `Iridium-<version>-<win32\|darwin\|linux>-<arch>.<ext>` — downloaded from the site's own server with a published SHA-256 per artefact. macOS is ad-hoc signed only, which makes the arm64 binary executable while conveying no identity, so every macOS installation runs in memory-only credential mode. No installer, no code signature, no notarisation, no in-application updater; every seam for them is kept configured. | G8 (2026-09-12) defers signing, installers and in-app updates, and a plan that shipped them anyway would be shipping a blocker it does not control. Choosing the archive format per platform rather than uniformly is the correctness choice: a zip cannot carry the Unix executable bit, and macOS `zip` via `ditto` is both correct and the format the epic's Squirrel.Mac path already needs. Keeping the fuse set, `asar`, `protocols`, `extraResources` and the `publish` block means the epic edits configuration rather than re-deciding the shell. | 7.6, 7.14 |
| D07-44 | `UpdateState` gains `manual-download {version, notes, mandatory, artifacts}`; `host.updates.check()` derives it by comparing `GET /desktop/update-policy`'s `latest.version` with `app.getVersion()` using the same comparison `CompatibilityGate` uses; `iridium:updates:install` stays declared and registered and always rejects with `updates_manual_only`; `policy:'silent'` is presented identically to `prompt` and the card says so; downloads are handed to `shell.openExternal`. | The seam has to stay whole or the epic becomes a redesign, and a channel removed now would have to be re-added with a new contract test. Deriving "is there something newer" in one place stops the About card and the blocking screen disagreeing. Presenting `silent` honestly matters because an administrator who set it would otherwise believe a fleet was updating itself. Handing the download to the browser keeps the shell from fetching, writing or launching an unsigned executable — which would be a worse surface than the prompt it avoided. | 7.15, 8.2 |
| D07-45 | The post-1.0 signing epic fixes the Windows `publisherName` before its first external build. It is a single-valued, immutable configuration item committed in `apps/desktop/build/publisher.json`, asserted equal to `electron-builder.yml`'s value by `desktop.publisher-name.guard`, and recorded in `docs/adr/0053-electron-shell.md`. | `electron-updater` compares every future update's Authenticode subject with the `publisherName` recorded in the installed app's `app-update.yml`; changing it after installations exist breaks their updates and is recoverable only through an intermediate release signed with the old subject. Committing the value and guarding it makes the irreversibility visible in a pull request rather than discovered in an updater log. Shipping 1.0 unsigned is what gives the epic one clean choice: no installed client carries a publisher name yet. | 7.14.6 |
| D07-46 | The Electron shell is the supported client at 1.0 and the web host is a development and internal surface (01 §4.6). Section 9.3 renders the matrix; section 6 is unchanged in substance; the Electron specs that discharge a spec §9 row carry `@smoke` so they run on every pull request across three operating systems; `desktop.a11y-keyboard-only.e2e` carries the accessibility commitment in the supported client. | The owner's G6 answer (2026-09-12) changes what the project promises, not what it builds. Keeping section 6 intact and moving only the commitment, the lanes and the wording is what preserves the brief's one-codebase requirement while making the 1.0 claim true; proving accessibility and the acceptance rows in the host that is actually supported is the difference between a claim and evidence. | 2, 6, 7, 9.1, 9.3 |
| D07-47 | The OAuth consent screen at `/oauth/consent` is the single, bounded exception to "every user-facing screen is a `@iridium/ui` route": it is server-rendered (06-mcp-and-agent-access.md D06-31), contains no `<script>` element at all, and reuses `packages/ui`'s design tokens through the static stylesheet `/app/assets/tokens.css` rather than through the application bundle (superseded in part by the amendment below: the stylesheet clause). The exception is enumerated here so it cannot grow: no other server-rendered user-facing page exists. **Amended 2026-09-25:** the page reuses `packages/ui`'s design tokens through a stylesheet the oauth module inlines under the response's style nonce, `apps/server/src/oauth/consent.css`, whose custom properties `pnpm gen` writes from `packages/ui/src/theme/tokens.css` from M4, and it references nothing under `/app/`. Its permission lines are `READ_PERMISSION_LABELS` from `packages/contracts/src/permission-labels.ts` (06 D06-40), the labels `en.ts` sources for the token dialog (9.2). This is the D06-31 amendment's consent-page clause, stated from the client side. | The screen must render before any application bundle has loaded and under a nonce CSP that admits no application JavaScript, and the single-use `request_id` must never enter client state or a history entry the router manages — three properties an SPA route cannot have. Sharing only the token stylesheet keeps it from looking foreign while keeping it outside the workspace, and naming it as a closed exception is what stops "the server renders this one too" becoming a pattern. The amendment keeps that property without the dependency on the web bundle: a stylesheet under `/app/` exists only where the server serves the web bundle, which an API-only deployment (`IRIDIUM_WEB_DIR` unset) does not, and `packages/ui`'s token file does not exist before M4, while the page is live from M3. So the page carries its own sheet and takes only data across the boundary — the generated token block and the permission labels, which also keeps its permission wording identical to the token dialog's. | 2, 4.15, 9.2 |
| D07-48 | The Integrations page renders two client enums rather than one: `SnippetClientSchema` for clients that send an integration-token header (`<origin>/mcp`) and `ConnectorClientSchema` for clients that sign in with OAuth (`<origin>/mcp/connect`). The old `claude-ai-connector` tab is renamed `claude-ai-connector-header` and stays in the token group with the request-header beta caveat; the new `claude-ai-connector` tab in the connector group is the OAuth one, and carries no secret and no `{{IRIDIUM_MCP_TOKEN}}` placeholder. | One enum cannot describe two audiences that differ in endpoint, credential and discovery posture, and a single tab named for a product would have silently changed meaning when OAuth arrived — the worst outcome for a user following a copied snippet. Two enums make the audience a type rather than a convention, keep the connector tabs structurally incapable of carrying a secret, and let the page state the one rule that prevents the failure mode 06 designs the two mounts to avoid: do not mix the URLs. | 4.15 |
