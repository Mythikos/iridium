...
OAuth credentials have their own policy group, `server_settings.oauth_policy`, admin-editable in the **OAuth** card of `/admin/settings` and validated by `settings.contract`:
...
`oauth.discovery-split.contract` holds the whole posture in one test and **replaces** `mcp.no-prm.contract`, whose spelling moves to the "Superseded spellings" table of `10-testing-and-quality.md`.
...
| `action` | `mcp.<tool_name>`, `mcp.resources.read`, `mcp.resources.list`, `mcp.completion`, `mcp.subscriptions.listen` (written when the stream closes) | `rest.<route-id>`, e.g. `rest.notes.markdown`, plus the four authorization-server actions `oauth.authorize`, `oauth.consent`, `oauth.token.issue` and `oauth.token.refresh`, which are `surface='oauth'` rows with a `NULL` `vault_id` and an empty `note_ids` |
...
| nightly real-client matrix | **two halves and a coexistence row**, all against one server with the OAuth endpoint live: (1) static header against `/mcp` — pinned Claude Code (≥ 2.1.232, v2 runtime), VS Code, Cursor, Windsurf, the bridge, `mcp-remote --header`, plus the assertion that Claude Code made no `/.well-known/` request and its tool list contains neither `authenticate` nor `complete_authentication`; (2) OAuth against `/mcp/connect` — a scripted CIMD client, a scripted DCR client, `claude mcp login`, `mcp-remote --protocol auto`; (3) both audiences configured simultaneously, asserting neither degrades the other. `proxied-stack.mcp-headers` runs against both mounts |
