...
Two reference conventions are used throughout. Identifiers of the form `A19` name a plan-wide settled decision and `F9` a deliberate deviation from the feature spec; both are catalogued in 13-decision-log.md. `§G-4` names an open question carried in 14-risks-and-open-questions.md. Identifiers of the form `ARCH-05` are decisions taken in this section and listed at its end.

## Architectural summary

Iridium is a single-server, multi-client system:

- **One server process** (`@iridium/server`, Node 24.21.0, Fastify 5.12.4) owns every I/O path: the REST API at `/api/v1`, the collaboration WebSocket at `/collab` (Hocuspocus 4.7.0 embedded as the `Hocuspocus` class), the MCP endpoint at `/mcp` (`@modelcontextprotocol/server` 2.0.0 through `@modelcontextprotocol/fastify` 2.0.0), the static web bundle at `/app/*`, the desktop update feed at `/desktop/updates/*`, the operational endpoints `/healthz`, `/readyz`, `/metrics`, the in-process job scheduler, and the `iridium` operator CLI (same binary, different sub-command).
- **MySQL 9.7 LTS** (`mysql:9.7.2-oraclelinux9`; 8.4.11 certified) is the only system of record: accounts, memberships, hierarchy, metadata, persisted Yjs state, the per-update durability log, revisions, audit and access logs, jobs, settings, and the rebuildable projections (Markdown text, search index, link index).
...
- **Two client hosts, one UI**: `@iridium/web` (a Vite 8 entry served by the server) and `@iridium/desktop` (Electron 44.3.0: hardened main process, sandboxed preload, renderer loading `app://iridium/`) both mount the same `@iridium/ui` React application through the single `IridiumHost` seam.
- **Agents** reach the server over Streamable HTTP with a personal access token, either directly (Claude Code, Cursor, VS Code, Windsurf, custom clients) or through the first-party stdio bridge `iridium-mcp` (`@iridium/mcp-bridge`) for stdio-only hosts such as Claude Desktop.
...
flowchart LR
  subgraph Clients
    WEB["Web SPA<br/>@iridium/ui in a browser<br/>(BrowserHost)"]
    DESK["Electron desktop<br/>main + sandboxed preload + renderer<br/>same @iridium/ui (ElectronHost)"]
  ...
  subgraph Server["apps/server: one Node 24.21.0 process, Fastify 5.12.4, port 4000 on 127.0.0.1"]
    ...
    MCP["/mcp Streamable HTTP<br/>MCP SDK v2, stateless, JSON mode"]
  ...
  DB[("MySQL 9.7.2 LTS<br/>system of record")]
  ...
  AGENT -- "HTTPS Bearer irid_pat_" --> PROXY
  AGENT -- "stdio" --> BRIDGE
  BRIDGE -- "HTTPS Bearer irid_pat_" --> PROXY
  PROXY --> REST
  PROXY --> WS
  PROXY --> MCP
  PROXY --> STATIC
...
Clients never touch MySQL or the attachment store. The three entry points share one `authorize()`, one `AuditWriter`, one configuration object, one HTTP server and one port.
...
|---|---|---|---|
| Server | `ghcr.io/<org>/iridium-server:<version>` (multi-stage image from `infra/docker/server.Dockerfile`; `dist/main.mjs` built by tsdown 0.23.0 with workspace packages inlined) | Node 24.21.0, non-root, read-only rootfs, `cap_drop: [ALL]` | REST, `/collab`, `/mcp`, static web bundle, update feed, jobs, CLI (`iridium <command>`) |
| Database | `mysql:9.7.2-oraclelinux9` with `infra/docker/mysql/my.cnf` and `init/01_roles.sql` | MySQL 9.7 LTS (8.4.11 nightly lane) | System of record; three roles `iridium_app`, `iridium_migrator`, `iridium_backup` |
...
| Edge | `caddy` (pinned tag) from `infra/caddy/Caddyfile`, or nginx from `infra/nginx/iridium.conf` | — | TLS termination, HSTS, WebSocket passthrough, long idle timeouts on `/collab`, unbuffered `/mcp`, forwarding of `Mcp-Method`, `Mcp-Name`, `MCP-Protocol-Version` |
| Web client | Static bundle built by Vite 8.3.0 from `apps/web`, copied into the server image and served at `/app/*` with a per-response CSP nonce | Chromium-class browsers at MVP (skeleton A55) | The shared UI with `BrowserHost` |
| Desktop client | Installers from electron-builder 26.16.1 (nsis + msi, dmg + zip, AppImage + deb/rpm) | Electron 44.3.0 | The shared UI with `ElectronHost`; main-process credential custody; bundled `iridium-mcp` |
...
flowchart TB
  subgraph Fastify["Fastify 5.12.4 instance (buildApp)"]
    ...
    MCPP["mcp plugin (/mcp)<br/>PAT verifier, per-request McpServer factory,<br/>six read-only tools, resources, cursors, per-token limits"]
...
`apps/server/src/main.ts` is the CLI entry (`serve | migrate | doctor | config | backup | restore | audit | reindex | admin | tokens | sessions | keys | jobs | trash | desktop-updates | mirror`). `serve` calls `buildApp({ mode: 'container' })` from `apps/server/src/app.ts`; the integration harness calls `buildApp({ mode: 'in-process' })`; the chaos harness spawns the same binary and calls `buildApp({ mode: 'child' })`. The three modes share one plugin tree and differ only in listening, signal handling and whether the scheduler starts:
...
| `child` | ephemeral port reported on stdout as `{"listening":<port>}` | SIGTERM drain; SIGKILL by the test | on unless `JOBS_ENABLED=false` | `IRIDIUM_FAULT` when `NODE_ENV=test` | `@iridium/testkit startServer({mode:'child'})`, chaos and E2E lanes |
...
4. **auth** — decorates `request.principal`; registers `authenticate()` as an `onRequest` hook; wires credentials (argon2id via `@node-rs/argon2` 2.2.1), sessions, PAT verifier, `TicketStore`, set-password links, login throttling (`rate-limiter-flexible` 11.2.0 `RateLimiterMySQL` on `login_throttle`).
5. **authz** — permission matrix from `@iridium/contracts`, `authorize()`, the `AuthzBus` in-process implementation, the epoch table, and the **route-policy boot assertion**: after every route is registered (`onReady`), every route must declare `config.auth`, every mutating route with a cookie-capable principal must be CSRF-guarded unless `bearerOnly`, and every `/mcp` route must be `bearerOnly` with `principalKinds:['token']`; a violation aborts boot.
...
7. **rest** — registers the `/api/v1` plugin tree, `@fastify/swagger` 9.8.1 (OpenAPI 3.1 via `fastify-type-provider-zod` 7.0.0), `@fastify/swagger-ui` 6.1.1 at `/docs`, `GET /openapi.json` rendering `app.swagger()`, and `@fastify/static` for `/app/*`, `/desktop/updates/*`, `/desktop/tools/*`. The two documentation routes are registered through Iridium's own wrapper rather than left to the plugin defaults, because the route-policy boot assertion of step 5 requires *every* registered route to carry `config.auth`: both declare `config.auth = {serverAdmin: true}` (relaxed to any principal when `NODE_ENV=development`), the operationIds `meta.openapi` and `meta.docs`, `Cache-Control: no-store`, and Swagger UI with "try it out" disabled outside development (09-api-reference.md §2.17, §2.18).
8. **collab** — constructs the `Hocuspocus` instance with the four Iridium extensions, mounts `app.get('/collab', { websocket: true, preValidation: [originAllowlist, connectionCaps] })`, starts the `CollabGateway`, performs the boot-time sweep that closes any loaded document whose note is trashed (skeleton A46 lock-order note).
9. **mcp** — verifies the SDK handler is constructible, registers `app.all('/mcp', …)` with the route-level `onRequest` array `hostHeaderValidation(PUBLIC_HOST)`, `rejectBrowserOrigin`, `ignoreCookies`, `mcpIpGate`, the `preHandler` chain `patAuth`, `mcpKillSwitch`, `chargeRateLimit`, and the fail-closed `reply.hijack()` handler. Fastify concatenates instance-level hooks ahead of route-level ones, so `authenticate()` (step 4) and the `@fastify/cookie` parser (step 3) both run *before* this route's own `onRequest` array: what keeps a browser session from ever resolving to a principal here is the `bearerOnly` branch of `authenticate()` (04-auth-and-access-control.md §6.1), while `ignoreCookies` is the defence-in-depth layer that clears `req.headers.cookie` *and* the already-parsed `req.cookies` jar for every later phase (06-mcp-and-agent-access.md, "Mounting `/mcp` on Fastify"). Neither may be dropped as redundant.
10. **ops** — `/healthz`, `/readyz`, `/metrics` (prom-client, pinned at M0), the fault registry when `NODE_ENV=test`, and the shutdown drain hook.
11. **jobs** — the scheduler (container and child modes) claims due rows in the `jobs` table with `locked_by = <instanceId>`.

After step 11 the process calls `app.ready()` (which runs the route-policy assertion), then `listen`. `iridium_build_info{version,node,commit}` and `iridium_boot_timestamp` are set at that moment.

#### Not-ready gating

Between `listen` and full readiness the process is alive but must not serve business traffic. `ops/readiness.ts` owns one `ReadinessState` with three values: `starting` (until step 11 completes), `not_ready` (migrations pending, or a fail-closed `/readyz` check failing after boot, or draining) and `ready`. While the state is not `ready`, an `onRequest` hook registered by the ops plugin answers every route outside `/healthz`, `/readyz` and `/metrics` with 503 `ProblemDetails {code:'not_ready', detail}` and `Retry-After: 5`; `/collab` upgrades are refused with the same status before the WebSocket handshake. The migration re-check runs every 5 s so an operator applying `iridium migrate` from a sidecar brings the server up without a restart (the documented HA procedure in 11-operations-and-deployment.md). The same state machine drives the drain in the shutdown sequence below, which is why there is exactly one place that decides whether the process accepts work.
...
| `/mcp` | HTTPS Streamable HTTP, MCP 2026-07-28 stateless and 2025-era legacy stateless, JSON response mode, POST only | `Authorization: Bearer irid_pat_…` only; a session cookie never resolves to a principal here (`bearerOnly`), and `ignoreCookies` strips the header and the parsed jar for every phase after route-level `onRequest` | Host validation, any browser `Origin` → 403, `bodyLimit` 1 MiB, per-token and per-process rate limits, `patAuth` and `mcpKillSwitch` preHandlers, fail-closed on missing `AuthInfo` | `mcp` plugin |
| `/app/*` | HTTPS static | none | CSP nonce per response, `Cache-Control` by asset hash | `rest` plugin (`@fastify/static`) |
| `/desktop/updates/*`, `/desktop/tools/*`, `/desktop/update-policy` | HTTPS static / JSON | none (artifacts are signed; electron-updater verifies) | — | `rest` plugin |
...
sequenceDiagram
  ...
  K->>P: afterLoadDocument: attach NoteWriter, register document.on('update') listener, lastPersisted = {head_seq, last sv_after}
  ...
  K-->>N: stateless {t:'persisted', seq, sv} from lastPersisted
  N->>N: SaveStateMachine: connected and synced and unsyncedChanges == 0 and dominates(persistedSv, localSv) -> saved
...
sequenceDiagram
  ...
  K->>W: update listener: svAfter = Y.encodeStateVector(doc) captured synchronously, enqueue {update, svAfter, actor {userId, sessionId}, origin}
  ...
  W->>K: document.broadcastStateless({t:'persisted', seq: head+N, sv: base64(svAfter_last)}) and lastPersisted = {seq, sv}
  K-->>N: stateless persisted
  N->>N: dominates(persistedSv, Y.encodeStateVector(ydoc)) over every (clientId, clock) -> saved
...
```mermaid
sequenceDiagram
  participant AG as Agent (Claude Code | iridium-mcp bridge)
  participant F as Fastify ALL /mcp
  participant V as mcp/verifier.ts (PAT)
  participant M as McpServer (per request factory)
  participant CR as ContentReadCore
  participant D as MySQL (dbApp)
  participant AL as AccessLogWriter
  AG->>F: POST /mcp Authorization: Bearer irid_pat_… MCP-Protocol-Version, Mcp-Method: tools/call
  ...
  F->>V: preHandler patAuth: parse irid_pat_<id16>_<secret43><crc6>, CRC check
  V->>D: access_tokens by token_id -> timingSafeEqual(SHA-256(secret)) -> revoked_at, expires_at, rotation_overlap_until -> owner users.status
  V->>V: per-token bucket 120/min burst + rate_limit_per_hour (search costs 3)
  V-->>F: request.mcpAuthInfo = AuthInfo {token, clientId:'pat:'+id, scopes, expiresAt, resource: PUBLIC_ORIGIN/mcp, extras:{principal}} or 401 WWW-Authenticate Bearer realm="iridium", error="invalid_token" (no resource_metadata)
...
On boot no document is loaded; the first connection to a note triggers `onLoadDocument`, which rebuilds the Y.Doc from `note_docs.snapshot` (V2) plus `note_updates` rows after `snapshot_through_seq` (V1). The client's provider reconnects with a fresh ticket, performs SyncStep1/2 (every pending local update is merged), sends `{t:'baseline'}` and re-derives `saved` from the returned state vector. Restart therefore never duplicates initial content (acceptance "Initialization/reconnection", test `collab.restart-no-duplication`) and never loses an acknowledged revision (acceptance "Durable saving", `collab.durable-ack.chaos`).
## Monorepo layout and package responsibilities

### Workspace conventions

Scope `@iridium/*`, every package private, pnpm 12.4.1 workspaces (`apps/*`, `packages/*`, `tooling/*`) with a single strict `catalog:` (`catalogMode: strict`, `saveExact: true`, `minimumReleaseAge: 4320`, `trustPolicy: no-downgrade`, explicit `allowBuilds: { electron, lefthook, @node-rs/argon2 }`), Turborepo 2.10.12 for the task graph and `boundaries`, TypeScript 7.0.2 plain (no alias) repo-wide, oxlint 1.82.0 + oxlint-tsgolint 7.0.2001 type-aware, oxfmt 0.67.0, tsdown 0.23.0 for Node bundles, Vite 8.3.0 for browser bundles, Vitest 5.0.0, Playwright 1.63.0, Changesets 3.0.2 with one `fixed` group so the server image, web bundle, installers and bridge share one product version, lefthook 2.1.12, commitlint 21.2.2, knip 6.35.1, Renovate `config:best-practices`, `mise.toml` + `.node-version` (24.21.0), `.gitattributes * text=auto eol=lf`.
...
iridium/
  apps/
    ...
    desktop/       @iridium/desktop      Electron 44.3.0: src/main (tsdown ESM), src/preload (tsdown single-file CJS), src/renderer (Vite 8 entry + ElectronHost). electron-builder 26.16.1.
  ...
  tooling/
    ...
    mutation/      @iridium/mutation     Stryker 10 lane: stryker.config.jsonc (the committed config, whose `mutate` globs 10-testing-and-quality.md owns) + vitest.stryker.config.ts (unit project only); its own `typescript` → @typescript/typescript6 alias
  infra/
    compose.yaml  compose.prod.yaml  docker/server.Dockerfile  docker/mysql/{my.cnf,init/01_roles.sql}  caddy/Caddyfile  nginx/iridium.conf  systemd/{iridium.service,iridium-backup.service,iridium-backup.timer}  backup/{backup.sh,restore.sh}  monitoring/{dashboard.json,alerts.yml}
  docs/
  ...
  .github/workflows/{ci,nightly,release}.yml  turbo.json  pnpm-workspace.yaml  package.json  tsconfig.json (solution)  vitest.config.ts  playwright.config.ts  oxlint.config.ts  .oxfmtrc.jsonc  lefthook.yml  commitlint.config.ts  renovate.json  knip.json  .changeset/  mise.toml  .node-version  .gitattributes  .editorconfig  SECURITY.md  CONTRIBUTING.md
```
...
| `@iridium/markdown` | compiled | `iso` | contracts → unified 11.0.5, remark-parse 11.0.0, micromark 4.0.2 GFM extensions, remark-frontmatter 5.0.0, remark-rehype 11.1.2, rehype-highlight 7.0.2 + highlight.js 11.12.0, rehype-sanitize 6.0.0, github-slugger 2.0.0, yaml 2.9.1, mdast-util-to-string 4.0.0, unist-util-visit 5.1.0 | server (projection worker, import scan, export), markdown-react (through the worker), ui | DOM or Node APIs; `remark-stringify` (banned by lint); `gray-matter` (banned) |
...
| `@iridium/server` | app (tsdown bundle) | `server` | contracts, crdt, markdown; testkit (dev only) → fastify 5.12.4 and plugins, @hocuspocus/server 4.7.0, @modelcontextprotocol/{server,node,fastify} 2.0.0, kysely 0.29.5, mysql2 3.24.4, pino 10.3.1, piscina 5.3.2, @node-rs/argon2 2.2.1, rate-limiter-flexible 11.2.0, @aws-sdk/client-s3 3.1131.0 (s3 driver), yauzl/yazl (pin at M0), prom-client (pin at M0) | nothing (`dependents.allow: []`) | browser packages, `@iridium/api-client`, `@iridium/mcp-bridge` |
| `@iridium/web` | app (Vite) | `app` | contracts, ui (and transitively iso/browser) | nothing | server code |
| `@iridium/desktop` | app (tsdown + Vite) | `app` | contracts, ui, api-client, collab-client, mcp-bridge (bundled binary) → electron 44.3.0, electron-updater 6.8.9, electron-log 5.4.4 | nothing | server code; renderer-held credentials |
...
| `@iridium/tsconfig`, `@iridium/oxlint-config`, `@iridium/mutation` | tooling | `tooling` | — | devDependencies of anything | — |
...
app.ts                       buildApp({ mode: 'in-process' | 'child' | 'container' }) — one boot path; plugin order: config → db → security → auth → authz → audit → rest → collab → mcp → ops → jobs
...
mcp/                         plugin.ts (route, hijack, fail-closed), factory.ts (buildIridiumMcpServer), tools/*.ts, resources.ts, verifier.ts, cursor.ts, rate-limit.ts, snippets.ts, instructions.md
...
| `auth/` | credentials, sessions, PAT verifier, `TicketStore`, set-password links, `authenticate()` | `authenticate(request): Principal \| null`, `verifyAccessToken(token): AuthInfo`, `issueTickets()`, `consumeTicket()` | make permission decisions |
...
| `mcp/` | route, verifier, per-request factory, tools, resources, cursors, per-token limits, snippets, `instructions.md` | nothing (leaf) | query tables directly (goes through `ContentReadCore`) |
...
|---|---|---|---|
| `core` | contracts | — (zod only) | `node:*`, `electron`, `react`, DOM globals |
| `iso` | crdt, markdown, api-client, collab-client | core, iso | `node:*` (except type-only), `electron`, `react`, DOM globals |
...
| `tooling` | tsconfig, oxlint-config, mutation | — | — (devDependencies of anything) |
...
```jsonc
{
  ...
  "boundaries": {
    "tags": {
      "core":    { "dependencies": { "allow": [] } },
      "iso":     { "dependencies": { "allow": ["core", "iso"] } },
      "browser": { "dependencies": { "allow": ["core", "iso", "browser"], "deny": ["node", "server", "app"] } },
      "node":    { "dependencies": { "allow": ["core", "iso"], "deny": ["browser", "server", "app"] } },
      "server":  { "dependencies": { "allow": ["core", "iso", "node"], "deny": ["browser", "app"] }, "dependents": { "allow": [] } },
      "app":     { "dependencies": { "allow": ["core", "iso", "browser", "node"], "deny": ["server"] }, "dependents": { "allow": [] } },
      "tooling": { "dependencies": { "allow": [] } }
    }
  }
}
```

The `server` tag allows `node` only so that `@iridium/testkit` can be a devDependency; the package-level `no-restricted-imports` rule bans `@iridium/mcp-bridge` from the server explicitly, and knip 6.35.1 (`--production`) fails when a dev-only package leaks into a production import.
...
| `updates` | `null` | `iridium:updates:{check,install}` + `iridium:event:update-state`, policy from `GET /desktop/update-policy` |
...
| `@iridium/desktop` | `tsdown` (main, preload) + `vite build` (renderer) + `electron-builder` | `out/` installers per platform, `latest*.yml`, blockmaps | release workflow, `iridium desktop-updates publish` |
...
7. **The build is diffed against the schema.** Turborepo runs with `envMode: strict`; `apps/server/turbo.json` lists every key the server reads, and the CI job `check-env-lists` (in `tooling/`) fails when the Turbo list and the `EnvSchema` keys differ in either direction (skeleton B.2).
...
| `ATTACHMENTS_ENCRYPTION=aes256gcm` requires an `ATTACHMENT_KEY_V<n>` keyring; in MVP the value `aes256gcm` is rejected with the message that §G-4 is undecided | Columns exist, the feature does not (skeleton A44) |
...
| `MCP_OAUTH_ENABLED=true` is rejected in MVP | The OAuth authorization server is a post-MVP epic (skeleton A33, §G-1) |
...
interface IridiumConfig {
  ...
  readonly mcp: { enabled: boolean; rateLimitPerHour: number; requestTimeoutMs: number; oauthEnabled: false };
...
| Key | Type | Default | Required | Notes |
|---|---|---|---|---|
| `DATABASE_URL` | `mysql://iridium_app:…@host:3306/iridium?…` | — | **yes** | Application role; both Kysely instances (`dbApp`, `dbPersist`) are built from it; TLS options travel as URL parameters (`ssl-mode`, `ssl-ca`) |
...
| `ATTACHMENT_KEY_V<n>` | 32 bytes, base64 | reserved (§G-4) | rejected unless `ATTACHMENTS_ENCRYPTION=aes256gcm` |
...
| `MCP_OAUTH_ENABLED` | bool | `false` | — | Reserved; `true` is rejected in MVP |
...
| `ATTACHMENTS_ENCRYPTION` | `none \| aes256gcm` | `none` | `aes256gcm` rejected until §G-4 is decided |
...
| `DESKTOP_UPDATES_DIR` | path | `/data/desktop-updates` | electron-updater feed root served at `/desktop/updates/*` |
...
| `IRIDIUM_E2E` | bool | unset | Read by the desktop main process only (disables updater and single-instance lock); listed here because the schema rejects it on the server with a hint |
...
| `desktop_update_policy` | `{policy:'disabled'\|'prompt'\|'silent', channel:'stable'\|'beta', minVersion, requireSecureStorage}` — `requireSecureStorage` is the desktop `safeStorage` requirement of skeleton A26 and lives here, not in `session_policy` | none |
...
| Awareness messages per connection | 10 / s (excess dropped, not closed) | `AWARENESS_MESSAGES_PER_SECOND` | `beforeHandleAwareness` | Cursor updates are throttled to 4 Hz in the editor | no |
...
| Rate-limit stores | `RateLimitStore` (`security/rate-limits.ts`) | `@fastify/rate-limit` 11.2.0 default in-memory store for REST buckets; `rate-limiter-flexible` 11.2.0 `RateLimiterMySQL` on `login_throttle` (persistent, survives restarts) with `RateLimiterMemory` insurance; `mcp/rate-limit.ts` token buckets in a `Map<TokenId, Bucket>` with lazy expiry | counters and bucket levels | REST and MCP counters yes (a restart briefly relaxes limits, acceptable); login throttle no | `@fastify/rate-limit` Redis store; `RateLimiterRedis`; MCP buckets in Redis with Lua token-bucket | Limits become fleet-wide instead of per-process |
...
| Metrics registry | `metrics` (`ops/metrics.ts`) | prom-client (pinned at M0) default registry per process, labelled `instance` | counters/gauges | yes (scrape semantics) | scrape every process | none |
...
| 401 | `token_expired` | PAT past `expires_at` or a rotated token past `rotation_overlap_until` | `WWW-Authenticate: Bearer realm="iridium", error="invalid_token"` (never `resource_metadata`, skeleton A33) |
...
**`/mcp` is the one route exempt from this envelope**, and the exemption is exactly as narrow as its CSRF exemption. It answers transport-level failures with the OAuth-shaped object an MCP client's error path reads and never `application/problem+json`: `401 {"error":"invalid_token","error_description":…}` with `WWW-Authenticate: Bearer realm="iridium", error="invalid_token"` (and no `resource_metadata`) for *every* credential failure — missing header, wrong scheme, unknown id, wrong secret, revoked, overlap elapsed, expired, inactive owner, all deliberately indistinguishable — `503 {"error":"mcp_disabled",…}` with `Retry-After: 60` when the server-wide switch is off, and `500 {"error":"server_error"}` (09-api-reference.md §4.6, 06-mcp-and-agent-access.md). The rows above are therefore what the PAT-enabled ★ REST read routes return for the same conditions (`token_expired` for a well-formed PAT past its expiry or overlap, `unauthenticated` otherwise); `invalid_token` is an RFC 6750 `WWW-Authenticate` parameter and is deliberately **not** a member of the closed `ErrorCode` enum.
...
| Client identification | `X-Iridium-Client: web \| desktop \| bridge \| cli` and `X-Iridium-Client-Version: <semver>` on every request; MCP clients are identified by `clientInfo` and `User-Agent` | A desktop client below `minClientVersion` receives 200 from `/meta` (so it can read the requirement) and shows the "update required" screen; every other route answers 426 is *not* used — the client gates itself, the server never breaks on version alone |
...
| Desktop IPC | No skew: renderer, preload and main ship in one installer; channel map generated from `contracts/desktop-ipc.ts` | A changed channel is caught by the generated typings at build time |
...
|---|---|---|
| 1 | One boot path: `container`, `child` and `in-process` modes differ only in listening, signals and the scheduler | `app.boot-modes.integration.test` boots all three from `buildApp` and asserts identical route tables and plugin order |
| 2 | Every route declares `config.auth`; every mutating cookie-capable route is CSRF-guarded; `/mcp` is `bearerOnly` | route-policy boot assertion (aborts boot) plus `authz.route-policy.test` which registers a deliberately unpolicied route and asserts the boot failure |
...
| 15 | Every singleton interface has one contract suite that any future implementation must pass | `authz-bus.contract.spec.ts`, `ticket-store.contract.spec.ts`, `rate-limit-store.contract.spec.ts`, `search-index.contract.spec.ts`, `storage-driver.contract.spec.ts`, `settings-store.contract.spec.ts`, `job-claim.contract.spec.ts` (decision ARCH-19) |
| 16 | Side effects never precede COMMIT, and audit rows never follow it | `db.transaction-effects.test` (a fault point rolls back after the effects are collected and asserts none ran) and the audit chain verification in `ops.backup-restore.drill.chaos` |
...
| M3 | `mcp/` plugin, PAT verifier, `ContentReadCore` consumers, the stdio bridge |
...
No milestone introduces a new process, a new datastore, a new authentication path or a second read model. That is the property this section exists to protect.
...
| ARCH-16 | All limits are constants in `@iridium/contracts/limits.ts`, and **the "Constant in `limits.ts`" column of the limits table in this section is the plan's sole naming authority** for those identifiers: every other section cites the same spelling, environment-variable names (`MAX_UPLOAD_BYTES`, `COLLAB_DEBOUNCE_MS`, …) are never reused as constant names, and a section that adds a limit of its own registers it in the same object and in the `LimitId` union. Only the operational subset is env-overridable. `GET /meta` gains an additive `limits` object and an additive `policies` object whose members are spelled as 09-api-reference.md §2.2 declares them — `limits: {uploadBytes, importBytes, importFiles, importDepth, noteSoftChars, noteHardChars, wsMaxPayloadBytes, bodyBytes}` and `policies: {passwordMinLength, passwordMaxLength, patMaxLifetimeDays, patAllowNoExpiry, patRotationOverlapMaxHours}` — which are *wire* names and deliberately not the constant names. `limits.single-source` bans stray numeric limits and `limits.policy.unit` (10-testing-and-quality.md) asserts every constant has an enforcement site | Clients must pre-validate against the same numbers and the same field names the server publishes; one naming authority is what makes `limits.single-source` and the `LimitId` exhaustive switch satisfiable instead of contradictory, and 01 §7 already makes 09 authoritative for wire field spelling, so constants and wire fields are two deliberately separate vocabularies |
...
| ARCH-19 | Every singleton interface (`AuthzBus`, `EpochTable`, `TicketStore`, `RateLimitStore`, `SearchIndex`, `StorageDriver`, `SettingsStore`, `JobScheduler` claim protocol) ships with a parameterised `<name>.contract.spec.ts` run against the MVP implementation; a future Redis/S3/Meilisearch implementation must pass the same suite | Makes the "interface now, scale later" promise of skeleton F9 verifiable rather than aspirational |
| ARCH-20 | `DATABASE_BACKUP_URL` (backup role) exists alongside `DATABASE_URL` (app role) and `DATABASE_MIGRATE_URL` (migrator role); `serve` opens only the app role unless `IRIDIUM_MIGRATE_ON_BOOT=true`; CLI commands fail with exit 2 when their role URL is absent (ARCH-22) | Three MySQL roles (A8) need three credentials; the serving process should never hold the DDL or dump credentials |
