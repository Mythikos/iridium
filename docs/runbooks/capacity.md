# The server is refusing work under load
...
**Triggered by.** `IridiumDocBudgetHigh`, `IridiumAdmissionRefusing`, `IridiumDbPoolSaturated`, `IridiumPressureShedding`.

## What this runbook will contain

Reading the admission-control and pool-saturation metrics against the capacity formulas of `docs/ops/deployment.md`'s capacity chapter, and the sizing decisions — more headroom versus genuinely outgrown capacity — that resolve each alert.
