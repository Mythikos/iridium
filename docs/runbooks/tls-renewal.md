# A TLS certificate is expiring soon (air-gapped profile)
...
**Triggered by.** `IridiumTlsCertExpiringSoon`, on the air-gapped deployment profile where Caddy's automatic ACME renewal is not in play.

## What this runbook will contain

The manual or scripted renewal procedure for the `tls internal` / private-CA deployment profile, and confirming the running proxy is actually serving the renewed certificate.
