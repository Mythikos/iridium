# No successful backup in the alert window
...
**Triggered by.** `IridiumBackupMissing` (no successful `iridium backup` in 26 hours).

## What this runbook will contain

Diagnosing why the scheduled backup job did not run or did not succeed, and confirming the fix by watching `iridium_backup_last_success_timestamp` recover.
