# No verified restore within the drill cadence
...
**Triggered by.** `IridiumRestoreDrillStale` (no verified restore within the site's documented drill cadence, default 30 days).

## What this runbook will contain

Why a stale drill is a Critical-impact condition even though nothing is visibly wrong (the deployment is running without a proven recovery path), and the procedure to run and record a fresh `iridium restore --verify` drill before any other change is deployed.
