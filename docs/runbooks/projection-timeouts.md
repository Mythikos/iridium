# Previews or search results are stale or failing to render
...
**Triggered by.** `IridiumProjectionTimeouts`, `IridiumProjectionLag`, "preview won't render" reports, agents seeing stale revisions.

## What this runbook will contain

Distinguishing projection-worker exhaustion from a pathological document, and the reindex path (`iridium reindex --vault <id>`) that rebuilds `note_projections`/`note_search` for an affected vault, with the expected degraded-but-honest staleness reporting while it runs.
