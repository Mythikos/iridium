# MCP factory errors or agent-reported failures
...
**Triggered by.** `IridiumMcpFactoryErrors`, agent reports of 401/500 from the MCP surface.

## What this runbook will contain

Diagnosing a factory error against the two-mount split (`/mcp` versus `/mcp/connect`), telling a credential problem apart from a server-side factory fault, and the kill switches available while investigating.
