# A note has grown past a size limit
...
**Triggered by.** `IridiumSnapshotLarge`, `IridiumCompactionRefused`, `size-exceeded` reports.

## What this runbook will contain

The soft and hard note-size caps, what a refused compaction means for a note (read-only until reduced), and the operator options for a note that has genuinely outgrown the shipped limits.
