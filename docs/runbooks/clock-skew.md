...
**Triggered by.** `IridiumClockSkew`.

## What this runbook will contain

Why clock skew matters to Iridium specifically (collaboration ticket TTLs, token re-validation windows, OAuth code and token lifetimes) and the fix — NTP/chrony configuration on the host — rather than an application change.
