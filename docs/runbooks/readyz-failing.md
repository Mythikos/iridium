# Readiness is failing on a check other than the database
...
**Triggered by.** `IridiumNotReady` with a check other than `db_app`/`db_persist`.

## What this runbook will contain

Reading which named `/readyz` check failed (migrations pending, durability setting, disk, clock skew, and the others `ReadyzCheckName` enumerates) and the fix specific to that check.
