...
**Triggered by.** `IridiumAuditChainBroken`, `IridiumAuditChainUnverified`.

## What this runbook will contain

`verify-chain` to find the first unverifiable prefix — a prefix that does not verify is never archived — and the incident procedure a broken chain triggers.
