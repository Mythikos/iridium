...
- `docs/milestones/CURRENT` and the matching exit record are the authoritative status; this section is a summary of them and is rewritten from the exit record whenever a milestone exits. `CURRENT` names the last *exited* milestone, never the one in progress, so read the working tree before deciding what remains.
- **Last exited milestone: M0 (repository bootstrap, harnesses, spikes), 2026-09-20 UTC**. The formal [M0 exit record](milestones/M0-exit.md) cites [CI 35485518914](https://github.com/Mythikos/iridium/actions/runs/35485518914) at `efa6de83b0cb5b28bc069cc32544787a89471f74` and the marker `v0.0.0` on its record commit. Both MySQL engines, all three Electron runners and report merging pass on Actions. The marker publishes nothing. Branch protection remains owner-deferred under the direct-main rule; local macOS validation remains outstanding.
- **M1 headless kernel: implemented and locally validated; formal exit pending**. Auth, vaults and collaborative note persistence have local proofs for two editors and a viewer, restart recovery, live revocation and truthful durable save acknowledgement, including deletion-only edits. See `docs/milestones/M1-progress.md` for the final58 input identities, per-lane results and limits. `CURRENT` remains `M0`.
- Then M2 structure and search, M3 MCP and OAuth, M4 the shared UI and web host, M5 the supported desktop client, M6 import/export and attachments, M7 the admin console, and M8 operations hardening and the 1.0 release.
- **2026-09-18 review, locally resolved in final58:** the durability, bounded-resource, grants/readiness and acceptance-enforcement findings have fixes and passing regressions. Earlier results remain historical evidence for their recorded inputs. Those local results remain separate from the actual remote checks now cited in the M0 exit record. The M1 version, fixture and exit record follow M0. Local nightly campaigns do not establish remote nightly history.
