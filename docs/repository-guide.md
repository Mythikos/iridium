...
- **Quality**: Vitest 5 (projects `unit`, `guard`, `component`, `integration`, `property`, `chaos`, `contract`, `mcp`, and from M3 the nightly-only `mcp-clients`, which no `ci.yml` job runs), Playwright 1.63, fast-check 4, Testcontainers + Toxiproxy, Stryker 10 (on Vitest 4.1.11 inside `tooling/mutation` only), oxlint 1.82 + tsgolint, oxfmt, knip, Redocly, k6 2.2 for the load lane; from M3, the MCP conformance suite 0.1.16 and MCP Inspector 2.6.0 inside `tooling/mcp-conformance` only (AG11)
- **Codegen**: `pnpm gen` — OpenAPI export and lint, `openapi-typescript` client types, kysely-codegen schema diff, MCP tool schema, the agent documentation (from M3: `docs/agents/tools-reference.md` and the generated snippet and client-status regions of `docs/agents/*.md`), desktop IPC typings, msw skeleton, the database grants script and snapshot (`docs/ops/db-grants.sql`), `docs/non-goals.json`, `docs/acceptance-map.json`
...
apps/
  ...
  e2e/           @iridium/e2e           tag app      Playwright projects: setup, chromium, electron; from M3, mcp-clients/ holds the nightly real-client matrix (Vitest project mcp-clients)
  e2e/mcp-clients/clients/              (M3) npm manifest and lockfile pinning mcp-remote and Claude Code: outside every workspace glob, installed only by nightly.yml under $RUNNER_TEMP, never in the checkout
packages/
...
tooling/
  ...
  mcp-conformance/ @iridium/mcp-conformance tag harness  (M3) dev-only leaf declaring @modelcontextprotocol/conformance and /inspector (no sources); the only workspace where MCP SDK v1 may resolve; it depends on nothing and nothing may depend on it (AG11)
...
docs/
  plan/  adr/  spikes/  ops/  runbooks/  milestones/  spec/  agents/ (M3)
...
- Banned imports per boundary tag: `node:*`, `electron`, `react` and DOM globals in `core`; `node:*` (except type-only), `electron`, `react` in `iso`; `node:*`, `electron` in `browser`; `yjs`/`lib0`/`y-protocols`, with their subpaths, anywhere but `packages/crdt` and the S04 spike harness `spikes/s04-editor-csp`, which is a declared exemption (below). Everywhere, `zod` is imported from its root (`import { z } from 'zod'`) and every `zod/*` subpath is banned, and MCP SDK v1 (`@modelcontextprotocol/sdk` and its subpaths) is banned. `setTimeout` sleeps are banned in test files.
- From M3 (A6, amended 2026-09-25), every `no-restricted-imports` configuration is composed by `restrictedImports()` from the owner table `RESTRICTED_IMPORT_OWNERS` in `oxlint.config.ts`, because an oxlint override replaces an earlier override's options rather than merging them. A carve-out is a `DECLARED_EXEMPTIONS` entry with its reason, never an inline disable directive, and `guards.restricted-imports.guard` proves every file's effective bans against the table. A ban that must cover a package's subpaths is a pattern group (`<pkg>` and `<pkg>/**`), because a `paths` entry matches only the exact specifier. In `apps/server/src` the MCP SDK v2 packages (`@modelcontextprotocol/{server,node,fastify,core,client,express,hono}`) are banned except where an exemption below names them, `@modelcontextprotocol/core/internal` is banned in every file, and `node:async_hooks` is banned outside `mcp/plugin.ts`, the one scoped `AsyncLocalStorage` (ARCH-11). The declared exemptions are the S11 `markdown-it` adapter (`packages/markdown/src/markdown-it/**`); the Yjs family in `packages/crdt` and in the S04 spike harness; ARCH-30's own module and composition root (`apps/server/src/mcp/**`, and `app.ts` registering `mcp/plugin.ts`); the MCP SDK v2 server, node, fastify and core packages under `apps/server/src/mcp/**`, with `async_hooks` in `mcp/plugin.ts`; `@modelcontextprotocol/core` in `oauth/metadata.ts`; and a type-only `@modelcontextprotocol/server` import in `auth/tokens/verify.ts`.
...
- Keep pnpm's default peer resolution (`resolvePeersFromWorkspaceRoot` true). The mutation lane isolates Vitest 4.1.11 by declaring its whole companion family through the `mutation` named catalog. The TypeScript 6 alias appears in exactly three leaf manifests (`tooling/mutation`, `tooling/api-codegen`, `spikes/s11-markdown`); the S11 leaf uses its AST factory/printer solely to reproduce the pinned parser packaging patch. `guards.mutation-lane.guard` asserts the exact inventory; the workspace compiler remains TypeScript 7.
- MCP SDK v1 (`@modelcontextprotocol/sdk` 1.x) resolves only in `tooling/mcp-conformance`, the dev-only leaf that declares `@modelcontextprotocol/conformance` and `@modelcontextprotocol/inspector` from M3 (AG11); `guards.mcp-sdk-confinement.guard` holds `pnpm-lock.yaml`, the importers and the sources to that. The leaf stays inside the one blocking `pnpm audit --audit-level high`: a high advisory in its closure is remedied by moving the pin, then by a parent-scoped override that leaves product closures untouched, then by a dated ignore under D10-15. Tests run its bins through `@iridium/testkit`'s toolchain helper, which resolves them from the leaf and spawns them with `process.execPath`, never through `npx` and never through `pnpm exec` from a Vitest worker; a person reproduces a run with `pnpm --filter @iridium/mcp-conformance exec mcp-inspector --cli …`.
- mcp-remote and Claude Code never enter the pnpm workspace: no catalog entry, no workspace manifest and no `pnpm-lock.yaml` line. From M3, `apps/e2e/mcp-clients/clients/{package.json,package-lock.json}` pins their closure outside every workspace glob, and only `nightly.yml` installs it, from a copy under `$RUNNER_TEMP` with `npm ci --ignore-scripts`, then `npm rebuild` for a package whose install script `apps/e2e/mcp-clients/versions.json` records as reviewed; nothing installs it in the checkout. `MCP_REMOTE_VERSION` in `apps/server/src/mcp/snippets.ts`, `versions.json` and that lockfile must agree (`guards.mcp-client-versions.guard`).
- `knip.jsonc` carries per-workspace `ignoreDependencies` for the dependencies declared ahead of their milestone; delete an entry the moment product code imports the package. From M3, `treatConfigHintsAsErrors` makes a stale entry fail `ci.yml › static`, so the entry is deleted in the commit that adds the import. A package a milestone completes carries no declared-ahead entry at that milestone's exit; a dependency still unimported then is removed from its manifest rather than ignored. Never add an ignore to hide a real finding.
...
MySQL only, two required lines. Kysely migrations under `apps/server/migrations/` run through the server's own migrator under `GET_LOCK('iridium_migrate', 60)`. The ordered list is `docs/plan/03-data-model.md` §14.1, which `migrations.plan-parity.guard` (M3) holds equal to `MIGRATION_NAMES` in `apps/server/src/db/migrations.ts`; `migrations.admission.guard` holds that registry equal to the directory. `apps/server/src/db/schema.ts` is hand-written and diffed against `kysely-codegen` output by `pnpm gen`. Three roles (`iridium_app`, `iridium_migrator`, `iridium_backup`) are created by `infra/docker/mysql/init/01_roles.sh`. The grant migrations record the table-grant matrix's applied or skipped provenance under `schema_meta` keys `acl.<table>` through `applyGrants`, without rewriting prior migration history (03 §2 and §14.1 name them). A missing `GRANT OPTION` or account records a skip: readiness retains an unverified warning, while rolled-back probes fail readiness for demonstrably missing critical application privileges. A DBA applies `docs/ops/db-grants.sql`; M1 retains historical skip metadata rather than claiming a reserved repair command exists. The schema, roles and SQL dialect floor are in `docs/plan/03-data-model.md`; the operator procedure is in `docs/plan/11-operations-and-deployment.md`.
...
The server reads its environment once, in `apps/server/src/config/env.ts` (`EnvSchema`): every documented key with default and floor is in `docs/plan/11-operations-and-deployment.md` and, as it lands, `docs/ops/configuration.md`. Secrets accept `*_FILE` twins; signing keys are versioned keyrings (`AUTH_PASSWORD_PEPPER_V1`, `AUDIT_HMAC_KEY_V1`, `MCP_CURSOR_KEY_V1`). Unknown `IRIDIUM_*` keys are fatal: the server reads exactly `IRIDIUM_MIGRATE_ON_BOOT`, `IRIDIUM_ALLOW_NEWER_SCHEMA`, `IRIDIUM_ALLOW_UNTESTED_MYSQL`, `IRIDIUM_WEB_DIR`, `IRIDIUM_TOOLS_DIR` (from M3; OPS-65) and `IRIDIUM_FAULT`, the last only under `NODE_ENV=test`. `IRIDIUM_E2E` (only the desktop process reads it), `IRIDIUM_ALLOW_NO_ORIGIN_WS` and, from M3, `PUBLIC_HOST` (derived from `PUBLIC_ORIGIN`) are rejected by name. `scripts/check-env-lists.ts` keeps `apps/server/turbo.json`'s env list and the schema keys equal in both directions. `UV_THREADPOOL_SIZE` and the production argon2 pair come from the deployment environment (`infra/compose.prod.yaml`), never from the process.
...
- **Last exited milestone: M2, structure and search.** [M2-exit.md](milestones/M2-exit.md) names implementation `46352e482f7dcbea249ad2f9cceb19bbc911e123`, [CI 36031452149](https://github.com/Mythikos/iridium/actions/runs/36031452149), the full-scope mutation proof, merged coverage, the nightly history that cleared both Schemathesis jobs, and the `v0.2.0` upgrade-fixture provenance. The root and every versioned workspace are `0.2.0`; hand-cut `v0.2.0` targets the exit-record commit, which advances CURRENT to M2. [M2-progress.md](milestones/M2-progress.md) remains the working record of how the milestone got there.
- **M1 and M0 closed first.** [M1-exit.md](milestones/M1-exit.md) names implementation `20494d32a1e1d84697c1a9dd35303caa1d9a297d` and product tag `v0.1.0`; [M0-exit.md](milestones/M0-exit.md) records marker `v0.0.0`, which publishes nothing.
- **Outstanding local checks:** macOS Electron (no local macOS environment) and a fresh local registry audit (egress approval). Remote macOS and remote audit results are recorded separately. Actual nightly history and carried findings are in the exit records and [remote-ci.md](milestones/remote-ci.md); no established green nightly history is inferred.
- The owner requires commits and pushes directly to main, with no branches or pull requests. Branch protection awaits an instruction restoring that workflow. No changeset is pending, and no version-PR job exists.
- M3, MCP and OAuth, is next. It is followed by M4 the shared UI and web host, M5 the supported desktop client, M6 import/export and attachments, M7 the admin console, and M8 operations hardening and the 1.0 release.
