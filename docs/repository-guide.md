...
- **Recorded bootstrap milestone: M0 (repository bootstrap, harnesses, spikes), 2026-09-13; formal exit pending**. Commit `71915f5` and `docs/milestones/M0-exit.md` preserve its local validation record. Remote CI is running; attempts and repairs are in `docs/milestones/remote-ci.md`. A complete green required-check run and `v0.0.0` remain outstanding. Branch protection is owner-deferred under the direct-main rule. Remote macOS Electron passes; local macOS validation remains unavailable. `CURRENT=M0` is the unchanged bootstrap value, not proof of formal exit.
...
- **2026-09-18 review, locally resolved in final58:** the durability, bounded-resource, grants/readiness and acceptance-enforcement findings have fixes and passing regressions. Earlier results remain historical evidence for their recorded inputs. Local validation does not establish the required remote checks, branch protection, macOS Electron evidence or formal M0/M1 tags. Local nightly campaigns do not establish remote nightly history.
