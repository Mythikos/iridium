...
Status: Accepted, 2026-09-17; amended 2026-09-23: the full profiles run at 250 examples rather than 500, because 500 could not complete inside a GitHub-hosted job's six-hour limit and 250 keeps the findings (13-decision-log.md#d12-20-schemathesis-milestone-scope-2026-09-17).
...
The full administrator and outsider profiles remain separate nightly coverage, at 250 examples per operation (amended 2026-09-23; the original 500 never completed a run. Measured, it took 127.7 min, paced by the fixture re-authenticating at the production login limit after the fuzzer revoked its own session 1,161 times, while two 250-example runs took 14.0 and 32.0 min and found all but one of its nine findings plus one it missed). Implementing those profiles early does not add their results to M1's required exit evidence. Moving light coverage to M2 would remove an explicit M1 gate; substituting an unauthenticated smoke would not exercise the authenticated API contract originally specified.
