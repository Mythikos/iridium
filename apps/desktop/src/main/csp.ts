...
export interface CspOptions {
  /**
   * The active profile's origin (`https://host[:port]`), or `null` before a profile is selected —
   * in which case the renderer may reach nothing at all.
   */
  readonly serverOrigin: string | null;
  readonly nonce: string;
  /**
   * `true` while the `IpcWebSocket` fallback is in use (§7.10): the renderer no longer opens a
   * socket, so `wss:` is removed from `connect-src` entirely.
   */
  readonly ipcWebSocket?: boolean;
}
...
function render(directives: readonly string[]): string {
 ...
 * scheme.
 */
export function packagedCsp(options: CspOptions): string {
  return render([
    "default-src 'none'",
    "script-src 'self'",
...
export interface DevelopmentCspOptions extends CspOptions {
  /** The Vite dev server origin the renderer is loaded from, e.g. `http://localhost:5173`. */
 ...
 */
export function developmentCsp(options: DevelopmentCspOptions): string {
