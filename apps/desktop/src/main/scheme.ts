...
export interface AppSchemeHandlerOptions {
  /** Absolute path of the built renderer (`dist/renderer`), resolved with `import.meta.dirname`. */
  readonly rendererRoot: string;
  /** The active profile's origin, read per load so the CSP is never hard-coded. */
  readonly serverOrigin: () => string | null;
  /** `app.isPackaged`. The development CSP is unreachable when this is `true`. */
  readonly isPackaged: boolean;
  /** The Vite dev server origin, when the shell is running against it. */
  readonly devServerOrigin: string | null;
}

function cspFor(options: AppSchemeHandlerOptions, nonce: string): string {
  const base: CspOptions = { serverOrigin: options.serverOrigin(), nonce };
...
export function installAppProtocolHandler(
  session: Session,
  options: AppSchemeHandlerOptions,
): void {
...
export function installDevelopmentCsp(
  session: Session,
  options: { readonly devServerOrigin: string; readonly serverOrigin: () => string | null },
): void {
