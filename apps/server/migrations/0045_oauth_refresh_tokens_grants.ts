/**
 * 0045_oauth_refresh_tokens_grants
 *
 * The `iridium_app` grants for `oauth_refresh_tokens`, rendered from `GRANT_MATRIX` in
...
export async function up(db: MigrationDb): Promise<void> {
  await applyGrants(db, ['oauth_refresh_tokens']);
}
