...
const ENFORCEMENT = {
  ...
  // M3 rows name the owner where the policy decision is made (10-testing-and-quality.md; D06-48,
  // D04-37, D06-49). An environment-backed row is owned by config/env.ts, whose default reads the
  // member, and wired to the consumers of the configured value.
  MCP_TOKEN_BURST_PER_MINUTE: at(
    3,
    SERVER + 'config/env.ts',
    'per-credential burst refused before any hourly point is charged',
    wire('auth/tokens/budget.ts', /\bburstPerMinute\b/),
    wire('auth/plugin.ts', /config\.mcp\.burstPerMinute/),
  ),
  MCP_TOKEN_PER_HOUR: at(
    3,
    SERVER + 'config/env.ts',
    'per-credential hourly budget: the PAT row, else the kind default resolved at charge time',
    wire('settings/merge.ts', /\btokens\.patDefaultRateLimitPerHour\b/, 'defaultRateLimitPerHour'),
    wire('settings/merge.ts', /\boauth\.defaultRateLimitPerHour\b/, 'defaultRateLimitPerHour'),
    wire('auth/tokens/budget.ts', /\bhourlyDefault\b/),
  ),
  MCP_SEARCH_COST: at(3, SERVER + 'auth/tokens/budget.ts', 'weightOf charges search points'),
  MCP_PROCESS_PER_MINUTE: at(
    3,
    SERVER + 'config/env.ts',
    '429 process ceiling across both MCP mounts',
    wire('mcp/rate-limit.ts', /\bprocessCeilingPerMinute\b/),
    // The configured value, read in plugin.ts (which injects it) or in rate-limit.ts itself.
    wire('mcp', /\bmcp\.processCeilingPerMinute\b/),
  ),
  BEARER_MAX_BYTES: at(
    3,
    SERVER + 'auth/tokens/verify.ts',
    "the mount's one 401 before the bearer is parsed, with no database read",
  ),
  MCP_AUTH_FAILURES_PER_IP_PER_MINUTE: at(
    3,
    SERVER + 'mcp/rate-limit.ts',
    '429 before any token read once the failed verifications are spent',
  ),
  MCP_REQUEST_DEADLINE_MS: at(
    3,
    SERVER + 'config/env.ts',
    'in-band deadline answer on the injected Clock, then the backstop',
    wire('mcp/plugin.ts', /config\.mcp\.requestTimeoutMs/, 'deadlineMs'),
    wire('mcp/web-response.ts', /\bdeadlineMs\b/),
    wire('mcp/web-response.ts', /\bdeadlineSignal\b/),
    wire('mcp/tools/register.ts', /\bextra\??\.deadline\b/),
    wire('mcp/resources.ts', /\bextra\??\.deadline\b/),
  ),
  MCP_DEADLINE_GRACE_MS: at(
    3,
    SERVER + 'mcp/web-response.ts',
    '504 deadline_exceeded one grace after the deadline; a drained non-reading peer disconnected',
    wire('mcp/plugin.ts', /\bLIMITS\.MCP_DEADLINE_GRACE_MS\b/),
  ),
  MCP_GET_NOTE_MAX_CHARS: at(3, SERVER + 'mcp', 'bounded note response'),
  MCP_GET_NOTE_HEADING_MAX_CHARS: at(
    3,
    CONTRACTS + 'mcp',
    'input-validation isError above the cap',
  ),
  MCP_HEADING_LIST_MAX: at(3, SERVER + 'mcp/errors.ts', 'bounded heading-not-found list'),
  MCP_HEADING_LIST_ITEM_MAX_CHARS: at(
    3,
    SERVER + 'mcp/errors.ts',
    'listed heading text cut at a grapheme boundary',
  ),
  MCP_MAX_RESOURCE_LINKS: at(3, SERVER + 'mcp', 'bounded resource links'),
  MCP_LIST_VAULTS_PAGE_MAX: at(3, CONTRACTS + 'mcp', 'input-validation isError above the cap'),
  MCP_LIST_VAULTS_PAGE_DEFAULT: at(3, CONTRACTS + 'mcp', 'default list_vaults page'),
  MCP_LIST_VAULTS_FREE_TEXT_MAX_CHARS: at(
    3,
    SERVER + 'mcp/tools/list-vaults.ts',
    'free text beyond the page budget omitted and flagged',
  ),
  // A derived ceiling with no enforcement site of its own: 06 names the test that asserts it.
  MCP_LIST_VAULTS_PAGE_TEXT_MAX_CHARS: at(
    3,
    SERVER + 'mcp/tools.unit.spec.ts',
    'a maximal page stays within the derived text ceiling',
  ),
  MCP_VAULT_INDEX_MAX_ENTRIES: at(3, SERVER + 'mcp', 'bounded vault index'),
  MCP_VAULT_INDEX_RECENT_NOTES: at(3, SERVER + 'mcp/resources.ts', 'bounded recent notes'),
  MCP_COMPLETION_MAX: at(3, SERVER + 'mcp/resources.ts', 'bounded completion values'),
  MCP_MAX_SUBSCRIPTIONS: at(
    3,
    SERVER + 'mcp/plugin.ts',
    'listen streams beyond the handler cap refused',
    wire('mcp/plugin.ts', /\bLIMITS\.MCP_MAX_SUBSCRIPTIONS\b/, 'maxSubscriptions'),
  ),
  MCP_KEEPALIVE_MS: at(
    3,
    SERVER + 'mcp/plugin.ts',
    'keep-alive frame on an open listen stream',
    wire('mcp/plugin.ts', /\bLIMITS\.MCP_KEEPALIVE_MS\b/, 'keepAliveMs'),
  ),
  MCP_INSTRUCTIONS_MAX_BYTES: at(3, SERVER + 'mcp/factory.ts', 'bounded server instructions'),
  MCP_TOOL_DESCRIPTION_MAX_BYTES: at(3, SERVER + 'mcp/factory.ts', 'bounded tool descriptions'),
  PAT_MAX_ALLOWLIST_VAULTS: at(
    3,
    CONTRACTS + 'rest/tokens.ts',
    '422 validation_failed; the consent picker never offers more',
    wire('oauth', /\bLIMITS\.PAT_MAX_ALLOWLIST_VAULTS\b/),
  ),
  PAT_MAX_ACTIVE_PER_USER: at(
    3,
    SERVER + 'auth/tokens',
    '422 validation_failed, errors[].code token_limit_reached',
  ),
  TOKEN_LIST_MAX: at(3, CONTRACTS + 'rest/tokens.ts', '422 validation_failed'),
  TOKEN_LIST_DEFAULT: at(3, CONTRACTS + 'rest/tokens.ts', 'default token page'),
  ADMIN_NOTE_MAX_CHARS: at(3, CONTRACTS + 'rest/tokens.ts', '422 validation_failed'),
  PAT_RATE_LIMIT_PER_HOUR_MIN: at(
    3,
    CONTRACTS + 'settings.ts',
    'exit 2 at load; 422 validation_failed below the floor',
    wire('config/env.ts', /\bSERVER_SETTING_RULES\b/),
  ),
  PAT_RATE_LIMIT_PER_HOUR_MAX: at(
    3,
    CONTRACTS + 'settings.ts',
    'exit 2 at load; 422 validation_failed above the ceiling',
    wire('config/env.ts', /\bSERVER_SETTING_RULES\b/),
  ),
  PAT_LIFETIME_DAYS_MAX: at(
    3,
    CONTRACTS + 'settings.ts',
    'exit 2 at load; 422 validation_failed above the ceiling',
    wire('config/env.ts', /\bSERVER_SETTING_RULES\b/),
  ),
  PAT_ROTATION_OVERLAP_HOURS_MAX: at(
    3,
    CONTRACTS + 'settings.ts',
    'exit 2 at load; 422 validation_failed above the ceiling',
    wire('config/env.ts', /\bSERVER_SETTING_RULES\b/),
  ),
  OAUTH_ACCESS_TOKEN_TTL_MINUTES_MIN: at(
    3,
    CONTRACTS + 'settings.ts',
    'exit 2 at load; 422 validation_failed below the floor',
    wire('config/env.ts', /\bSERVER_SETTING_RULES\b/),
  ),
  OAUTH_ACCESS_TOKEN_TTL_MINUTES_MAX: at(
    3,
    CONTRACTS + 'settings.ts',
    'exit 2 at load; 422 validation_failed above the ceiling',
    wire('config/env.ts', /\bSERVER_SETTING_RULES\b/),
  ),
  OAUTH_REFRESH_DAYS_MAX: at(
    3,
    CONTRACTS + 'settings.ts',
    'exit 2 at load; 422 validation_failed above the ceiling',
    wire('config/env.ts', /\bSERVER_SETTING_RULES\b/),
  ...
  AUDIT_TARGETS_MAX: at(
    1,
    SERVER + 'audit/chain.ts',
    'targets capped on one chained row with the reserved targets_truncated marker',
  ),
  ACCESS_LOG_MAX_NOTE_IDS: at(3, SERVER + 'audit/access-log-writer.ts', 'truncate logged note ids'),
  ACCESS_LOG_FLUSH_INTERVAL_MS: at(
    3,
    SERVER + 'audit/access-log-writer.ts',
    'interval flush on the injected Clock',
  ),
  ACCESS_LOG_FLUSH_ROWS: at(
    3,
    SERVER + 'audit/access-log-writer.ts',
    'early flush at the row count',
  ),
  ACCESS_LOG_QUEUE_MAX_ROWS: at(
    3,
    SERVER + 'audit/access-log-writer.ts',
    'drop the oldest queued rows, counted',
  ),
  ACCESS_LOG_QUEUE_MAX_BYTES: at(
    3,
    SERVER + 'audit/access-log-writer.ts',
    'drop the oldest queued rows by serialised bytes, counted',
  ),
  TOKEN_LAST_USED_FLUSH_INTERVAL_MS: at(
    3,
    SERVER + 'auth/tokens/last-used.ts',
    'last-used flush on the injected Clock in every verifying process',
  ),
  TOKEN_LAST_USED_PENDING_MAX: at(
    3,
    SERVER + 'auth/tokens/last-used.ts',
    'a new token at a full map dropped, counted',
  ),
  SHUTDOWN_FLUSH_MARGIN_MS: at(
    3,
    SERVER + 'ops/shutdown.ts',
    'each close-time flush stops before the drain budget ends',
  ),
  BRIDGE_LIST_REFRESH_MS: at(3, 'packages/mcp-bridge/src', 'refresh the cached lists'),
  BRIDGE_UPSTREAM_TIMEOUT_DEFAULT_MS: at(
    3,
    'packages/mcp-bridge/src',
    'default upstream request timeout',
  ),
  BRIDGE_UPSTREAM_TIMEOUT_MIN_MS: at(
    3,
    'packages/mcp-bridge/src',
    '--timeout-ms below the floor refused',
  ),
  OAUTH_CODE_TTL_SECONDS: at(3, SERVER + 'auth/oauth/codes.ts', 'expired code refused'),
  OAUTH_CONSENT_REQUEST_TTL_SECONDS: at(
    3,
    SERVER + 'oauth/consent-request-store.ts',
    'expired consent refused',
  ),
  OAUTH_MAX_PENDING_CONSENTS: at(
    3,
    SERVER + 'oauth/consent-request-store.ts',
    'evict the oldest pending consent',
  ),
  OAUTH_CIMD_MAX_BYTES: at(3, SERVER + 'oauth/cimd.ts', 'refuse oversized metadata'),
  OAUTH_CIMD_TIMEOUT_MS: at(3, SERVER + 'oauth/cimd.ts', 'abort metadata fetch'),
  OAUTH_CIMD_CACHE_SECONDS: at(3, SERVER + 'oauth/cimd.ts', 'expire metadata cache'),
  OAUTH_CIMD_FETCHES_PER_USER_PER_HOUR: at(
    3,
    SERVER + 'oauth/client-resolution.ts',
    'refuse a metadata fetch beyond the per-user budget',
  ),
  OAUTH_CLIENT_ID_MAX_CHARS: at(
    3,
    CONTRACTS + 'rest/oauth.ts',
    'refuse an overlong client_id',
    wire('oauth/cimd.ts', /\bLIMITS\.OAUTH_CLIENT_ID_MAX_CHARS\b/),
  ),
  OAUTH_DCR_PER_IP_PER_HOUR: at(3, SERVER + 'oauth', '429 registration budget'),
  OAUTH_MAX_UNUSED_CLIENTS: at(3, SERVER + 'oauth/register.ts', 'refuse excess unused clients'),
  OAUTH_UNUSED_CLIENT_TTL_DAYS: at(3, SERVER + 'auth/oauth/clients.ts', 'sweep unused clients'),
  OAUTH_MAX_REDIRECT_URIS: at(
    3,
    SERVER + 'auth/oauth/redirect-uri.ts',
    'refuse oversized redirect list',
  ),
  OAUTH_MAX_REDIRECT_URI_CHARS: at(
    3,
    SERVER + 'auth/oauth/redirect-uri.ts',
    'refuse an overlong redirect URI',
  ),
  OAUTH_CODE_VERIFIER_MIN: at(3, SERVER + 'oauth/pkce.ts', 'refuse a short code_verifier'),
  OAUTH_CODE_VERIFIER_MAX: at(3, SERVER + 'oauth/pkce.ts', 'refuse a long code_verifier'),
  OAUTH_TOKEN_ENDPOINT_PER_IP_PER_MINUTE: at(
    3,
    SERVER + 'oauth',
    '429 temporarily_unavailable on the token and revoke routes',
  ),
  OAUTH_AUTHORIZE_PER_SESSION_PER_HOUR: at(3, SERVER + 'oauth', '429 authorize error page'),
  // Shipped code still enforces these caps through its own literals and constants. Each row moves
  // to the milestone that shipped its cap (M1, and M2 for CURSOR_TTL_SECONDS) once its owner reads
  // the member (10-testing-and-quality.md; D06-48); until then an earlier row would fail today. The
  // moves are lead merge steps, recorded in docs/milestones/M3-progress.md, and this comment goes
  // with the last of them.
  BOUNDED_LIST_MAX: at(
    3,
    CONTRACTS + 'rest',
    'one bounded response with no cursor',
    wire('members/service.ts', /\.limit\(\s*LIMITS\.BOUNDED_LIST_MAX\b/),
    wire('vaults/read.ts', /\.limit\(\s*LIMITS\.BOUNDED_LIST_MAX\b/),
  ),
  LIST_PAGE_MAX: at(3, CONTRACTS + 'rest/tree.ts', '422 validation_failed above the cap'),
  LIST_PAGE_DEFAULT: at(3, CONTRACTS + 'rest/tree.ts', 'default list page'),
  AUDIT_DEDUP_SHORT_WINDOW_MS: at(
    3,
    SERVER + 'auth/audit.ts',
    'one chained failure row per key per short window',
    wire('collab/audit.ts', /\bLIMITS\.AUDIT_DEDUP_SHORT_WINDOW_MS\b/),
  ),
  AUDIT_DEDUP_LONG_WINDOW_MS: at(
    3,
    SERVER + 'auth/audit.ts',
    'one chained denial row per key per long window',
  ),
  AUDIT_DEDUP_KEYS_MAX: at(3, SERVER + 'auth/audit.ts', 'a full gate evicts its oldest key'),
  CURSOR_TTL_SECONDS: at(
    3,
    SERVER + 'pagination/cursor.ts',
    'expired cursor refused with the one cursor text',
  ),
  ...
  CURSOR_PATH_HEAD_MAX_BYTES: at(
    3,
    SERVER + 'pagination/path-keyset.ts',
    'anchored after-key past the literal-path budget, so no issued cursor outgrows its bound',
  ),
